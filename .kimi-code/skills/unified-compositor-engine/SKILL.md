...
description: "Work on the Unified Compositor Engine (UCE): CCP's single Skia render surface that draws video, flash images, subliminals, bouncing text, spiral, brain drain, pink tint, and bubbles as z-ordered layers inside one topmost click-through window per monitor. The engine ALREADY EXISTS and is adopted; UCE scope = ambient/session conditioning ONLY — DTRH/Chaos game mode went web-only in a dedicated WebView window (owner ruling 2026-07-10, board row #6; run-only chaos layers are decommission candidates with ambient carve-outs). Use this skill whenever you touch anything under CCP.Avalonia/Compositor/, any *Layer class, video rendering, overlay z-order, or overlay flicker/lag. Also use it when someone asks why an overlay effect renders wrong, out of order, or not at all in the Avalonia head."
---
