...
description: "Click-through, topmost, transparency, focus, and global-hook input handling for CCP's fullscreen overlays. The product requirement: overlays render over everything while the user keeps using the PC normally ('tinted glass'), with selected regions (bubbles, clickable flashes, lock cards) interactive. Use this skill whenever you touch overlay windows, WS_EX_* ex-styles, P/Invoke on windows, Topmost behavior, mouse hooks, hit-testing, focus stealing, screen-capture exclusion, or any bug where clicks are blocked, leak through, or land on the wrong thing; and when planning Linux/macOS click-through."
---
...
Overlays are passive tinted glass: fullscreen, topmost, invisible to input, invisible to Alt-Tab, never stealing focus. The user must be able to keep typing and clicking into their normal apps while conditioning runs in the background. Specific elements opt back INTO interactivity (chaos bubbles, clickable flash images, lock cards, mandatory-video controls). Getting this wrong either locks the user out of their desktop or makes effects unpoppable.

Authoritative product spec: `ConditioningControlPanel/docs/crossplatform-rebuild-plan.md` section 7.4 ("tinted glass", two-level passthrough, degrade-gracefully off Windows).
...
1. **Static click-through** (most overlays): transparent forever. Spiral, pink tint, brain drain (now compositor layers with no windows of their own), subliminals, keyword highlights.
...
3. **Shared host + global mouse hook** (the UCE model): the window is always click-through; interactivity is app-side hit-testing. A `WH_MOUSE_LL` hook feeds pointer events; services hit-test layer geometry (`FlashLayer.HitTest`, `BubbleLayer.HitTest` in `CCP.Avalonia/Compositor/Layers/`).
...
- **Avalonia** (`CCP.Avalonia/Platform/AvaloniaMouseHook.cs`): event-based and ALWAYS calls `CallNextHookEx`. **It cannot swallow clicks.** This is a real, currently-unfixed parity gap: popping a bubble in the Avalonia head also delivers the click to the app underneath. If your task touches bubble/flash input, decide explicitly whether to fix the hook (give it a swallow path like WPF's) or accept the leak, and record the decision in the task board. Do not paper over it.
