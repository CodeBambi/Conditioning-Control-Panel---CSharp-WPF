...
ok(!(await ev('K.kit.webgl')), 'reading kit.webgl before any draw makes no context');
...
const noNow = await ev(`(() => { const g = document.createElement('canvas').getContext('2d'), r0 = K.kit.debug().renders;
  for (let i = 0; i < 13; i++) K.kit.draw(g, 'backs', 0, 0, 64, 90, { backing: 'small' }); return K.kit.debug().renders - r0; })()`);
ok(noNow <= 1, `13 backs drawn without now in one task cost ${noNow} render (the task clock)`);
const lost = await ev(`(async () => { const ext = K.kit.debugGl().getExtension('WEBGL_lose_context');
  ext.loseContext(); await new Promise((r) => setTimeout(r, 300)); const during = K.kit.webgl, r0 = K.kit.debug().renders;
  ext.restoreContext(); await new Promise((r) => setTimeout(r, 600)); return { during, after: K.kit.webgl, renders: K.kit.debug().renders - r0 }; })()`);
report.contextLoss = lost;
ok(lost.during === false && lost.after === true && lost.renders > 0, `a lost context falls back to 2D and a restored one draws the field again (${JSON.stringify(lost)})`);
