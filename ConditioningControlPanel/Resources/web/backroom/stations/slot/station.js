...
export async function mount(ctx) {
  ...
  function sync() {
    ...
    if (scene) {
      ...
      scene.screen('screen_status', s.melt ? t('br_slot_screen_melt', 'MELT Â· {n} SPINS AT HALF', { n: s.melt })
        : t('br_slot_screen_status', 'WIN {n} Â· FREE {m}', { n: fmt(s.lastWin), m: s.free }));
    }
  }
  ...
  function renderOdds() {
    ...
    table.replaceChildren(...s.lines.map(l => {
      ...
      const odds = l.id === 'emi3' && s.jackpotOdds ? s.jackpotOdds
        : l.id === 'emi2' ? `${l.odds || ''} Â· ${t('br_slot_respin', 'One more look')}` : String(l.odds || '');
  ...
  function land(landed, before) {
    ...
    board(o.pay > 0 ? t('br_slot_screen_win', 'WIN +{n}', { n: fmt(o.pay) })
      ...
      : after.melt ? t('br_slot_screen_melt', 'MELT Â· {n} SPINS AT HALF', { n: after.melt })
