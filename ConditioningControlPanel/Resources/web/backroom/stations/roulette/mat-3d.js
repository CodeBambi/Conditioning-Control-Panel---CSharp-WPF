...
export function createMat3D({ stage, spots, label }) {
  ...
  const chipsMesh=new T.InstancedMesh(chipGeometry,[chipMaterial,chipTop,chipMaterial],12);chipsMesh.count=0;chipsMesh.frustumCulled=false;group.add(chipsMesh);
  ...
  function pick(event){if(disposed)return null;return stage.pick(event,targets)[0]?.object.userData.spot||null;}
  ...
  return {layout(){},hit(){return null;},pick,rectOf,draw,dispose,animate(list,now){anims=list.map(a=>({...a,at:now}));},clearAnims(){anims=[];},
    debug(){return {view:'3d',cells:cells.size,chips:{...lastChips},anims:anims.map(a=>a.kind+':'+a.spot),disposed};}};
}
