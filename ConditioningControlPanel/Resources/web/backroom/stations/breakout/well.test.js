...
test('a well spawns with the spiral brick\'s own field, no picture', () => {
  for (let i = 0; i < 12; i++) {
    ...
    assert.equal(w.r, 70 * 1.33); assert.equal(w.pull, 110 * 1.33); assert.equal(w.captured, null);
...
test('the dev hook still works with no spiral brick alive: the pop leaves from the field centre', () => {
  ...
  const pop = game.spawnWellNow();
  assert.ok(Math.abs(pop.x - game.snapshot().w / 2) < 1 && Math.abs(pop.y - 360) < 1);
  for (let i = 0; i < 90 && !game.snapshot().well; i++) game.step(1 / 60, {});
...
test('the well still captures the ball, orbits it and pays out on release', () => {
  ...
  s.bricks = []; // Isolate orbit payout from incidental brick hits on release.
...
test('born keeps counting while the well holds a ball (the renderer fades the field in on it), age does not', () => {
  ...
  game.launchNow();
  const s = game.snapshot(), b = s.balls[0];
  b.x = w.x + 50; b.y = w.y; b.vx = 0; b.vy = -200; b.stuck = false;   // inside the pull radius while the well is still appearing
  game.step(1 / 60, {});
  assert.equal(w.captured, null, 'an almost invisible new well cannot capture');
  b.x = w.x - 300;
  for (let i = 0; i < 7; i++) game.step(1 / 60, {});
  b.x = w.x + 50; b.y = w.y; b.vx = 0; b.vy = -200;
  game.step(1 / 60, {});
  assert.equal(w.captured, b, 'the visible well captures a passing ball');
