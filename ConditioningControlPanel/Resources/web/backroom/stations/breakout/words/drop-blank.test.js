...
test('DROP holds the sim for three frames, then the ball falls straight down its lane and is never lost', () => {
  const { game } = make();
  const s = game.snapshot();
  const b = airborne(game);
  const fx = game.fireWordNow('DROP');
  assert.ok(fx && fx.key === 'DROP', 'fired');
  assert.ok(s.hitStopMs >= 50 && s.hitStopMs <= 80, `held ${s.hitStopMs} ms`);
  assert.equal(b.vx, 0); assert.ok(b.vy > 0, 'heading down');
  const x0 = b.x, y0 = b.y;
  run(game, 2 / 60, { x: 60 });
  assert.equal(b.x, x0); assert.equal(b.y, y0);
  assert.ok(s.hitStopMs > 0, 'still held');
  assert.equal(s.fx.active[0].t, 0, 'the word clock waits with the sim');
  run(game, 4 / 60, { x: 60 });
...
test('BLANK hides the wall on its curve: up in 0.2 s, held, back over the last 0.5 s, gone after the end', () => {
  assert.equal(blankCurve(0, 1.2), 0); assert.equal(blankCurve(0.1, 1.2), 0.5); assert.equal(blankCurve(0.2, 1.2), 1);
  ...
  game.step(1 / 60, { x: 60 });
  assert.ok(s.mod.hideBricks > 0 && s.mod.hideBricks < 0.2, `just starting (${s.mod.hideBricks})`);
