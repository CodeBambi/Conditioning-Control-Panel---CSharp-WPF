...
test('RELAX: slowmo eases to 0.5, holds, ramps back to 1; the paddle widens 20% and the ball cannot speed up', () => {
  const { game } = make({ words: ['RELAX'] });
  const s = game.snapshot();
  const fx = game.fireWordNow('RELAX');
  assert.ok(fx && fx.key === 'RELAX');
  run(game, 0.05);
  assert.ok(s.timeScale > 0.6 && s.timeScale < 1, `easing in, not a pop: ${s.timeScale}`);
  run(game, 0.95);                                                  // t = 1.0 s, mid-hold
  near(s.timeScale, 0.5, 0.02, 'timeScale mid-word');
  near(s.mod.paddleW, 1.2, 0.01, 'paddleW mid-word');
  near(s.mod.ballSpeed, 0.9, 0.001, 'ballSpeed mid-word');
  assert.equal(s.mod.safe, true, 'safe mid-word');
  run(game, 1.5);                                                   // t = 2.5 s, halfway up the ramp
  assert.ok(s.timeScale > 0.6 && s.timeScale < 0.95, `ramping back: ${s.timeScale}`);
  assert.ok(s.mod.ballSpeed > 0.9 && s.mod.ballSpeed < 1, `ball speed easing back: ${s.mod.ballSpeed}`);
  ...
  assert.equal(timeScaleAt(0), 1); assert.equal(timeScaleAt(1), 0.5); assert.equal(timeScaleAt(3), 1);
});
...
test('LET GO: autopilot and safe while it runs, both off after; the paddle sits under the ball while the input says x=400', () => {
  const { game } = make({ words: ['LET GO'] });
  const s = game.snapshot();
  const fx = game.fireWordNow('LET GO');
  assert.ok(fx && fx.key === 'LET GO');
  game.launchNow();
  const b = s.balls[0]; b.x = 60; b.y = s.h - 20; b.vx = 0; b.vy = 900;
  run(game, 0.5, { x: 400 });
  assert.equal(s.mod.autopilot, true, 'autopilot mid-word'); assert.equal(s.mod.safe, true, 'safe mid-word');
  assert.equal(s.state, 'colour', 'safe: the floor bounced'); assert.equal(s.balls.length, 1);
  assert.ok(Math.abs(s.paddle.x - s.balls[0].x) < 120, `paddle under the ball (paddle ${s.paddle.x | 0}, ball ${s.balls[0].x | 0})`);
  assert.ok(Math.abs(s.paddle.x - 400) > 120, `the player's x=400 was ignored (paddle ${s.paddle.x | 0})`);
  // The word clock pauses through the sim's hit-stops (a ball this fast hits the paddle a lot), so wait for the end rather than count frames.
  let n = 0; while (s.fx.active.length && n++ < 400) game.step(1 / 60, { x: 400 });
  assert.ok(n >= 60 && n < 400, `ended after ${n} frames`);
  assert.equal(s.fx.active.length, 0, 'ended');
  run(game, 0.2, { x: 400 });                                       // the ending frame still carried the word's mods; the next fresh ones are clean
  assert.equal(s.mod.autopilot, false, 'autopilot off after'); assert.equal(s.mod.safe, false, 'safe off after');
  near(s.paddle.x, 400, 1, 'control returned: the paddle follows the input again');
});

test('LET GO render: bloom rises, holds and fades; the hooks draw without throwing, with and without a live ball', () => {
  assert.equal(bloomAt(0), 0); assert.equal(bloomAt(0.3), 1); assert.equal(bloomAt(0.8), 1); assert.equal(bloomAt(1.5), 0);
  assert.ok(bloomAt(0.15) > 0.3 && bloomAt(0.15) < 0.7);
  ...
  for (const balls of [[], [{ x: 200, y: 400, r: 8, vx: 10, vy: 200 }], [{ x: 200, y: 400, r: 8, stuck: true }, { x: 100, y: 500, r: 8, lost: true }]]) {
    const fx = { key: 'LET GO', word: 'LET GO', t: 0, dur: 1.5, phase: 0, heavy: false, x: 240, y: 300, data: {} };
    for (let i = 0; i <= 90; i++) { fx.t = i / 60; fx.phase = Math.min(1, fx.t / 1.5); LETGO.render.over(ctx, { paddle, balls }, fx, R); }
