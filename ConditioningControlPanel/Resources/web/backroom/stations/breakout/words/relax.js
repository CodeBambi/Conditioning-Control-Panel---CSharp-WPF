 ...
 * Play: slowmo eases to 0.5x over 0.25 s, holds until t = 2 s and ramps back
 * over the last second; the paddle widens 20% (eased in and out); the ball
 * cannot speed up (0.9x during the hold, back to 1 over the ramp); the ball is
...
const DUR = 3, SLOW_IN = 0.25, HOLD_TO = 2, RAMP = DUR - HOLD_TO;
...
/** The slowmo curve: 1 -> 0.5 over SLOW_IN, 0.5 until HOLD_TO, 0.5 -> 1 over RAMP. */
export const timeScaleAt = t => t < SLOW_IN ? 1 - 0.5 * smooth(t / SLOW_IN) : t < HOLD_TO ? 0.5 : 0.5 + 0.5 * smooth((t - HOLD_TO) / RAMP);
...
export default {
  ...
  sim: {
    ...
    tick(g, fx, dt, api) {
      ...
      const ramp = t < HOLD_TO ? 0 : smooth((t - HOLD_TO) / RAMP);
      m.ballSpeed = Math.min(m.ballSpeed, 0.9 + 0.1 * ramp);
    },
