...
- Spirals: a spiral brick's pop bursts into a spiral well (radius 70, pull radius 110, 6 s, no bubble skin: the field's own torn rim is the edge, plus a pink halo and a dashed mint rim for presence). When the ball enters the pull radius it is captured: it orbits the centre counter-clockwise at its current speed for 1..2 turns, the spiral fades, the ball leaves on its tangent. The swirl is a Loom field (shared/hypno/loom.js, render-well.js) and it is the brick's own: preset, spin factor and hue ride in on the pop, so the well is the spiral the player saw in the wall. No pictures in the whirlwind. At most one spiral live.
