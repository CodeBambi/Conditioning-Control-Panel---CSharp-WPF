// Re-run on every new wheel.glb drop (Scripts/sync-backroom-wheel-assets.ps1 calls it):
//   node ConditioningControlPanel/Resources/web/backroom/stations/wheel/tests/nodes-check.mjs [path/to/wheel.glb]
...
if (process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1]) {
  const path = process.argv[2] || fileURLToPath(new URL('../assets/wheel.glb', import.meta.url));
  ...
  if (!materials.includes(FACE_MATERIAL)) console.warn(`  missing material ${FACE_MATERIAL} (EMI face stays as modelled)`);
