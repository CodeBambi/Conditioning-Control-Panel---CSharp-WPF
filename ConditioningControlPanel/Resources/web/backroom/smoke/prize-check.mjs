...
ok(await ev(`!window.__backroom.scene.scene.getObjectByName('unlocked_arcade').visible`),'unlocked cabinet hidden before ownership');
ok(await ev(`window.__backroom.scene.scene.getObjectByName('racing_cabinet').visible`),'locked cabinet remains behind the parlour');
...
await sleep(1000);
await ev(`document.querySelector('[data-id="flashes_v2"] .counter-try').click()`);
await sleep(800); await shot('flash-preview-drift.png');
ok(await ev(`Array.from(document.querySelectorAll('[data-id="flashes_v2"] .counter-demo-pic')).some(p=>!p.hidden&&p.complete&&p.naturalWidth>0)`),'flash preview paints an image');
await sleep(3400); await shot('flash-preview-shatter.png');
ok(await ev(`document.querySelectorAll('[data-id="flashes_v2"] [data-kind="shard"]').length===4`),'flash preview shatters into four pieces');
await ev(`document.querySelector('[data-id="bubbles_v2"] .counter-try').click()`);
await sleep(600); await shot('bubble-preview.png');
ok(await ev(`document.querySelectorAll('[data-id="bubbles_v2"] .counter-demo span[data-kind="bubble"]').length===4 && !document.querySelector('[data-id="bubbles_v2"] .counter-demo img')`),'bubble preview uses bubble artwork without GIFs');
...
ok(await ev(`!window.__backroom.scene.scene.getObjectByName('racing_cabinet').visible && window.__backroom.scene.scene.getObjectByName('owned_rt_demo').visible`),'locked cabinet retires and demo model joins owned shelf');
...
ok(displayBounds.every(b=>b.min[0]>3.3&&b.max[0]<6.35&&b.min[2]>-7.8&&b.min[2]<-7.55&&b.max[2]<-6.2),'arcade and shelf backs sit against the north wall, clear of counter and card table');
const statue=await ev("window.__backroom.scene.scene.getObjectByName('statue_spot_1_queen')?.position.toArray()");
ok(statue && statue[0]===5.9 && statue[2]===3.8,'sculpture relocated to east wall');
