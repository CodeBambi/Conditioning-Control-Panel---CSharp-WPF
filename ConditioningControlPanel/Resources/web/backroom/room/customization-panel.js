...
export function createCustomizationPanel({mount,vending,decorations=[],lex=(_,f)=>f,select,getState,restore,preview=()=>{},slotOrder=[0,1,2],hasOwnership=()=>false,onClose=()=>{}}){
  ...
  function choose(i){const keep=chosen>=3&&chosen<6&&i>=3&&i<6;chosen=i;if(!keep){target=i>=15?i-15:0;use=i>=15?'statues':'handles';}view.focus(i);paint();showPreview();equip();}
  ...
  const isLocked=()=>chosen>=BAYS&&chosen<15&&!hasOwnership(items[chosen][0]);
  ...
  function dismount(i){
    ...
    else if(i>=15){s.statues.forEach((piece,spot)=>{if(piece===i-15)pending.push(select('statues',-1,spot));});}
  ...
  function paint(){
    ...
    if(!locked&&fitting())button(hud,fitting().fitted?L('remove','Remove'):L('use','Use'),fitting().fitted?unequip:equip).className='br-custom-remove';
