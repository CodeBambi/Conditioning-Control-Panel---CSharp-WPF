...
function fixture(){const scene=new T.Scene(),camera=new T.PerspectiveCamera(50,.5,.1,100);const model=new T.Group();model.name='prop_monstera';const geometry=new T.BoxGeometry(.5,1,.5),map=new T.Texture(),material=new T.MeshStandardMaterial({map});model.add(new T.Mesh(geometry,material));scene.add(model);let calls=0;const stage={scene,camera,emi:{trigger(){calls++;}}};return {stage,model,geometry,map,material,get calls(){return calls;}};}
