...
export function createVendingView({mount,vending,onSelect}) {
  ...
  const renderer=new T.WebGLRenderer({antialias:true,powerPreference:'default'});renderer.setPixelRatio(budget.dpr(mount.clientWidth,mount.clientHeight));renderer.outputColorSpace=T.SRGBColorSpace;
  ...
  const ray=new T.Raycaster(),pointer=new T.Vector2();let selected=-1,zoom=4,goalZoom=4,disposed=false,elapsed=0;
  ...
  return {focus,update(dt,still){if(disposed)return;elapsed+=dt;if(elapsed<1/30)return;dt=elapsed;elapsed=0;const w=mount.clientWidth,h=mount.clientHeight;if(!w||!h)return;
