...
export function createVendingView({mount,vending,onSelect}) {
  ...
  const outline=new T.BufferGeometry().setFromPoints([new T.Vector3(-.145,-.16,0),new T.Vector3(.145,-.16,0),new T.Vector3(.145,.16,0),new T.Vector3(-.145,.16,0)]);
  const ring=new T.LineLoop(outline,new T.LineBasicMaterial({color:0xffc978,toneMapped:false}));ring.visible=false;scene.add(ring);
  function focus(index){selected=index;model.updateMatrixWorld(true);if(index<0){goal.set(0,1.18,0);ring.visible=false;}else{const bay=bays[index];if(!bay)return;bay.getWorldPosition(goal);goal.y+=.13;goal.z=.35;ring.position.copy(goal);ring.position.z=.49;ring.visible=true;}}
  ...
  },dispose(){disposed=true;renderer.domElement.removeEventListener('click',pick);renderer.dispose();renderer.domElement.remove();ring.geometry.dispose();ring.material.dispose();}};
}
