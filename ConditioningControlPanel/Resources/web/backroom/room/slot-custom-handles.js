import { siliconeRebound, SILICONE_SETTLE_MS, customLeverReturn, LEVER_RETURN_MS, releasedLeverAngle } from './lever-return.js';
...
export async function createSlotCustomHandles({holders,loader,base,sources=[],onCue=()=>{}}) {
  ...
  function update(dt=0,still=false){
    ...
    pulls.forEach((p,index)=>{
      ...
      if(!p.returnCue&&p.t>=DOWN_S+LEVER_RETURN_MS/1000){p.returnCue=true;if(active[index])onCue('silicone',index);}
      if(active[index]){active[index].node.userData.flexTip?.(still?0:siliconeRebound((p.t-DOWN_S)*1000-LEVER_RETURN_MS));}
      if(p.pivot)p.pivot.quaternion.copy(p.rest).multiply(swing.setFromAxisAngle(AXIS,
        p.t<DOWN_S?PULL_MAX*ease(p.t/DOWN_S):active[index]?releasedLeverAngle((p.t-DOWN_S)*1000,PULL_MAX):p.t<DOWN_S+UP_S?PULL_MAX*(1-ease((p.t-DOWN_S)/UP_S)):active[index]?customLeverReturn((p.t-DOWN_S)*1000-LEVER_RETURN_MS):0));
      p.reels.forEach((r,i)=>{
