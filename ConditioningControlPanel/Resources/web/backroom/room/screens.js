 ...
 * under the station id `room` and puts the player's own GIFs on the walls (a
 * GIF shows its first frame; WebGL cannot play it). Anything the feed marks
...
export async function createScreens(o) {
  ...
  art.forEach((t, i) => { if (t) { house.push(t); captions.push(labelTexture(o.ads[i].caption)); } });
  if (!house.length) {
    const c = document.createElement('canvas'); c.width = c.height = 4;
    house.push(prep(new T.CanvasTexture(c))); captions.push(labelTexture(''));
  }
  let gallery = house, custom = false, epoch = 0;
  for (const m of o.meshes) m.material = material(house[0], captions[0]);
  ...
  function update(t) {
    ...
    o.meshes.forEach((mesh, i) => {
      ...
      u.ratioA.value = a.image.width / a.image.height; u.ratioB.value = b.image.width / b.image.height;
  ...
  async function deal(now) {
    ...
    const loaded = (await Promise.all(urls.map((u) => loader.loadAsync(u).then(prep).catch(() => null))))
      .filter((t) => t && t.image && t.image.width > 0);
  ...
  return { update, deal, get pictures() { return custom ? gallery.length : 0; } };
}
