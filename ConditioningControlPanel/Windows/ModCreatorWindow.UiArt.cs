...
namespace ConditioningControlPanel
{
    ...
    public partial class ModCreatorWindow
    {
        ...
        private void BuildUiArtSection()
        {
            ...
            stack.Children.Add(CreateSectionDescription("Hero and card art on the Play door. Wide 16:9 (1376x768) reads best; the cards crop to a short header plate, so use Frame to say which part of the image that plate shows. Three of these are SHARED files, one image feeding several differently-shaped frames: features/dtrh.png is also the dashboard's Rabbit Hole tile, features/lab_quiz_hero.png also paints the Graded Intake header and the quick-launch rail chip, and features/lab_aimemory_hero.png also paints the Companion door's permissions grid. Framing is per surface, so a shared file opens one preview for each surface that actually crops and you frame them independently (dashboard tiles do not crop to order -- see Vault & Dashboard below). features/goon_game.png is the one slot here with no Frame button at all: its card fits the whole image inside a plate instead of cropping it, so there is no part to choose and you will see all of what you supply. The \"lab\" filenames are historical and are never renamed -- the path is the compatibility surface, not the room."));
            stack.Children.Add(BuildUiArtSlotWrap(PlayWallSlots, 150, 84));

            // ── Vault & dashboard ──
            stack.Children.Add(CreateSubHeader("Vault & Dashboard"));
            stack.Children.Add(CreateSectionDescription("Dashboard tiles, Velvet Vault faces and the premium quick-launch rail chips. Wide 16:9 (1376x768) unless the slot says otherwise. Most of these files feed both a dashboard tile and a rail chip, so one upload repaints both -- and a chip is barely wider than it is tall, so it takes a hard crop out of a 16:9 image. Frame the chip rather than fight it: it gets its own preview and its own crop, stored separately from every other surface the same file paints. The dashboard tiles themselves have no Frame button -- the mosaic always centre-crops and does not read framing numbers -- so compose these files with the subject somewhere near the middle."));
