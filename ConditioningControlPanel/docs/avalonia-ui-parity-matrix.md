...
|---|---|---|---|
| 1 | Data/settings persistence + paths | **passed 2026-07-02** | 7 defects found+fixed (`e9501ce8`,`a2d1b9a8`,`b694b543`): secret-store no-op regression, 2 missing migrations, exit-flush gaps, quests serializer, roadmap drift, Roaming-migration orphaning, corrupt-file quarantine. Core tests 108→119; smoke 45 tabs clean for this area; deferred rows on task board ("Discovered by WS0 lot 1"). ◐ PARTIALLY RE-CLOSED (merge `5ce70de6`): the prestige/season-reset LOCAL primitives LANDED in lot 7 — `AvaloniaSkillTreeService.OnSeasonReset` (`:341`) + `PermanentIds` prune (`:357`) + `TrackSkillPointsSpent`/`ReconcileLifetimePointsSpent` (monotonic `lifetime_points_spent`). REMAINING (deferred workstream, ~2800 LOC, server-contract bound): `ProfileSyncService` SERVER profile sync + cloud backup + leaderboard SUBMIT + HMAC. This is the sole large WS0 re-open still open. **PORT IN PROGRESS per `docs/profilesync-port-plan.md`** — **Slices 1–6 landed unwired (Core 199/199, NOT in DI, byte-identical app behavior, no stub):** s1 (`c4b2583a`) seam + 18 DTOs + HMAC + 5t; s2 (`a3215fc9`) injectable-handler test ctor + heartbeat + 4t; s3 (`fafd22b0`) pull + full merge + 3 real `IQuestService` DIM methods + 5t, **independently adversarially reviewed SAFE-TO-BANK**; s4 (`34fc5f16`) push (leaderboard SUBMIT) w/ **fresh-defaults cloud-wipe guard byte-identical to WPF** + 401 `restore-session` recovery + 5t; s5 (`44ea16fa`) cloud backup/restore — **P0 `ExcludedBackupProperties` ported VERBATIM (grep-verified 18==18 set-equality), strip-before-upload confirmed, P0 test asserts the decoded wire body carries no token** + 4t. s6 (`766d8322`) server actions purchase/oopsie/change-name — **independently reviewed SAFE-TO-BANK; economy bug (Math.Max on the post-purchase balance would make skills free) caught + fixed pre-commit, pinned by test** + 6t. Core 199/199. ✅ **RE-CLOSED 2026-07-04 — SLICE 7 SHIPPED, ProfileSync is LIVE:** s7a (`4f051ab0`) GDPR export + easter-egg, DeleteAccount stays auth-owned; s7b (`80e1442`) LIVE WIRING — DI singleton, login/logout/startup wiring, SINGLE heartbeat owner (grep-proven: `AvaloniaV2AuthService.SendHeartbeatAsync` had no timer/callers), §5 sync triggers + bounded 2s exit sync, cloud backup/restore/status UI live (compose local+cloud; WPF identity-preserve on restore), server-authoritative purchase (no local deduction), oopsie caller-side effect, season-recap ProfileLoaded nudge. P0s intact (token-log diff audit clean; strip/fresh-defaults/direct-set tests untouched). Gates: slnf 0 · WPF sln 0 · Core **205/205** · smoke `Findings: 5` w/ no token material · `--verify-video` exit 0. Evidence per checklist step: `docs/profilesync-port-plan.md` §8. NOT exercised: live logged-in purchase/name-change manual run (follow-up). **Row 1 fully re-closed — last `5ce70de6` re-open cleared.** |
...
> 2026-07-04. The port now registers **20 compositor layers** (9 session + 10 chaos + 1
> attention-check); the current full registry + coverage verdicts live in `uce-coverage-audit.md`
> (UCE tracker, reference-only). The snapshot below is intentionally not expanded, to preserve
> the original harness-run record.
