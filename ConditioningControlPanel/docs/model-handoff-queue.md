...
| Q5 | PARTIAL | **Chaos S9 — verification sweep.** `--max-benchmark` 2026-07-05: the 4-min max session incl. Phase 3 heavy chaos ran to COMPLETION (session complete, XP awarded) — **chaos port S5-S8 VALIDATED under max load, no gameplay crash**. STILL OPEN: (1) FPS number not captured — the app segfaults during SHUTDOWN teardown before the results write (see UCE plan Phase E MEDIUM follow-up: VideoLayer.Stop fire-and-forget vmem teardown races app shutdown under a thrashing stream; normal video + clean exit is fine per --verify-visible x3); fix that (or capture FPS another way) then re-run. (2) Side-by-side-vs-WPF human eyeball still pending. | plan §S9 | S | Low |
...
| Q11 | ready | **Chaos gameplay bark members** (2026-07-04 gap sweep). Core `IBarkService` lacks `NotifyChaosDarterCaught`, `NotifyChaosFreezeCaught`, `NotifyChaosTeaseClicked`, `NotifyChaosTeaseDebut` (+2 more; see AvaloniaHeadStubs.cs:842,1229,1257,1291,1310 comments). Pattern established: default-no-op interface member -> raise bark -> AvatarTube subscriber. WPF ground truth: Services/Companion/BarkService.cs:275-316, callers ChaosModeService.cs:1364,2299,2317. | this row + cited comments | S | Low |
