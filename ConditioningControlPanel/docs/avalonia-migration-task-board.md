...
> **BARK-2 (follow-up, medium/wide):** wire the REMAINING contract-E triggers that have port event sources but weren't wired in slice 3 — webcam (Blink/Mouth/Tongue/Stare/Face*), gaze, bubbles (Popped/Missed/Count*), FX (Flash/Subliminal/BrainDrain/MindWipe), KeywordTriggerFired, progression extras (XPChanged/CompanionLevelUp/UserMessageSent/SkillUnlocked/LuckyProc/PinkRush), roadmap, quest extras, quiz, mantra, control (Lockdown/Remote/Idle/Update/Patreon/Discord/Tutorial), AttentionCheck, LockCardCompleted, and the Notify* set (Panic/AppOpened/FeatureOpened/TabNavigated/SettingChanged/AvatarClicked/etc.). Mechanical but wide; wire where a port source exists, note the rest.
