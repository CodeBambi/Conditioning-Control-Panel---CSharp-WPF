...
- **Smoke band**: 17→~13 (4 ChaosRun telemetry findings gone); 44-tab contract unaffected.
...
- **TWO edits exceed the literal owner authorization — owner ack pending before the destructive strip**: (a) `SmokeTestRunner` hub-menu exercise removal (`ExerciseChaosHubMenuAsync`/`ClickHubButtonAsync`) is compilation-forced beyond the "ChaosRun exercise ONLY" wording; (b) `BenchmarkContext.cs:454-475` live-calls `chaos.StartRun` under `--benchmark` and must drop the chaos phase. Both are callers of the deleted native run (cannot compile otherwise).
