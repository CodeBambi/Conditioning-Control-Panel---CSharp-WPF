...
**✅ NATIVE STRIP LANDED 2026-07-11 — merge `36d39fd5` (strip commit `94db82d2`).** Native/Skia DTRH chaos-run REMOVED from the Avalonia head; web is the sole DTRH path. 27 files deleted (21 Avalonia run windows/overlays + 3 Core [`ChaosDraftPool`/`ChaosScoring`/`ChaosSpawnDirector`] + 3 tests), `AvaloniaHeadStubs.cs`/`AvaloniaChaosStubs.cs`/`AvaloniaChaosCatalogs.cs` gutted, `−14,695` lines. Gates (driver-recertified on the merge): slnf **0 err**, WPF **0 err**, **Core 496/0 (NEW FLOOR — supersedes 604)**, smoke **44 tabs / 0 unhandled / ZERO ChaosRun / 10 benign** (1 StartSession + 8 availablesubjects loc-key + 1 ConnectCommand — count-free invariant satisfied), **13 layers** intact. Parity audit: **SAFE** (9/9 invariants). Deviations (advisor-approved, audit-verified, all conservative KEEPS): **D1** kept `ChaosRunRules`(+test) — web `ChaosRunConfig.FromSettings` live-calls it → floor 496 not ~458; **D2** `AvaloniaChaosCatalogs` gutted-not-deleted (seeds kept meta the web reads); **D3** `ChaosRunState` teardown stripped `ChaosLifetimeBoon.Apply`+25 seed lambdas; **D4** `AvaloniaChaosAnim.cs` RESTORED — its `OpacityFade` is LIVE in AvatarTube `CirceEmoteEngine.cs:71/665` (⚠ live AvatarTube peripheral dep — heed in the AvatarTube rebuild). `SmokeTestRunner.cs` owner-acked widened edit applied (ChaosRun + hub-menu exercises removed; 44-tab sweep intact). `run-gates.sh` CORE_TEST_FLOOR 542→496; its stale `SMOKE_FINDINGS_EXPECTED=5` left per advisor (board count-free invariant governs) — follow-up gate/doc fix recommended. REMAINING on row #6: **web-boot verify (headed/owner-gated)** — see BLOCKED task.
...
**STRIP MANIFEST — RECON COMPLETE 2026-07-11 (evidence-verified, advisor-approved; workflow `dtrh_strip_recon`). Trust THIS over the older decommission table below — read-only recon overturned several of its assumptions:**
- **Avalonia whole-file deletes (14)** under `CCP.Avalonia/Chaos/`: hub, hud, overlay, intro, boon-bar, unlock-cards, anim, narrative-director, backdrop, catalogs, toy-button, hud-vm, windowz, menu-art.
- **Core whole-file deletes (EXACTLY 4)**: `ChaosDraftPool`, `ChaosRunRules`, `ChaosScoring`, `ChaosSpawnDirector` (+ their 4 test files).
- **Surgical gut (NOT delete)**: `AvaloniaHeadStubs.cs` — keep `AvaloniaChaosService` + ambient surfaces + the 4 non-chaos services (L3314-3587); gut the run loop (~L578-1750). `AvaloniaChaosStubs.cs` — KEEP `ChaosRunConfig/AvaloniaChaosApp/ChaosMeta/RevealService/RevealIds/ChaosRanks/ChaosUpgrades/ChaosLifetimeBoons/ChaosBubbleVariants`, delete the rest. `ChaosHappyPath` — relocate `BuildFirstRunConfig` (L69-96) into `DtrhNativeEffects.cs` before deleting.
- **ZERO compositor-layer deletions** — `LayerVerification.cs` ExpectLayers (L177-189) treats all 13 chaos layers as AMBIENT; "run-only layers" = empty set. Do not touch `LayerVerification.cs`.
- **5 board candidates OVERTURNED (KEEP — live consumers)**: `ChaosSpawnCatalog`←`AvaloniaBubbleService.cs:462`; `ChaosRunKnobs`←`BubbleEngine.cs:67`; `ChaosEconomy`←`DtrhMetaBridge`; `ChaosSkipClassification`←`DtrhGameHostService`; `ChaosLessons`←`ChaosServices.cs:351/379/421/430`. NEVER glob-delete `Chaos*`.
- **NO DI lines removed**: `IChaosService`→`AvaloniaChaosService` (SCE:281) stays (gutted; App.axaml.cs:456 + LayerVerification resolve it); `ChaosCrashSentinel` (SCE:311), App.axaml.cs:107-109, Windows Program.cs:98-102 survive. Do NOT touch Core `IChaosService` interface (`CCP.Core/App.cs:429`, WPF-shared DIMs) — stub Avalonia members to no-ops.
- **Sever order (5 slices, one commit each, full gates + `--verify-layers`)**: (1) smoke edits + `MainWindowViewModel.ShowChaosOverlaySmokeTest` (unbound L1401-1424) + BenchmarkContext chaos phase + remove `IChaosMetaService.ApplyLifetimeBoons` (`ChaosServices.cs:104/256` + `AvaloniaChaosStubs:570` wrapper); (2) gut HeadStubs; (3) delete windows; (4) split stubs; (5) Core + tests.
- **SmokeTestRunner edit**: call sites ~L214-215 & ~218-231; methods `ExerciseChaosHubMenuAsync`(1088-1165), `ExerciseChaosRunAsync`(1167-1358), `ClickHubButtonAsync`(1360-1399).
- **Test floor**: 146 cases removed → **expected Core 604→458** (certify from the actual post-strip run, never assume).
- **Web boot = sole path (confirmed)**: `LabTabViewModel:463`→`IChaosWebGameService`→`DtrhGameHostService.Launch:100-190`→`Resources/web/dtrh/index.html`; no native fallback (owner-approved).
- **Smoke band (corrected — the recon's "17→~13" derived from a stale count; the certified baseline at `8dbc42ed` was 15 incl. 4 ChaosRun `[Info]`)**: do NOT pin a post-strip count (findings are server-content-coupled). Certify the INVARIANT instead: **44 tabs · 0 unhandled · ZERO ChaosRun findings · remainder ⊆ {StartSession baseline blocker, availablesubjects loc-key misfires, ConnectCommand}** (expected ≈ 11).
- **EXPLICIT-KEEP, verify during exec**: Core `ChaosNarrativeModels.cs`, Avalonia `Chaos/ChaosTuning.cs`, `ChaosSidebarBoon` (`AvaloniaChaosCompat:401`).
- **TWO edits exceed the literal owner authorization — OWNER ACKED 2026-07-11 (both approved)**: (a) `SmokeTestRunner` exception WIDENED to remove ALL DTRH smoke coverage — the ChaosRun exercise AND the hub-menu exercise (`ExerciseChaosHubMenuAsync`/`ClickHubButtonAsync`); (b) `BenchmarkContext.cs:454-475` chaos phase (`chaos.StartRun` under `--benchmark`) dropped. Both are compilation-forced callers of the deleted native run.
- **Workflow-authoring note (lesson):** the 3 fan-out recon agents returned empty conversational replies; only the big-tier synthesis agent produced the manifest (self-gathering all evidence). Future dispatches MUST force the artifact via `opts.schema` or an explicit "RETURN the text as your final message" instruction — `return {manifest}` captured a described-but-not-returned artifact. Manifest recovered from Engram obs #2 (`dtrh-native-strip-manifest`) + #3.
...
#### REBUILD SPEC — 2026-07-11 (evidence + advisor + OWNER-CONFIRMED). Authoritative contract for the redo; supersedes the attempt-1/2 history below.

**OWNER-CONFIRMED behavior (this session):**
- The tube AND the avatar inside it **SCALE WITH THE MAIN WINDOW** — resize the main app bigger/smaller and both grow/shrink proportionally (owner-approved deviation from WPF, which scales to screen only). KEPT from attempt #2; the deviation was never the bug.
- The avatar **HOVERS CENTERED** inside the tube vessel at all times (centered by layout, not code).
- **ATTACHED vs DETACHED sizing (owner-confirmed):** ATTACHED (hooked) = tube+avatar scale proportionally WITH the main window (above). DETACHED (unhooked) = tube+avatar are FREELY user-resizable — "as small or big as we want" — **independent of the main window**; resizing the detached tube must NOT affect the main window's size or layout (no reverse coupling). Detached uses its own manual-scale writer (generous clamp, wider than WPF's 0.5–1.5 step-.25 if feasible; exact bounds a retest tunable), NOT the parent-proportional path.
- The avatar **PULSES / gently MOVES like WPF** — the ±4px sine "bob" on an inner transform. This is a DESIRED liveness cue (owner: "nice if the avatar moves or pulses, gives the idea it's doing stuff"); keep it lively, do NOT freeze the avatar stiff while fixing the size bug. It is cosmetic on an inner transform — it does NOT change window size or position, and is a DIFFERENT thing from the size-oscillation bug.
- MUST be STABLE in SIZE/POSITION: no size oscillation, no drift-to-top, no freeze, no double-avatar on theme switch — while the cosmetic bob keeps playing. Distinguish carefully: kill the 0.527↔0.738 scale flip + reposition storm; KEEP the ±4px liveness bob + the avatar's own video/gif animation.

**ROOT CAUSE (telemetry `.pi/telemetry/avatartube-retest/app-20260711.log` + Engram obs #6 `avatartube-rootcause-2026-07-11`):** two compounding feedback loops produced **5,196 UpdatePosition calls/session** (3,529 sub-frame <16ms, 1,698 same-ms) on the UI thread = **freeze**; `finalScale` oscillated **0.527↔0.738** because `parentRatio` flipped **0.714↔1.000** (ApplySizing read TRANSIENT parentClient mid-move: 1424x714 vs 1405x1000) = **pulse/drift-to-top**; theme switch leaked a 2nd tube = **double avatar**. Loops: (a) parent `WM_WINDOWPOSCHANGED` hook → `UpdatePosition` (conflated z-order with position) + PositionChanged/SizeChanged/Activated all repositioning; (b) **per-`ShowGiggle` 300ms z-order DispatcherTimer** (leaked, one per speech event) → `BringAttachedPairToFront` → `SetWindowPos` → `WM_WINDOWPOSCHANGED` → back into `UpdatePosition`.

**WPF REFERENCE CONTRACT (behavior-only; `AvatarTubeWindow.xaml/.xaml.cs/.Windowing.cs`):**
- Separate top-level Window: `WindowStyle=None, AllowsTransparency=True, Background=Transparent, ShowActivated=False, ShowInTaskbar=False, ResizeMode=NoResize, WindowStartupLocation=Manual`, `WS_EX_TOOLWINDOW`, `Owner` NOT set. `Topmost=false` attached / `true` detached.
- **Sizing (WPF):** scale computed ONCE from PRIMARY screen work area in `OnLoaded` — `_scaleFactor = min(0.85*screenH/1020, 0.30*screenW/780)` clamped `[0.4,1.0]` (`Windowing.cs:425-461`); `ContentViewbox` pinned to `DesignW/H*_scaleFactor`; `SizeToContent` forced `Manual` after first paint (`xaml.cs:483-520,:505`) + `ContentViewbox.SizeChanged` clamps `Window.W/H == Viewbox`(`:532-538`). **THIS SIZE-PIN IS THE ANTI-FREEZE MECHANISM** — never auto-size a transparent layered window (synchronous `HwndTarget.OnResize→CompleteRender` deadlocks the GIF render thread; rationale `xaml.cs:487-501`). *[Our deviation: scale SOURCE = main-window client size, not screen; but the pin + single-writer discipline still applies.]*
- **Position (WPF, attached):** left-docked + vertically centered on the MAIN WINDOW rect: `newLeft = parent.Left - tubeW - (350*scale)`; `newTop = parent.Top + (parent.H - tubeH)/2 + 20*scale` (`Windowing.cs:587-620`). Event-driven ONLY (parent Location/Size/State/Visible/Activated) — NO timer, NO per-frame chase. `UpdatePosition` NO-OPs unless attached (`:588`).
- **Z-ORDER separate from position:** parent `WM_WINDOWPOSCHANGED` hook raises z-order ONLY, never repositions (`Windowing.cs:~254`).
- **Float pulse:** 16ms timer writes ONLY ±4px sine to inner `TranslateTransform` (`AvatarTranslate.Y`), never `Window.Left/Top` (`Windowing.cs:790-808`, `FloatDistance=4 :49`). DPI change stops float 450ms to dodge the layered `CompleteRender` deadlock.
- **Detached:** placed once, user-draggable (drag only on real art, #346 dead-zone guard), `Topmost + ReassertTopmost`; never auto-repositioned. `_currentScale` 0.5–1.5 step .25 user Ctrl+scroll; Attach resets `_currentScale=1.0`.
- **Lifecycle:** `MainWindow.InitializeAvatarTube` → `new AvatarTubeWindow(this)`; `OnClosed` tears down ALL timers/hooks/GIF frames/subscriptions. Parent minimize→pause+hide; restore→resume+show+UpdatePosition. Parent closed: attached→Close, detached→keep floating.

**DESIGN INVARIANTS (the rebuild MUST satisfy):**
1. **SINGLE WRITER** of window size; **SINGLE WRITER** of window position. No other code path touches `Position` or Viewbox W/H.
2. Scale (tube+avatar) = `clamp(parentClientHeight / ReferenceParentHeight, min, 1.0)` × screen-fit cap, computed from **SETTLED** parent geometry only — debounce `SizeChanged` (recompute after resize settles / coalesce to ≤1 per frame via `Dispatcher.Post`), with hysteresis/rounding so a FIXED window size yields ONE stable scale (no 0.714↔1.000 flip). Window size **PINNED** between recomputes (never auto-size — the WPF freeze lesson).
3. **Z-ORDER strictly separated from POSITION:** any parent hook may ONLY raise z-order, never call `UpdatePosition`. ONE shared debounced z-order raise path. **DELETE** the per-`ShowGiggle` timer.
4. Reposition event-driven + coalesced (≤1/frame), never synchronously inside a `WndProc` hook; no follow storm.
5. Avatar centered in tube by layout (Viewbox); ±4px pulse bob on inner transform retained.
6. **Deterministic single-instance lifecycle:** theme/reskin restyles IN PLACE; if recreate is unavoidable, dispose-old-before-show-new. Regression: never two tubes.
7. Preserve public API `SmokeTestRunner.cs` consumes: `Attach/Detach/SetDetached/UpdatePosition`, ctor, `Pause/Resume/StopSpokenAudio`; `UpdatePosition` idempotent + no-op-unless-attached. Peripherals (chat / Circe emotes / reactions / speech / bubbles) **RE-HOSTED not rewritten**.

**ACCEPTANCE GATE (telemetry — cheap grep of `app-*.log` during smoke):**
- Main window NOT resized: `UpdatePosition` fires **< 50 total**, **ZERO** same-ms bursts, `finalScale` **CONSTANT**.
- `finalScale` changes ONLY when the main window actually resizes, tracks smoothly, and SETTLES to ONE value per window size (no A↔B oscillation).
- Zero freeze (no sub-frame storm); zero double-tube on theme switch.
- Plus standard gates: slnf 0err, WPF 0err, Core green, `--smoke-test` 44/0/benign, `--verify-layers`.

**SEQUENCING:** dispatch the rebuild AFTER the DTRH strip lands (two concurrent `--smoke-test` workflows collide — advisor). Worktree-isolated, `port-slice-executor` (big) with THIS spec + obs #6 embedded verbatim, `tokenBudget` set, `port-parity-auditor` + the UpdatePosition-count gate mandatory. Owner headed retest is the only close gate (batch with DTRH web-boot verify).
...
**QUEUE RE-OPENED BY OWNER 2026-07-11 (2nd) · @driver — two owner directives create two new claimable rows.** Fresh-driver full-gate certification at `8dbc42ed` (before this session's work): **slnf 0 err · WPF sln 0 err · Core 604/604 · smoke 44 tabs / 0 unhandled / Findings 15 ⊆ recorded benign drift** (1 StartSession baseline blocker + 9 subjects loc-key misfires + 1 ConnectCommand + 4 ChaosRun `[Info]`). At `8dbc42ed` **604 was the authoritative Core floor**; the DTRH native-strip (below) removed 3 test files → **496 is now the authoritative floor** (certified on merge `36d39fd5`). Both supersede the stale `543` copy-paste in older entries (incl. `1ff8bd49`). Advisor-validated sequencing:
  1. **DTRH native-strip** — **✅ LANDED 2026-07-11 (merge `36d39fd5`, strip `94db82d2`).** slnf/WPF 0 err, Core **496/0**, smoke 44/0/zero-ChaosRun/10-benign, 13 layers; parity audit SAFE; deviations D1–D4 (see row #6 LANDED banner). `SmokeTestRunner.cs` widened edit (ChaosRun + hub-menu) owner-acked. Remaining: web-boot verify (headed — BLOCKED task).
  2. **AvatarTube core redo** — **CLAIMED @driver, spec COMPLETE (see AvatarTube REBUILD SPEC 2026-07-11), dispatching.** Owner-confirmed contract: attached=scale-with-main-window, detached=free-resize-independent, avatar centered-in-tube + liveness pulse, all STABLE (kill reposition-storm + scale-oscillation + dual-tube). Root cause = obs #6; contract = obs #7. Worktree-isolated executor + parity-auditor + UpdatePosition-count telemetry gate. Lands → owner headed retest (cannot close autonomously).
...
| **AvatarTube REDO — RESOLVED, owner-accepted 2026-07-11** (supersedes the FAILED banner ~L1415) | Real root cause was the Circe emote layer transform diverging from WPF (emote rendered larger + shifted up vs neutral pose). Fixed `2662c952` via new pure `AvatarTubeLayoutFold` (CCP.Core, WPF-parity, folds mod scale + emote delta into `EffAvatarScale/Offset`; bottom-anchored origin) + removed the separate center-origin transform. Portrait mode was a red herring, disabled `e935d64a`. Owner watched live build: "looks good". Ground truth for this layered/GPU topmost window = live desktop CopyFromScreen + owner eyes (RTB/AvatarBorder.Bounds telemetry are FALSE POSITIVES). `--verify-avatartube` 93/93 harness added `ec40459`. Merged `73f56bfb` |
| **UI bug batch #2/#4/#5 — LANDED** (owner-verify via round-2 build) | #2 preset Height 80→100 `02ce0978`; #4 WPF blink-folder remove button + RemoveFolder cmd `bf64ec7d`; #5 1-deep no-repeat guard on flash/gif picks (Avalonia-only intentional divergence; WPF FlashService.cs:1858 is true-random, reference-only) `6bb30388`. Audit SAFE |
| **Round-2 dashboard/assets/blink/browser/cards — LANDED, audit SAFE, owner-retest-pending** (branch `round2/dashboard-fixes`, merged `1795a835`) | #1 dashboard tab unlit base theme (App.axaml:253 `AccentGradientBrush`→`AccentTintedBgBrush`) `9a325dae`; #3a per-host WebView2 UserDataFolder so browser loads `96a45308`; blink demo cross-fade slideshow port `9efbcee5`; assets off-thread enumeration+virtualized grid (no freeze on network drive) `aa317abd`; cards AvatarTube per-pixel hit-test in attached mode so 4 left-column cards click `3cc27d50`. Opus-4-5 audit SAFE, WPF untouched |
| **main → feat/crossplatform sync (DTRH v6.3.1 "Deeper Down")** | merge `81d34bec` (origin/main `e55f8370`; 9 commits, 819 files, mostly binary VO/asset). Web DTRH shared → lands free. Reconciliation doc `e18f7d0e` (`docs/wpf-v6.3.1-port-reconciliation.md`): every changed WPF C# file + port disposition. NA-WPF: ChaosMetaStore 3-slot disk, ChaosUpgrades slot facade, ChaosSlotPickerWindow, VideoService LibVLC fixes, MainWindow.Lab. Clean merge, no conflicts |
| **v6.3.1 port parity — LANDED, audit SAFE, owner-retest-pending** (branch `round2/v631-parity`, merged `7e606f2a`) | A `6d594633` UpdateService CurrentVersion+AppVersion=6.3.1 + Deeper Down notes + all 8 csprojs `<Version>`=6.3.1 (stops update popup); B `91858ec6` ChaosMetaState.TutorialStage + DtrhMetaBridge set-num tutorialStage(climb-only≤16)+reset-onboarding purge `cheshire:` (Cheshire VN tutorial persistence); C `373f5822` TrackBubblesPopped(int) on IAchievement/IQuest + DtrhHostOrchestrator.OnRunEnded CALLS both (callerWired=YES; DtRH web-run bubble credit); D `19055f68` DtrhNativeEffects ownedHabitIds. Opus-4-5 audit SAFE. Core floor **561→567** (+AchievementService/DtrhMetaBridge/QuestService tests). Combined gates GREEN: slnf 0 · WPF 0 · Core 567 · smoke exit0/44 tabs · verify-avatartube 93/93 |
| **Round-3 assets recursive + blink live-preview — LANDED, audit SAFE, owner-retest-pending** (branch `round3/assets-blink`, merged `e3dde82a`) | Assets tab now loads media RECURSIVELY (owner request; disk truth: Z:\CCP Vids\videos had 0 top files + 810 in 16 subfolders — old per-folder grid showed none) `c32b616c` (EnumerationOptions RecurseSubdirectories+IgnoreInaccessible, preserves off-thread/virtualized/capped-decode); BlinkTrainer LivePreview stage ported (blink->real in-tab asset cross-fade, proves detection) + consent-version gate fix (IsConsentCurrent flag+version) `2d08537f`. Documented adaptation: in-tab preview is image-only (Avalonia tab has no video MediaElement); full-screen session still plays videos. Opus audit SAFE. Gates: slnf 0 · WPF 0 · Core 567 · smoke exit0/44 tabs |

## AI features backlog (2026-07-12 audit — obs #28; owner directive "make sure AI feats all work")

Audit verdict **MOSTLY-with-gaps**: Core chat SOLID (all 3 transports WORK: cloud codebambi-proxy V2, local Ollama with enrichment+dispatch, OpenAI; companion chat + quiz WORK; cloud omitting dispatch = WPF parity). Gaps below. **Dependency chain: AI-1 engine → AI-2 consumers → AI-10 barks (do NOT batch — hard event dependency on ActivityChanged/StillOnActivity).**

| Row | Tier/Eff | Gap | Port target + WPF cite |
|---|---|---|---|
| **AI-1** awareness engine | big/L | IAwarenessService has ZERO impl + ZERO DI → companion never reacts to foreground window; 'Live' awareness chip/tab are no-ops | Impl in Desktop.Windows head (GetForegroundWindow Win32), IAwarenessService stays Core, DI reg, Linux graceful gap. WPF Services/UI/WindowAwarenessService.cs:59,337-352,451,478-510; MainWindow.Patreon.cs:1146-1154. REUSE existing awareness consent/enable flags — NO new consent surface / NO ConsentVersion bump (STOP+surface if no flag exists). Window titles in-memory only |
| **AI-2** awareness consumers | medium/M | OnActivityChanged reaction + 1-in-4 double-click comment not wired (subsumes AI-9 canned-Giggle stub) | AvatarTube/AvatarTubeWindow.Reactions.cs:79, ChatInput.cs:215-268. BLOCKED until AI-1 |
| **AI-3** moderation file sink | small/S | AvaloniaModerationLog.cs:7-33 Serilog-only stub; no append-only moderation.log CCBill retention file; CoreAiService.cs:44-46 doc claim FALSE | WPF Services/Moderation/ModerationLog.cs (pipe-delimited ISO8601 append-only) |
| **AI-4** AI video master-switch bypass | small/S | AiCommandService.cs:335 gates video on AllowAiVideo only → AI plays video while feature OFF | AND MandatoryVideosEnabled (WPF :189, #512) |
| **AI-5** OpenAI key UI | medium/S | OpenAI transport unreachable: no UI writes 'openai-api-key' ISecretStore secret (OpenAiService.cs:67) | Settings/companion key field via ISecretStore ONLY (never settings.json); dashboard-design skill |
| ~~**AI-6** command param threading~~ ✅ **RESOLVED-AS-DOCS `c2778d44`** | Investigated: the "dropped" params are NOT omissions — they are Core effect-SEAM GAPS. Threaded already: bubbles.frequency, haptic.intensity/duration, mantra.amount, spiral/pink.intensity, bounce.words, subliminal.text. Removed a forbidden TODO + documented each gap with WPF cite. | (done) |
| **AI-6-SEAMS** extend effect seams for AI command params (from AI-6) | medium/M | The AI can TRIGGER flash/subliminal/bubbles but cannot TUNE them: Core IFlashService has no multi-image one-shot with amount/duration/size (WPF FlashService.cs:309 TriggerFlashOnce(amount,durationMs,size)); ISubliminalService.FlashSubliminalCustom lacks opacity(0-60) (WPF SubliminalService.cs:241); IBubbleService.Start() is argless — no runtime frequency override / bypassLevelCheck (WPF BubbleService.cs:160 Start(bool,int?)); bounce bypassLevelCheck. | Extend each Core seam + Avalonia impl to accept the WPF params, then thread from AiCommandService. NOT broken (effects fire), just not AI-tunable. Lower priority than BARK-1 |
| **AI-7** Live Actions feed | medium/M | AppendLiveAction/FormatLiveAction 30-item capped companion feed missing | WPF AiCommandService.cs:67,114-125 |
| **AI-8** haptic vibration-mode | medium/S | synthesized single Test buzz vs ApplyVibrationModeAsync seam | AiCommandService haptic case |
| **AI-10** awareness-gated barks | small/S | **BLOCKED on BARK-1** — becomes a ~14-line ActivityChanged/StillOnActivity subscription once the bark engine exists | WPF BarkService.cs:536-549 |
| **BARK-1** port the bark RULE ENGINE (prerequisite of AI-10; discovered 2026-07-12) | big/M-L | The port's reactive-dialogue system SILENTLY DOES NOT FIRE beyond canned chaos bare-string stubs. `BarkRuleLoader/BarkRuleSet/BarkContext` models ARE ported (CCP.Core/Services/Bark/) + `bark_rules.json` manifests shipped, but ZERO-consumed; `IBarkService` (App.cs:483) exposes only `NotifyChaos*` — no `Raise`/conditions/`Speak` pipeline. Un-stubs the WHOLE companion bark feature, not just awareness. | Port WPF Services/Companion/BarkService.cs: `Raise(trigger,ctx)` :751-817 → `ForTrigger` rule walk → conditions evaluator (app_cluster_eq/app_eq/still_minutes/…) → `DecideLocked`+`BarkState` gate/cooldown/once-only → `Speak` (UI-marshal bubble + PlayBarkVoice audio, companion_audio folder + mod overlay). Preserve existing `NotifyChaos*`->trigger mapping so chaos barks keep working. Archaeology-first (like AI-1). **Audit-miss lesson: the AI audit reported AI-10 as the gap but ASSUMED the engine existed — engine "wired" claims need a consumption grep (ForTrigger/Raise call sites), not model-file existence.** |
| **AI-11** SystemPromptBuilder branches | medium/M | community-prompt branch, active-preset lookup, GetDefaultBambiSpritePrompt fallback, KnownVideoLinks reveal deferred | WPF BambiSprite.cs:510,552-729 |
| **AI-12** stale AI wiring docs | small/S | ServiceCollectionExtensions.cs:173-175 'PENDING for Local/Cloud' false (+2 more) | doc-only |

**LIVE AI claim-priority (advisor 2026-07-12):** ~~Lane1 [AI-4, AI-3, AI-12]~~ ✅ **LANDED `a6cc42b5`** · ~~AI-1 engine~~ ✅ **LANDED `edc176ae`** (2 opus audits SHIP; Core engine+classifier+AppClusterMap verbatim + IForegroundWindowTitleProvider Win32 seam + toggle wired to real IsRunning; consent reuses AwarenessModeEnabled+AwarenessConsentGiven NO new surface/version; privacy: raw title memory-only, WPF debug-log-of-title NOT ported, engine no network; **+29 tests**) → ~~AI-2 consumers~~ ✅ **LANDED `a4e53143`** · ~~AI-5 key UI~~ ✅ **LANDED `acc5c989`** · **AI-10 barks BLOCKED on new BARK-1 (bark rule engine unported)** → **Lane3 COMPLETE**: ~~AI-6~~ resolved-as-docs `c2778d44` · ~~AI-8 haptic~~ `793e86fd` · ~~AI-7 live-actions~~ `1694282f` · ~~AI-11 system-prompt~~ `bf4df001` (all audit SHIP) · AI-6-SEAMS DEFERRED (needs effect-service capability recon first). → **BARK-1 COMPLETE + AI-10 CLOSED** (`d65020df`, 3 slices, all audit SHIP): slice1 engine `34fd72be` (+71 tests) · slice2 AvatarBarkSpeaker `b6f6d645` (+25) · slice3 activate+triggers `d65020df` (+9). Engine started at app init; **awareness barks LIVE (AI-10)**; 36 chaos triggers routed rule-backed (no regression); session/video/progression/achievement/quest/mod wired. Companion reactive-dialogue system revived. **Core floor 741.**

> **BARK-2 DONE** (`f0efb51f`, +9 tests, Core floor **750**, audit DEVIATION fixed): wired 34 remaining triggers WHERE A PORT SOURCE EXISTS — webcam (Blink/Mouth/Tongue/LongStare/FaceLost/FaceFound/TrackingState), gaze (GazePopped/ActiveChanged), BlinkTrainerStateChanged, bubbles (Popped/Missed), FlashDisplayed, mantra (Completed/StreakChanged/StreakBroken), roadmap (StepCompleted/TrackUnlocked), QuizCompleted, LockCardCompleted, QuestsRefreshed, UpdateAvailable, KeywordTriggerFired, SkillUnlocked, PinkRushStarted, lockdown (Activated/Deactivated/CountdownTick), remote (ControllerConnected/CommandReceived), AttentionCheck (Pass/Fail), **UserMessageSent** (raise-then-stamp so the chat-suppression window works), VideoAboutToStart. noDoubleFire+noLeaks verified.
> **BARK-2-FOLLOWUP (deferred, needs new sources/seams — NOT simple wiring):** 21 triggers have NO port event source yet: BouncingText/Subliminal/BrainDrain/MindWipe/BubbleCount* (services expose no events), XPChanged/CompanionLevelUp/PinkRushEnded/BadgeEarned/QuestProgressChanged/IdleState/PersistentMemory/WakeBambi/LuckyProc (no source). PatreonTierChanged + TutorialCompleted sources exist ONLY in CCP.Avalonia (AvaloniaPatreonProvider/AvaloniaTutorialService) — need a new Core seam to reach BarkTriggerWiring. The Notify* UI set (Panic/AppOpened/StreakMilestone/FeatureOpened/TabNavigated/SettingChanged/AvatarChanged/EnhancementApplied/UiAction/LeaderboardViewed) is absent from the port IBarkService entirely + has no call sites. AvatarClicked: NotifyAvatarClicked exists but doesn't route through the engine + clicks_60s needs BarkState (BARK-4). Each item = find/add a source, not just subscribe.
> **BARK-3 DONE** (`97198c9f`, +12 tests, Core floor **762**, grep-verified inert plumbing): added `IsWhisperAudioPlaying`+`MarkWhisperAudio` to the audio seam (`WhisperAudioBusyness` byte-mirrors WPF `AudioService.cs:737-761` only-extend Interlocked window +0.25 tail, NaN/≤0 no-op) and wired `IBarkGateSignals.IsWhisperAudioPlaying` → `IAudioPlayer` (SCE:168-171). **INERT-BUT-READY**: lights up the moment whisper voice audio is ported + calls `MarkWhisperAudio`. Narrator gate moot (DTRH web-only).
> **LINUX OVERLAY X11 BACKEND LANDED** (`f643fa91`, fable-5-hardened, Core floor **820**): `IOverlaySurface` Linux impl = X11-only (XFixes input-shape click-through + `_NET_WM_STATE_ABOVE` post-map topmost) + FallbackBackend. Dead Wayland/layer-shell backend STRIPPED (Avalonia 12.0 X11-only; XWayland covers Wayland sessions). Chained Xlib error trap (no process-kill on BadWindow), own `XOpenDisplay`, per-region = union of CAPTURING layers (spiral+color-filter pass through). Pure routing in `LinuxOverlayBackendPlan.Choose` + tests. **OVERLAY-CAP-FOLLOWUP (medium):** `AvaloniaPlatformCapabilities.SupportsClickThrough` hardcodes `IsWindows` -> Linux overlay is built+correct but DORMANT until this reflects the selected backend (watch DI ordering). E/F slices moot (Wayland stripped); G multi-monitor X11 deferred.
> **WHISPER-VOICE-AUDIO DONE** (`23b28c3b`, GLM impl + opus audit SHIP, +11 tests, Core floor **796**): ported subliminal/flash whisper VOICE audio (was text-only stub) — `WhisperVoicePlayer` + `SettingsBackedWhisperService` (IWhisperService now non-null), `PlayOneShot` audio seam, 2 play sites (AvaloniaSubliminalService.FlashSubliminal + AvaloniaFlashService.TriggerFlashOnce) call `MarkWhisperAudio` — which **ACTIVATES the BARK-3 whisper-overtalk gate** (companion no longer talks over whispers). Mute-respected; WPF-parity clean (only divergence: mute=>no audio+no mark, harmless).
> ~~**NEW FEATURE GAP discovered by BARK-3 (file as own row, MEDIUM):** the port plays NO subliminal/flash/session whisper VOICE audio at all — `AvaloniaSubliminalService` is text-only (compositor SubliminalLayer, zero audio), `AvaloniaFlashService.TriggerFlashOnce` has an unused `playSound` param, session `AudioWhispers` only flips `SubAudioEnabled` with no consumer, and `IWhisperService` (IAvatarTubeServices.cs:28) has NO impl (GetService null). WPF plays whisper voice via `SubliminalService~508-534` + `FlashService:903`. Porting this = a real conditioning feature + it auto-activates the BARK-3 whisper gate. NOT a bark residual — an unported audio feature.~~ ✅ DONE above.
> **Bark backlog remainder DEFERRED (pivoting to Linux per owner):** BARK-4 (BarkState phase_name/blink_count/clicks_60s live fields), BARK-2-FOLLOWUP (source-less triggers needing new Core seams: Patreon/Tutorial/Notify* set).
> **BARK-4 (follow-up, small):** phase_name/phase_is_deepener live fields return null (BarkState unported) and shadow the SessionPhaseChanged ctx vars — port the minimal BarkState phase fields or resolve them from ISessionService. **Core floor 567→596 (new LIVE floor).** Max 2 concurrent while owner tests. Contract: `docs/awareness-contract.md`.

> AI-1 follow-ups: (parity gap) Linux/macOS register no IForegroundWindowTitleProvider → engine Start() no-ops gracefully; a future Linux/macOS provider needs native foreground-title read (X11/Wayland portal / macOS AXAPI). (AI-10 note) WPF `app_clusters.json` override loader deliberately OMITTED (WPF-only companion-audio folder); embedded default cluster tables ARE ported so bark AppCluster/AppId ids work.

| **DEFER — v6.3.1 follow-ups** (not required for version/popup parity) | (a) **report-bug head seam**: WPF DtrhHostService `report-bug` page-msg opens BugReportWindow; port has no equivalent dialog — needs a head seam on the DTRH host route (S-M). (b) **3-save-slot Avalonia picker**: slots are host-side (web game doesn't handle them); to port = ChaosMetaStore multi-slot→Core + extend IChaosMetaService (ActiveSlot/SwitchSlot/AllSlotSummaries/DeleteSlot) + Avalonia picker dialog + wire LabTabViewModel.StartChaosAsync (M-L feature). (c) round2 suggestions: exclude `generate_demos.ps1` from the AvaloniaResource glob (CCP.Avalonia.csproj:16); unlit Dashboard tab still carries base `BorderBrush=PinkBrush` (App.axaml:255, pre-existing — owner to confirm look); `IsLoadingThumbnail` stays true on cancelled items (AssetsTabViewModel.cs:625, harmless); WebView2 hosts log warnings under their own category not WebView2BrowserHost |
