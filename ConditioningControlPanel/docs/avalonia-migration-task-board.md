...
|---|---|---|---|---|
**Claim-priority order (LIVE — the claimer updates this line as rows close/land):** IMP-7 →
IMP-10 → IMP-4 (mechanical/trivial first) → IMP-9 → IMP-6 → IMP-2 (STANDARD) → IMP-8 (JUDGMENT,
flag-don't-force) → major rows #1–#8. IMP-1 DONE `49ec3707`. IMP-11 DONE `28fa06a2`. IMP-5 DONE (this commit;
hash reconciled at next claim). IMP-3 is CONDITIONAL on row #3's libmpv spike gate — never do both.
