...
- [~] WS0 complete: the ENTIRE port reviewed lot by lot (contract + adversarial rubric + optimality), corrections merged, the parity matrix re-earned from a full reset with evidence per row, calibration-port blockage resolved or formally re-scoped. Any lot RE-OPENED by a later merge from main (see the task-board "Sync-from-main" backlogs; merge `5ce70de6` re-opened lots 1/2/3/4/6) must be re-closed before WS0 is done.
  - **STATUS 2026-07-04: all 11 review lots have PASSED first-pass** (parity matrix rows 1–11 all `passed`, evidence per row; calibration re-scoped in lot 5). This continuation closed lots 7–11 (progression, browser/integrations, tabs/dialogs, theming/mods, heads/DI/startup) + fixed the lot-2 ramp P0, with the merge `5ce70de6` backlog folded in. **REMAINING before this box can be checked (re-triaged 2026-07-04, markers made truthful):** the merge re-opens have been reduced to their true surface — **#462 session-summary defer RE-CLOSED** (`410bef87`, row 4); **row 2** interaction-race async dispatch + `ForceReset` ported (residual ordering slices deferred); **row 3** animated-.webp gates DONE (only `SubliminalSolidMode` #461 remains, a bounded overlay-lane fix); **row 6** `EffectPayload.Ambient` is DORMANT (no live bug — `ArmRandomSegment` has zero call sites in the port; moot until the chaos run-engine slice ports). The SOLE genuine LARGE re-open still open is **server-progression-sync `ProfileSyncService`** (~2800 LOC, server-contract bound; the local prestige/season-reset primitives already landed lot 7). So line-284's "re-closed" bar now hinges on: ProfileSync SERVER sync + the bounded SubliminalSolidMode slice + the interaction-race residual, plus the standing deferred workstreams (Ditzy Data PRO, Discord Rich Presence, companion AI, CompanionTab full port, chaos run-engine faithful port, calibration 16-point pipeline).
