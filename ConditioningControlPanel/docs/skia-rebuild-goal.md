...
- [ ] No passive effect window remains in `CCP.Avalonia` (audited); interactive windows are justified. — **AUDITED GREEN 2026-07-05 (`docs/uce-coverage-audit.md`, freshly re-verified post-v6.2.10-merge by @glm5.2): every LIVE passive effect is a compositor layer (20 layers; the window-migration lane is complete — last one was standalone attention-check→`AttentionCheckLayer`). No new LIVE passive window in the 6.2.10 merge (it touched zero `CCP.Avalonia` source). 4 window classes remain (`ChaosEStim`/`EStimGlow`/`VibeTrail`/`SkiaFx`) but are UNWIRED (blocked features, no live caller — not active gaps); the E-Stim one is actively being converted to a layer by a co-agent. Interactive windows justified (audit §C). Box left unchecked only because the 4 unwired window classes still exist as code; functionally satisfied for the live rendering set.**
