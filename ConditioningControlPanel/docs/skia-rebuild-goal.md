...
- [~] WS0 complete: the ENTIRE port reviewed lot by lot (contract + adversarial rubric + optimality), corrections merged, the parity matrix re-earned from a full reset with evidence per row, calibration-port blockage resolved or formally re-scoped. Any lot RE-OPENED by a later merge from main (see the task-board "Sync-from-main" backlogs; merge `5ce70de6` re-opened lots 1/2/3/4/6) must be re-closed before WS0 is done.
  - **STATUS 2026-07-04: all 11 review lots have PASSED first-pass** (parity matrix rows 1–11 all `passed`, evidence per row; calibration re-scoped in lot 5). This continuation closed lots 7–11 (progression, browser/integrations, tabs/dialogs, theming/mods, heads/DI/startup) + fixed the lot-2 ramp P0, with the merge `5ce70de6` backlog folded in. **REMAINING before this box can be checked:** 5 residual `⚠ RE-OPENED` markers in the matrix (lots 1/4/6) whose merge-reopened items were formally RE-SCOPED as deferred workstreams rather than re-closed in place — chiefly **server-progression-sync `ProfileSyncService`** (~2800 LOC absent; the lot-1/lot-7 P0 re-open), **#462** session-summary-vs-video-teardown (lots 2/4), and **`EffectPayload.Ambient`** (lot 6, folded into the Chaos run-engine faithful-port workstream). Those workstreams (+ Ditzy Data PRO analytics, Discord Rich Presence, companion AI, CompanionTab full port, chaos run-engine faithful port, calibration 16-point pipeline) must land (or the re-open markers be cleared) to fully satisfy line-284's "re-closed" bar.
