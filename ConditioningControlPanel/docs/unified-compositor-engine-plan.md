...
| Chaos overlays | ⚠️ migration UNDERWAY (WS2/WP3): NINE passive overlays are layers now, harness-verified — `ChaosFieldFxLayer` (Z=100) + `ChaosDvdLayer` (Z=105) + `ChaosGifCascadeLayer` (Z=110) + `ChaosFlashWashLayer` (Z=115) + `ChaosFxLayer` (Z=118, vignette pulses — migrated 2026-07-05 from `ChaosFxWindow`) + `ChaosCursorGlowLayer` (Z=130, the template) + `ChaosEffectBannerLayer` (Z=140) + `ChaosPopTextLayer` (Z=145) + `ChaosAnnouncerLayer` (Z=150); remaining = 5 passives still on windows (`ChaosWaveTimerOverlay` is LIVE-wired; `ChaosEStim`/`EStimGlow`/`VibeTrail`/`SkiaFx` unwired) + the interactive set (hook click-swallow gap). Full coverage audit: `docs/uce-coverage-audit.md`; queue + recipe in Phase F below |
