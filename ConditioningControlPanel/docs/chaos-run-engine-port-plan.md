...
- **S8 deferrals (frozen-zone STOPs — need seam/engine work first):**
  - **Hint-pill DISPLAY at spawn + HideChaosHints strip.** Core `ChaosBubbleHints.KeyFor/TextFor`
    are ported and `MarkHintLearned` persists the learned-set, but the pill has no render seam:
    the WPF pill is a per-bubble `Border` added to the bubble's visual tree (Bubble ctor
    BubbleService.cs:4158-4187) and `IBubbleRenderer` (frozen) has no hint callback. Wire when a
    bubble-hint render seam exists; then also thread `HideChaosHints(key)` (WPF :1543) to strip the
    on-screen pill on first learn, and add the still-unwired MarkLearned sites: the treat/golden/
    heart/droplet/prism/freeze/heavy CLICK-pop mark (WPF PopByClick :3688 — needs a click-vs-sweep
    signal the engine benign-pop callback lacks, since it also fires for toy sweeps BubbleEngine.cs:342),
    the bound PAIR-clear (WPF OnBoundHalfResolved :1477), and the brittle-dodge (:3333).
  - **Residue / Size-Queen ripple / EStim-shockwave field hazards.** WPF OnDefused/OnBenignPopped
    call `App.Bubbles?.AddChaosResidue`/`TriggerChaosRipple`/`TriggerEStimShockwave`
    (ChaosModeService.cs:2037/2042/1880) which do BOTH gameplay + visual. The Core engine has
    public `AddChaosResidue`/`TriggerChaosRipple`/`TriggerPlayerRipple` but they are NOT on
    `IBubbleService`, and they need the pop point in PHYSICAL px — the frozen engine stamps
    `ChaosLastPopX/Y` in DIP only (no per-pop scaling exposed; `CenterPx = (X+Size/2)*Scaling`
    BubbleEngine.cs:1603). EStim has no layer in this head. Wire by adding DIM `AddChaosResidue`/
    `TriggerChaosRipple` to `IBubbleService` (AvaloniaBubbleService forwards to the engine) + a
    physical ChaosLastPop path, then call the `ChaosFieldResidue`/`ChaosFieldRipple` seams.
  - **Bound-tether + rabbit trail-dot per-frame visuals.** WPF draws these each anim tick from
    live field state (BubbleService `TickBoundPairs` `SetTether` :1522 / `TickFieldHazards` trail
    sweep). The Core engine owns bound-pair geometry and darter `TrailPoints` internally but exposes
    only `ActiveRipples`; per-frame tether/trail rendering needs the engine to surface pair endpoints
    + trail points (a frozen-engine state-exposure change). `ChaosFieldSetTether`/`ClearTether`/
    `TrailDot` seams exist and are ready for that caller.
...
| S6 | (this commit) | slnf 0 · WPF sln 0 · Core **450/450** (+2) · smoke 44 tabs / Findings: 5 / 0 unhandled | port-parity-auditor: all 8 CLAIM rows Verified line-for-line vs WPF, 0 Weakened/0 Falsified, **SHIP**. Missed-site sweep clean: 3 detonation sites route through `FirePayloadForDetonation` (WPF :1353/:1386/:2070), prism→FireScaledPayload, treat→direct Fire (3-way split faithful); VideoEnded subscribe symmetric (run-start + BOTH teardown funnels); `_chaosVideoCapUtc` in per-run reset (no heavy-gate leak). Invariants all PASS. Contested htlink→GifCascade confirmed WPF-faithful (ChaosBubbleVariants.cs:674 — run htlink carries GifCascade kind, so the IsIntrusivePayload(HtLink) ambient-remap branch is dead in a run in BOTH heads). | Collapsed `BuildPayload`→`AvaloniaEffectPayloadFactory.ForVariant` (single map; htlink→GifCascadePayload fix); ported `FireScaledPayload` (OnPayloadFired lesson hook + `DetonationDurationMult` wrap, WPF :2196-2210) and `FirePayloadForDetonation` (ambient soft-remap HtLink-only intrusive→cascade/text coin; heavy gate Video/GifCascade via `_heavyUntilUtc`/`_chaosVideoCapUtc`; StingerForVariant; WPF :2205-2260); RunTick 15s video cap + `OnVideoEndured` on capHit only (WPF :1043-1064). **P3 CLOSED**: new per-instance `EffectPayload.Ambient` (Core, WPF EffectPayload.cs:48-54) threaded `spec.Ambient`→`payload.Ambient`; `VideoPayload.Fire` gates `ArmRandomSegment(15)` on `!Ambient` (per-instance flag, NOT cfg.AmbientMode) — run bubbles arm the random slice, dashboard-trigger videos don't start mid-file (#456/#458). **S1 EXTRA-1 CLOSED**: `ChaosSpawnCatalog.Build(ambient:)` branch (IsLive=false/FuseMs=0/FloatUp/TreatLifeMs=7000/Ambient=true, WPF ChaosBubbleVariants.cs:752-772) + 2 deterministic tests. New `ArmRandomSegment` DIM on IVideoService (no-op default; real impl pre-exists `2572535d`). |
| S7 | (this commit) | slnf 0 · WPF sln 0 · Core **467/467** (+17) · smoke 44 tabs / Findings: 5 / 0 unhandled | port-parity-auditor: SHIP. All 3 P0 invariants VERIFIED with WPF cites — P0-3 XP-pre-multiplier fixed AT THE ROOT (old stand-in paid the spark formula as XP; now score-capped `BaseXp` handed UN-multiplied to an internally-re-multiplying `AddXP`, `finalXp` display-only; pinned by a distinct-economies test), P0-5 sentinel Clear at BOTH EndRun+CleanupAfterRun + unconditional Mark cadence (run-start + `_memSampleTick>=60`), P0-4 meta save untouched/atomic/schema-2-additive. 8 CLAIM rows Verified, 0 Falsified; 2 golden spark values re-derived by hand; tests PIN term-placement not just output. | NEW pure Core `ChaosEconomy` (SparkReward + BaseXp + constants, verbatim WPF ChaosUpgrades.cs:499-514) + 17 tests (spark goldens, first-fall once-ever, drip capstone, trickle placement, completion-bonus scaling, XP cap/floor, spark≠XP). EndRun payout rewrite (order: loop-tip→OnRunCompleted→teardown→XP→previousBest→spark→Sync("run_end")→rankUp→ShowResults, WPF :3168-3198); sentinel Mark cadence + reset; T-10s + Relapse `ExtendOneLoop`; `AwardLoopTip` per-loop via `_waveDetonations` (reset after read; ++ at OnDetonated + touched-Tease, NOT brittle; ×2 clean + GoldScaled, WPF :1752-1755); `CheckComboMilestone` streak_milestone + `freeze_shatter` cues; `ForceShutdown()` wired into `desktop.Exit` funnel (WPF OnExit analog); `EndSlowMo()+EndFreeze()` in EndRun. New `IChaosService.ForceShutdown` DIM no-op. Deferred (non-P0, follow-up): 3 IBarkService bark-line members absent in this head. |
| S8 | (this commit) | slnf 0 · WPF sln 0 · Core **513/513** (+46) · --verify-layers exit 0 (all migrated layers PASS incl. ChaosPopTextLayer + ChaosFieldFxLayer; LockCard honest SKIP) | self-audit vs WPF: hints tables byte-identical to WPF ChaosBubbleHints.cs; ShowPopScore gate/color/offset match WPF :1689-1694; player-ripple visual matches WPF TriggerPlayerRipple :1323-1333. Field-hazard/tether/trail deferrals are frozen-zone STOPs, filed as follow-up rows below. | **Hints:** new pure Core `ChaosBubbleHints` (KeyFor priority ladder + TextFor lexicon, verbatim WPF; input retyped EffectBubbleSpec→ChaosBubbleSpec) + fail-toward-no-hint `IsLearned` predicate + 46 deterministic tests. Head `MarkHintLearned` persists to `ChaosMeta.State.BubbleHintsLearned` (+Save) at the PLAYER-ONLY correct-play callbacks: OnDefused viaChannel && !IsBoundHalf (WPF CompleteDefuse :3669 — a completed player hold, never a toy) and OnTeaseDenied "tease" (WPF :3890 — restraint). The OnBenignPopped click-pop mark (WPF PopByClick :3688) is intentionally NOT wired: the engine benign-pop callback fires for toy sweeps/chains too (BubbleEngine.cs:342), so marking there would over-teach automated pops — deferred to the hint-pill slice which will carry a click-vs-sweep signal. **Pop-text:** head `ShowPopScore(pts)` gated on `ShowPopScores`, DIP→physical via primary-screen scaling (frozen engine stamps ChaosLastPop in DIP, BubbleEngine.cs:258; pop-text layer anchors physical), routed through the existing `ShowChaosPopText` seam; wired at OnBenignPopped treat (:1871), prism (:1838), OnDefused (:2023). **Field-FX:** player-ripple SnapRipple visual fired at `CastRippleWave` alongside the existing gameplay `TriggerPlayerRipple` (WPF :1323-1333). `--verify-layers` unchanged (only added CALLERS). **DEFERRED (frozen-zone STOPs, follow-up rows):** hint-pill DISPLAY (no IBubbleRenderer hint seam — WPF pill is a per-bubble Border in the frozen bubble visual); residue / Size-Queen ripple / EStim-shockwave (gameplay methods AddChaosResidue/TriggerChaosRipple not on IBubbleService + need physical ChaosLastPop the frozen engine doesn't expose + EStim layer absent); bound-tether + trail-dot per-frame visuals (engine field-hazard geometry unexposed; only ActiveRipples is public). |
| S9 | — | — | — | — |
