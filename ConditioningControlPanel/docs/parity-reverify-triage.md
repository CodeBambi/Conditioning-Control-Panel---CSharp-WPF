...
| Overlays per-region click-through (only color-filter/spiral pass; every other layer captures) | **RESOLVED 2026-07-09** (team review) | DoD #3b `AvaloniaMouseHook` click-swallow decision MADE: the hook MUST get a WPF-style swallow path driven by the compositor capture mask; only color-filter/spiral regions stay click-through. Remaining = implementation + headed verification, not a decision. See task-board row 'Per-region UCE input mask + hook swallow'. | Implement, then headed verify: clicks land through color-filter/spiral regions; clicking a flash/subliminal/brain-drain region blocks the desktop. |
