...
internal sealed class BubbleCountLoop : HelpLoopScene
{
    ...
    private static readonly Brush DimBrush = LoopPalette.Solid("#07040f99");
    private static readonly Brush OkText = LoopPalette.Solid("#10231d");
    ...
    public override void Draw(LoopFrame f, double t)
    {
        ...
        if (askO > 0)
        {
            ...
            var edge = new Pen(p.Border, 1);
            for (int i = 0; i < Options.Length; i++)
            {
                bool ok = i == 2 && t > 4550;
                var r = new Rect(RowX + i * 50, RowY, 40, 40);
                dc.DrawRoundedRectangle(ok ? p.Mint : p.Panel2, ok ? new Pen(p.Mint, 1) : edge, LoopFrame.Inset(r, .5), 9.5, 9.5);
