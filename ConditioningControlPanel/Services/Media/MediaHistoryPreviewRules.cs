...
namespace ConditioningControlPanel.Services
{
    ...
    public sealed class MediaPreviewPlan
    {
        ...
        /// <summary>Offer a deliberate one-off fetch so the user can still see what played.</summary>
    ...
    public static class MediaHistoryPreviewRules
    {
        ...
        /// <c>File.Exists</c> and <paramref name="remoteCached"/> its
        /// <c>RemoteMediaCache.IsCached</c>; both are passed in so this stays testable and free of
        /// disk and network.
        /// </summary>
        public static MediaPreviewPlan Plan(string? path, MediaType type, bool localExists, bool remoteCached)
        {
            ...
            if (IsRemote(path))
            {
                ...
                return new MediaPreviewPlan
                {
                    ...
                    CanOpenFile = false,
                    CanCopyLink = true,
                    CanOpenSource = IsBrowsable(path),
                    CanLoadPreview = !canDraw && type == MediaType.Image,
                };
            }
