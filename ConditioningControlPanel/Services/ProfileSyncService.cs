...
namespace ConditioningControlPanel.Services
{
    ...
    public class ProfileSyncService : IDisposable
    {
        ...
        /// </summary>
        public bool IsSyncEnabled => _syncEnabled && App.Patreon?.IsAuthenticated == true;
        ...
        private void MergeCloudProfile(CloudProfile cloudProfile)
        {
            ...
            if (cloudProfile.Xp > settings.PlayerXP)
            {
                App.Logger?.Information("Cloud has higher XP ({CloudXp} vs {LocalXp}), updating local",
                    cloudProfile.Xp, settings.PlayerXP);
                settings.PlayerXP = cloudProfile.Xp;
                needsSave = true;
            }
