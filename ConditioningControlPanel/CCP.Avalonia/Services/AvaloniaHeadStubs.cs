...
public sealed class AvaloniaBarkService : IBarkService
{
    ...
    /// <param name="engine">Optional rule engine. When supplied, each NotifyChaos* routes through the
    /// engine when a rule exists for its WPF trigger (the rule bark fires, gates apply); otherwise the
    /// existing random-phrase <see cref="BarkRequested"/> fallback keeps the chaos companion audible
    /// (BARK-1 slice 3 regression guard — WPF Services/Companion/BarkService.cs:261-365). Null keeps the
    /// legacy bare-string behavior (heads/tests that do not resolve the engine).</param>
    public AvaloniaBarkService(BarkEngine? engine = null) => _engine = engine;
    ...
    // ---- BARK-1 slice 3: each NotifyChaos* routes through the rule engine when a rule exists for its
    //      WPF trigger (ctx vars per contract E, docs/bark-engine-contract.md), else keeps the
    //      random-phrase RaiseBark fallback so chaos barks never go silent. Exactly one path per fire. ----

    public void NotifyChaosDollhouseFirstOpen() => Chaos("ChaosDollhouseFirstOpen", "chaos.dollhouse");
    public void NotifyChaosRevealFlash(string id) => Chaos("ChaosRevealFlash", "chaos.reveal", ctx => ctx.Set("element", id ?? ""));
    public void NotifyChaosResultsShown(double score, double best, double delta, bool pb,
        ...
        => Chaos("ChaosResultsShown", "chaos.results", ctx =>
        {
            ctx.Set("score", score);
            ctx.Set("best_score", best);
            ctx.Set("pb_delta", delta);
            ctx.Set("is_pb", pb);
            ctx.Set("defused", defused);
            ctx.Set("detonated", detonated);
            ctx.Set("best_combo", bestCombo);
            ctx.Set("difficulty", difficulty ?? "");
        });
    public void NotifyChaosRankUp(string rankName) => Chaos("ChaosRankUp", "chaos.rankup", ctx => ctx.Set("rank", rankName ?? ""));
    public void NotifyChaosGiftGiven() => Chaos("ChaosGiftGiven", "chaos.gift");
    public void NotifyChaosDraftAutopick() => Chaos("ChaosDraftAutopick", "chaos.autopick");
    public void NotifyChaosRunStarted(string difficulty) => Chaos("ChaosRunStarted", "chaos.runstarted", ctx => ctx.Set("difficulty", difficulty ?? ""));
    public void NotifyChaosFocusLow() => Chaos("ChaosFocusLow", "chaos.focuslow");
    public void NotifyChaosGoldFirst() => Chaos("ChaosGoldFirst", "chaos.goldfirst");
    public void NotifyChaosDuoDemo() => Chaos("ChaosDuoDemo", "chaos.duodemo");
    // S5 draft/wave choreography barks (WPF Services/Companion/BarkService.cs:247-290).
    public void NotifyChaosWaveEscalated(int wave) => Chaos("ChaosWaveEscalated", "chaos.waveescalated", ctx => ctx.Set("wave", wave));
    public void NotifyChaosWaveCleared(int wave) => Chaos("ChaosWaveCleared", "chaos.wavecleared", ctx => ctx.Set("wave", wave));
    public void NotifyChaosBoonPicked(string boon) => Chaos("ChaosBoonPicked", "chaos.boonpicked", ctx => ctx.Set("boon", boon ?? ""));
    public void NotifyChaosCursePicked(string boon, string rarity, double runMultBonus)
        => Chaos("ChaosCursePicked", "chaos.cursepicked", ctx =>
        {
            ctx.Set("boon", boon ?? "");
            ctx.Set("rarity", rarity ?? "");
            ctx.Set("run_mult_bonus", runMultBonus);
        });
    public void NotifyChaosBoonSkipped(int shieldsNow) => Chaos("ChaosBoonSkipped", "chaos.boonskipped", ctx => ctx.Set("shields_now", shieldsNow));
    public void NotifyChaosActChanged(int act, int wave)
        => Chaos("ChaosActChanged", "chaos.actchanged", ctx => { ctx.Set("act", act); ctx.Set("wave", wave); });
    // Q11 chaos gameplay barks (WPF Services/Companion/BarkService.cs:275-318).
    public void NotifyChaosEndingSoon() => Chaos("ChaosEndingSoon", "chaos.endingsoon");
    public void NotifyChaosDarterCaught(double points, int combo, bool quick)
        => Chaos("ChaosDarterCaught", "chaos.dartercaught", ctx => { ctx.Set("points", points); ctx.Set("combo", combo); ctx.Set("quick", quick); });
    public void NotifyChaosFreezeCaught(double points, int combo)
        => Chaos("ChaosFreezeCaught", "chaos.freezecaught", ctx => { ctx.Set("points", points); ctx.Set("combo", combo); });
    public void NotifyChaosComboMilestone(int combo, string difficulty)
        => Chaos("ChaosComboMilestone", "chaos.combomilestone", ctx => { ctx.Set("combo", combo); ctx.Set("difficulty", difficulty ?? ""); });
    public void NotifyChaosComboBig(int combo, double threshold)
        => Chaos("ChaosComboBig", "chaos.combobig", ctx => { ctx.Set("combo", combo); ctx.Set("threshold", threshold); });
    public void NotifyChaosTeaseDebut() => Chaos("ChaosTeaseDebut", "chaos.teasedebut");
    public void NotifyChaosTeaseDenied(int deniedCount) => Chaos("ChaosTeaseDenied", "chaos.teasedenied", ctx => ctx.Set("denied_count", deniedCount));
    public void NotifyChaosTeaseClicked() => Chaos("ChaosTeaseClicked", "chaos.teaseclicked");
    public void NotifyChaosTeaseDeniedStreak(int deniedCount) => Chaos("ChaosTeaseDeniedStreak", "chaos.teasedeniedstreak", ctx => ctx.Set("denied_count", deniedCount));
    // S2c-2a bubble-economy / lesson / run-end barks (WPF Services/Companion/BarkService.cs).
    public void NotifyChaosBenignPopped(string variant, string payload, int combo)
        => Chaos("ChaosBenignPopped", "chaos.benignpopped", ctx => { ctx.Set("variant_id", variant ?? ""); ctx.Set("payload", payload ?? ""); ctx.Set("combo", combo); });
    public void NotifyChaosBubbleDefused(int combo, string variant, string difficulty)
        => Chaos("ChaosBubbleDefused", "chaos.defused", ctx => { ctx.Set("combo", combo); ctx.Set("payload", variant ?? ""); ctx.Set("difficulty", difficulty ?? ""); });
    public void NotifyChaosBubbleDetonated(string variant, double strength, double runDetonations, int combo, string difficulty)
        => Chaos("ChaosBubbleDetonated", "chaos.detonated", ctx =>
        {
            ctx.Set("payload", variant ?? "");
            ctx.Set("strength", strength);
            ctx.Set("run_detonations", runDetonations);
            ctx.Set("combo", combo);
            ctx.Set("difficulty", difficulty ?? "");
        });
    public void NotifyChaosBubbleDetonatedAbsorbed(string variant, double strength, double runDetonations, int combo, string difficulty, int shields)
        => Chaos("ChaosBubbleDetonatedAbsorbed", "chaos.detonatedabsorbed", ctx =>
        {
            ctx.Set("payload", variant ?? "");
            ctx.Set("strength", strength);
            ctx.Set("run_detonations", runDetonations);
            ctx.Set("combo", combo);
            ctx.Set("difficulty", difficulty ?? "");
            ctx.Set("shield_absorbed", true);
            ctx.Set("shields_left", shields);
        });
    public void NotifyChaosDefuseFirst() => Chaos("ChaosDefuseFirst", "chaos.defusefirst");
    public void NotifyChaosDefuseNoFocus() => Chaos("ChaosDefuseNoFocus", "chaos.defusenofocus");
    public void NotifyChaosDefuseRelease() => Chaos("ChaosDefuseRelease", "chaos.defuserelease");
    public void NotifyChaosClickDetonate() => Chaos("ChaosClickDetonate", "chaos.clickdetonate");
    public void NotifyChaosLessonComplete(string id) => Chaos("ChaosLessonComplete", "chaos.lessoncomplete", ctx => ctx.Set("lesson_id", id ?? ""));
    public void NotifyChaosRunCompleted(int finalXp, string difficulty)
        => Chaos("ChaosRunCompleted", "chaos.runcompleted", ctx => { ctx.Set("xp", finalXp); ctx.Set("difficulty", difficulty ?? ""); });
    // NotifyChaosFirstTime has a default no-op body on the interface (App.cs:505); override it so the
    // ChaosFirstTime{bonus_id} rule (present in the base manifest) fires through the engine.
    public void NotifyChaosFirstTime(string id) => Chaos("ChaosFirstTime", "chaos.firsttime", ctx => ctx.Set("bonus_id", id ?? ""));

    /// <summary>
    /// Route a chaos trigger through the rule engine when a rule exists for it, else fall back to the
    /// random-phrase <see cref="BarkRequested"/> bark. Never both (BARK-1 slice 3 regression guard).
    /// </summary>
    private void Chaos(string trigger, string fallbackKind, Action<BarkContext>? fill = null) =>
        BarkTriggerRouting.RouteOrFallback(_engine, trigger, fill, () => RaiseBark(fallbackKind));
