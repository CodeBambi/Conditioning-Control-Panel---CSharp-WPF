...
public sealed class ArcademyProtocolTests : IDisposable
{
    ...
    [Fact]
    public void PageVocabulary_EveryUpstreamTypeIsHandledHere_AndAnythingElseIsRefusedOutLoud()
    {
        // The classification table has two states, and after slice 7 nothing sits between them.
        // meta-command (slice 3), the three class frames (slice 4), resume-request (slice 6 — it is
        // the PANIC ladder's frame, because OnResumeRequest refuses every other reason at :349-352)
        // and assets-request (slice 7) are all HANDLED; their facts are in ArcademyMetaTests,
        // ArcademyPanicTests and ArcademyRemoteMediaTests.
        ...
        Assert.Equal(ArcademyProtocol.ArcademyHandling.HandledHere, ArcademyProtocol.Classify("assets-request"));
