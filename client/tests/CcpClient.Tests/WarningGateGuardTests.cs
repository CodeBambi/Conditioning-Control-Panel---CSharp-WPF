...
public partial class WarningGateGuardTests
{
    ...
    {
        // A gate nobody is told to run is a script. The blind auditor matters most: it is the one
        // check meant to catch a lying land, and until SP-114 it was told to read `dotnet build`
        // output with its own eyes — the exact reading that produced four false claims.
        var root = FindRepoRoot();
