...
/// <summary>
/// SP-114: the census that covers the one hole a warning gate can NEVER cover.
///
...
public partial class WarningSuppressionCensusTests
{
    ...
    /// SP-114 on base <c>4332b8b9</c>. Nine inline pragma sites, all CS0067 ("event is never used"
    ...
    public void EveryWarningSuppressionUnderTheClientTree_IsPinnedByFileAndCode()
    {
        ...
        Assert.True(added.Count == 0 && removed.Count == 0,
            "the warning-suppression census under client/ no longer matches its pin (SP-114).\n"
            + (added.Count > 0
