...
/// This is the packet's named trap, and it is the same trap <see cref="TrayShellProbe"/> was
/// written for: an audio capability is trivial to fake because every failure mode is inaudible.
/// A test that measured "a sound played" through the same code that performs the playback could
/// be fooled by one edit to that code. These declarations are deliberately a second, independent
/// copy, so "the product says it opened a render session" and "the OS holds an active render
/// session for this process id" are two different facts produced by two different code paths.</para>
///
...
/// <c>:754</c> / <c>:565</c> / <c>:424</c> (IMMDeviceEnumerator / IMMDeviceCollection / IMMDevice
/// and their vtable order), <c>um/audiopolicy.h:1155</c> / <c>:762</c> / <c>:1058</c> / <c>:337</c>
...
/// (IAudioMeterInformation), <c>um/audiosessiontypes.h:149-154</c> (AudioSessionState), and
...
internal static class WasapiRenderProbe
{
    ...
    /// established by the test rather than taken from the product. Zero is a real answer (a
    /// headless build agent, a box with every output disabled) and it is what flips the honest
    /// expectation, exactly as <c>TrayShellProbe.MachineHasNotificationArea</c> does.
    /// </summary>
    internal static int ActiveRenderEndpointCount()
    {
        ...
        try
        {
            ...
            if (enumerator.EnumAudioEndpoints(ERender, DeviceStateActive, out collection) != 0 || collection is null)
            {
                return 0;
            }
    ...
    /// <summary>
    /// What the OS says about THIS process's render session on the default console endpoint.
    /// <paramref name="SessionsOnEndpoint"/> is how many sessions the endpoint holds in total —
    /// the negative control for the search, so a probe that degenerated into "found nothing,
    /// ever" is visible rather than silently certifying an absence.
    /// </summary>
    internal sealed record SessionFact(
        ...
        float Peak)
    {
        /// <summary>AudioSessionStateActive (audiosessiontypes.h:152) — the OS is pulling samples
        /// from a stream this process owns right now.</summary>
        internal bool Active => SessionForThisProcess && State == 1;

        internal static SessionFact None { get; } = new(false, 0, false, -1, false, 0f);
    }

    /// <summary>
    /// Enumerate the default render endpoint's sessions and report the one whose owning process id
    /// is this process. Never throws: a machine with no endpoint answers
    ...
    internal static SessionFact SessionForThisProcess()
    {
        ...
        try
        {
            ...
            if (enumerator.GetDefaultAudioEndpoint(ERender, EConsole, out endpoint) != 0 || endpoint is null)
            {
                return SessionFact.None;
            }
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                try
                {
                    ...
                    if (control2.GetProcessId(out var pid) != 0 || pid != self)
                    {
                        continue;
                    }

                    var state = control2.GetState(out var s) == 0 ? (int)s : -1;
                    var meterReadable = false;
                    var peak = 0f;
                    if (control is IAudioMeterInformation meter && meter.GetPeakValue(out var p) == 0)
                    {
                        meterReadable = true;
                        peak = p;
                    }
    ...
    {
        // IAudioSessionManager (audiopolicy.h:762) — inherited, so its two slots come first.
        [PreserveSig]
    ...
    {
        // IAudioSessionControl (audiopolicy.h:337) — inherited, so its nine slots come first.
        [PreserveSig]
        int GetState(out uint state);
