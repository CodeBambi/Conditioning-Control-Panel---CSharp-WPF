...
public class CompositionRootValidationTests
{
    ...
    public void Build_AfterValidate_ConstructsHostWithNamedParticipants()
    {
        ...
        Assert.IsType<HeartbeatParticipant>(host.Participants[2]);
        // The demonstrator ticker registers AFTER the store — phase-3 start order
        // IS the restore-then-start ordering.
        Assert.IsType<CcpClient.Desktop.Features.StatusTickerParticipant>(host.Participants[3]);
        ...
        // phase-3 load).
        Assert.IsType<CcpClient.Desktop.Session.SessionParticipant>(host.Participants[8]);
