...
// calls, no UI, no moderation engine. The capability-honesty rule: nothing here is a capability
...
/// <summary>
/// AI operation domain result (contract §1). Rides inside the OperationOutcome's
...
public abstract record AiReply
{
    ...
    /// <summary>The provider could not produce a reply (offline, unconfigured, not proven by a capability probe). Not an error; distinct from Refused. Code is a stable machine token.</summary>
...
public static class AiReplyCodes
{
    ...
    /// <summary>The selected provider's capability state is not Available (selection/registration never prove availability).</summary>
