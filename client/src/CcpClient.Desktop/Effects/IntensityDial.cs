...
/// named settings by hand inside its tick (<c>MainWindow/MainWindow.StartStop.cs:503-539</c>), which
...
/// deliberate.</b> WPF does both inside one <c>Dispatcher.Invoke</c> (<c>:503</c>). Here:</para>
/// <list type="bullet">
...
/// </list>
///
/// <para><b>What the split buys, beyond thread safety.</b> On the teardown path a post is not
/// delivered (the dispatcher is already down), so a single combined operation would leave the ramped
/// value in the document and the persistence flush would write it to disk — the user's own opacity
/// silently replaced by whatever the ramp had reached. <b>That is upstream's behaviour today</b>:
/// closing the window calls a bare <c>_rampTimer?.Stop()</c> with no restore
/// (<c>MainWindow/MainWindow.WindowChrome.cs:167</c>), five lines after <c>SaveSettings()</c> at
/// <c>:162</c>. Splitting the write means the VALUE always comes back even when the re-apply cannot
/// be delivered. Recorded as D95.</para>
///
...
public interface IIntensityDial
{
    ...
    /// spiral 100 (<c>MainWindow.StartStop.cs:517</c>), pink filter <b>50</b> (<c>:521</c>) — and for
...
/// <c>RampLinkSpiralOpacity</c> branch (<c>MainWindow/MainWindow.StartStop.cs:514-519</c>).
///
...
/// <c>RampLinkPinkFilterOpacity</c> branch (<c>MainWindow/MainWindow.StartStop.cs:520-524</c>), whose
...
public sealed class PinkFilterOpacityDial : IIntensityDial
{
    ...
    /// (<c>MainWindow.StartStop.cs:521</c>), and the document's own clamp ceiling.</remarks>
