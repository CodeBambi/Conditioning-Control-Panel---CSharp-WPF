...
## 2026-08-05 (addendum 4) — pi 0.84.1 update wiped the billing fix + kimi 403 batch kill

- **pi 0.84.1 REGRESSED the OAuth workaround:** the owner's pi update (0.83.0→0.84.1) wiped `__PI_BILLING_HEADER_FIX__` AND upstream moved pi's system prompt INTO `system[]` (the exact shape the classifier 400s on) + dropped the UA suffix + cc_version to 2.1.75. **v2 applied:** system[0]=billing token (cc_version=2.1.221), system[1]=identity, pi prompt restored to [CONTEXT] user-prefix+ack, UA `claude-cli/2.1.221 (external, sdk-cli)` — `pi -p` 200 on opus-5 AND fable-5. **Ritual after EVERY pi update: `grep -c __PI_BILLING_HEADER_FIX__ <nested pi-ai anthropic-messages.js>`; never blind-reapply across versions (the block changes shape — read it first).**
- **kimi quota 403 killed wave-13's first run (batch `20260805T102230`):** "You've reached your usage limit for this billing cycle" — the watcher AND both lane workers died in the same 403 window (~10:30 UTC). Quota self-refreshed by the owner's return (days later). Recovery: salvage-commit the start-marker (UNVERIFIED), `spine batch retry SP-052` + `resume --force` — replacement worker live. The stale "Macro phase: Failed" display lags the detached resume; trust the journal's heartbeats over the phase line.
- **System date note:** the laptop's local clock moved (WEDT→TST, multi-day gap between sessions) — journal timestamps are UTC; compare against `date` before reasoning about ages.
