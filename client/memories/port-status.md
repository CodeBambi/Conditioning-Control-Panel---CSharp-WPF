# Port Status (as of 2026-08-13, eleventh export — laptop)

## Wave 25 AUTHORED + LAUNCHED (2026-08-13) — SP-068, single lane, NOTHING LANDED YET

- **Phase was case B (author + launch).** Reconciled from durable evidence: spine idle, no STOP, board had claimable work. Counts unchanged at **903 unit / 35 headless / 2 named skips, 0W/0E** — the authoring phase touched only packet/planning/doc files.
- **SP-068 = three SUBTRACTIVE privacy filters over ALREADY-LANDED port code** (board row `:46`, acting on SP-060's audit): **F1** incognito hard-drop (audit A6, ADOPT), **F2** title scrub — emails / `\d{6,}` / control chars / collapse / cap 80 (A10, ADOPT), **F3** unsanctioned-link strip on companion prose (C3, **MERGE**, strip half only). Every change **narrows**; nothing new observed, persisted, logged, or transmitted.
- **PROVENANCE DISCIPLINE (consult condition — do not restate it loosely):** the set was **derived at authoring** by filtering the audit's §5 sizing table for S + dep-none + unit-evidence-only. That yields six; the other three (A11, D6, D11) need headed evidence and are deferred to a machine with the owner's display. **Not "the audit's six adopts"; C3 is a MERGE row.**
- **"An audit is not a decree" is NOT violated — and the reason matters.** Authorization is **board row `:46` as queue authority**, not the audit. What the wave-17 decree protects is *replacing the landed c1–c7 architecture*; three narrowing filters over existing paths are not that. Verdict from the decomposition consult (solo Opus 5), recorded verbatim in the packet amendments.
- **CONSULT TRUNCATION, 6th OCCURRENCE.** First decomposition call returned **reasoning with no verdict**; a narrow re-ask **capped at 150 words** surfaced cleanly. The cap remains the working fix (4th consecutive wave). **Never stitch a verdict out of reasoning** — the reasoning was substantive and still was not treated as a verdict.
- **THE AUDIT'S LINE CITES ARE STALE — proven, not assumed.** `AwarenessObserverPolicy.cs:319-327` is `ResolveOwnProcessName()`; `:277-279` is a `catch` returning `PolicyUnavailable`. Semantics all present by grep; offsets moved. The packet treats every WPF offset as a **hint to re-derive by symbol** and requires found-vs-given recorded. **Generalize: a landed audit ages into a map, not a citation.**
- **Two findings the worker must resolve, not inherit:** WPF has **TWO** incognito marker lists (`AwarenessPrivacyRules` + `AwarenessObserverPolicy`) → reconcile into ONE port definition (SP-055 discipline); and F3's insertion point is inside the **memory-persist path** (`AiOperationPipeline.cs:344`) → the strip changes what c4 persists (less text), which must be proven the only persistence change.
- **HONESTY PINNED UP FRONT:** `ObserveForegroundTitle` / `RunReactionAsync` are called **only from tests** today (sole product wiring = `AiModerationBoundary.cs:110`), so **F1/F2 harden a path no user path drives yet**; **F3's path IS live**. The packet bans claiming a user-visible win for F1/F2.
- **THE SIZING PASS IS DEFERRED, MACHINE-GATED — NOT DROPPED.** Wave-16's constraint (Goon/FYP/Trainer Card/Haptics v2 undecomposed until a sizing pass) **still stands**. Three of the four need headed/payload/Linux evidence this laptop cannot produce. Raised to the owner in the digest: **most remaining parity is desktop-only**, with an explicit offer to write the plan anyway if they want it ready.
- **`spine preflight` `prelanded-file-scope` PASSED for this packet** (it fired as a false positive at SP-067). The standing analysis stands: it compares against **`main`**, which has no `client/` tree; its suggested remediation manufactures the docs-only-contract class. **If it fires mid-batch, do not act on it.**
- **Land obligations inherited by the NEXT phase:** verify the merged tree yourself in a scratch worktree through `check-floor.mjs` (never bare `dotnet test`), prove `git diff` EMPTY between the verified tree and the integrated tip (three dots, not two), **verify the tree actually being pushed LAST**, and **budget the `task-board.md` row update into the land** (ENABLER 2 keeps it out of worker scope; SP-001's gap recurred at SP-067).
- **Next unused task ID: SP-069.** Standing limits unchanged: **zero WSL distros → every Linux gate is a named limit, never faked**; **MCP 0/3 connected** (cached only) — named limit, never a blocker; `Z:\CCP Vids`, DISPLAY3 and the WSL2 Linux gate are **DESKTOP-only**.
