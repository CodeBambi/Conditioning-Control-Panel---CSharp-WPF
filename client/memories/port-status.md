...
**Standing trap RETIRED at SP-142 (2026-08-22) - kept as history, do NOT act on it.** The fix landed: `Begin()` and `Cancel()` now capture the source under `_gate` and cancel AFTER releasing it. **The inversion was LATENT and never observed**; what held it was an unenforced convention across thirteen subclasses, not a mechanism. **The lock-free callback discipline SURVIVES for a different reason** - a callback runs synchronously inside whatever thread called `Cancel`/`Begin`, so anything it blocks on blocks that caller. Original text: ~~`AsyncOperationOwner` runs cancellation callbacks INSIDE its own lock (`Lifecycle/OperationRegistry.cs:148-158`, `:161-167`), while `OwnedSessionEffect.Dot` holds the effect gate across `_owner.IsLive`. Taking the effect gate in a cancellation callback therefore inverts lock order and hangs shutdown. SP-106 tried it, passed a fully green suite anyway (every test drives that path on one thread), and reverted. New P1 board row owns the real fix.
...
## 2026-08-20 - wave 54 landed
Floor pin **1938 unit / 117 headless**. Next task ID **SP-115**. Eleven modules of fifteen; this wave added none.
**A plain `dotnet build` CANNOT be trusted to report warnings.** MSBuild skips `CoreCompile` on a rebuild, so it prints 0 warnings over a tree that still holds a live one - proven twice, independently. **Always run `node client/tests/floor/check-warnings.mjs`**, which forces `--no-incremental`. Never read build output through a grep filter you have not tested against a known-matching line.
**The census's failure mode is silence.** It proves nothing suppresses, and it has been wrong twice - missing `.globalconfig` and `[assembly:` forms. It now executes its own pattern against five shapes before asserting an absence. Gate-green plus census-green means no suppression OF AN ENUMERATED SHAPE was added, never no warning was suppressed.
**A new flake is open** (`SpiralOverlayEffectTests.DisarmReleasesTheWorkUNCONDITIONALLY`) that contradicts SP-106's recorded conclusion: it touches no OS, so real-OS desktop contention does not explain it. Do not count three greens as determinism there.

## 2026-08-20 - wave 55 landed
Floor pin **2062 unit / 121 headless**. Next task ID **SP-116**. **TWELVE modules of fifteen** - remaining: Visuals, Haptics, Scheduler (Scheduler is not a session module).
**THE TOLERANCE RULE is now in `client/docs/port-workflow.md`:** a tolerance sized to an observed error is exactly the size of the defect it will next hide. Prefer finding the model wrong over widening the window - a disagreement between a model and a measurement is evidence about the model at least as often as about the world. SP-115's one-unit residue was its own oracle rounding three times where the formula rounds once.
**THE SUITE IS INTERMITTENTLY RED AT BASE.** A lane ran the unmodified base tree seven times and got one red on a test it had not touched. Three strands now (SP-106 blamed real-OS contention; SP-114's touches no OS; this one is red on an unmodified tree) with no shared explanation. **Do not treat three consecutive greens as determinism.** Never retry, quarantine or skip to get past it.
**The dot has now meant eight things** - clock, screen, change, custody, reach, demand, motion, COMPOSITE.

## 2026-08-20 - wave 56 landed
Floor pin **2067 unit / 121 headless**. Next task ID **SP-117**. Twelve modules of fifteen; **no module this wave**.
**THE FLAKE'S MECHANISM: a composited-desktop read with no happens-before edge against the compositor.** NOT occlusion - SP-107 and SP-115 both guessed that and the control refutes it (every miss: our window owned the point, drew its own colour, identical background behind it). `DwmFlush` fence at `FlashPixelProbe.CaptureDesktop`, the single `CAPTUREBLT` choke point. 34/1200 -> 0/1500.
...
## 2026-08-20 - wave 59 landed
Floor pin **2247 unit / 141 headless**. Next task ID **SP-120**. **FIFTEEN of fifteen rack rows - the rack is COMPLETE.** Say the second half in the same breath every time: **a landed row is not a working feature**, and haptics is the clearest case in the port - the seam refuses every caller, `Effects/**` is byte-identical, and nothing in this build sends anything to a device.
**The haptic dependency is the last of its four blockers and it is SMALLER than it was filed as.** "Haptics needs a NuGet package" is half wrong in the half that matters: **Lovense needs ZERO packages** (loopback HTTP over the BCL's own `HttpClient`, one file, plus a keep-alive); **Buttplug** is the one that needs it, and its net10.0 TFM and licence were **READ from source, never checked against the nupkg**. So three of the four options are answerable today. The port ships **admit nothing**, which is why it refuses.
**A refusal must name the gap that is actually blocking.** This one names the **admitted-provider** gap; "no device found" would have been false, because the port has no client to look with. The same discipline gives the dot only two reachable values, with `Live` unreachable BY CONSTRUCTION and that absence recorded as the finding.
**D179 was an undercount AND carried a path that does not exist** - EIGHT sites and `Services/SubliminalService.cs:230`; the truth is **THIRTEEN** and `Services/Subliminal/SubliminalService.cs`. That list is a **later packet's work item**, which is why the error mattered more than its size. **An enumeration handed forward is a work item: re-derive it by grep at the hand-off, never copy it.**
**`felt-verified` is undischargeable by code - but do not let it swallow the claim next to it.** No API depth reaches a device, so "it moved" needs a human. **"The stop reached the wire" is a DIFFERENT claim, it is pinnable, and it is not pinned**; only the in-process ordering is. STOP is the safety-relevant half (upstream: a Lovense level has no server-side watchdog).
**D197, in the shipping WPF app:** its three haptic gates use different predicates, so on a server-declared free day the rail unlocks and the mixer opens while the checkbox still refuses.

## 2026-08-21 - wave 60 landed (SP-120 + SP-121)
Floor pin **2270 unit / 141 headless**. Next task ID **SP-122**. Two instruments, no product code, no feature. Rack still fifteen of fifteen; the product is still not at parity.
**THE ORCHESTRATOR WAS WRONG MORE OFTEN THAN THE LANES.** Say this plainly to the next session, because it is a pattern, not one slip: the base was pushed RED, the exclusion rule handed to SP-121 had two defects that would each have hidden real shipped types, and SP-120's packet carried four citations that did not say what it claimed. **Lanes refused all of it and proved why.** A packet is a claim, not evidence — the same rule the board applies to rows applies to the packets I write.
**`validate-wave.mjs` and `FloorWrapperGuardTests` HAVE DRIFTED and the validator's own MIRROR note forbids it.** Check 4 matches glob-aware (`patternCovers`, `:458`); the test matches a literal substring (`.Contains`, `:224`). So `client/tests/floor/**` prints WAVE OK and reds the suite. **Filed P0 — do not close it by editing packets to please the stricter of two rules that were meant to be one.**
**PACKET FILES ARE READ BY A GUARD AT RUNTIME.** They are not inert documentation. Authoring a packet after the final verification run and pushing without re-running is the wave-18 class, and it happened here.
**The haptic site count has been wrong FOUR times: 8 -> 13 -> 14 -> 18.** Every correction came from widening the UNIVERSE, never from reading harder — each search was a file LIST. The pin now walks DIRECTORIES, which closes that mechanism; it does not close the general case, since the family is still three hand-chosen directories.
**Eighteen upstream sites reach FIVE port statements.** 7 collapsed, 3 present, 8 absent-by-decision with the decision quoted, 0 unexplained. The vibe vocabulary is PRICED (four options plus C+, C recommended) and **not chosen** — owner's call.
**42 of 649 shipped types have ZERO executed lines**, regenerable by `node client/tools/coverage/census.mjs`. **The naive rule produces FALSE ZEROS** (async state machines hold a type's every source line) and **a member-less record reads zero even when constructed**. No threshold was set and none should be.
**`SystemSoundClock` (`Audio/AudioSeams.cs:133-137`) is the third instance of a shape that bit twice** — the only one of three clocks without the fault guard its siblings carry and document. Found mechanically rather than by luck, which is why the instrument was worth building.
**In the shipping WPF app: panic-keying a video leaves the haptic layer latched**, and the comment beside the stop claims it "runs on every teardown path" — a written guarantee the code does not keep.

## 2026-08-21 - wave 61 landed (SP-122 + SP-123)
Floor pin **2309 unit / 144 headless**. Next task ID **SP-124**. Rack still fifteen of fifteen; product still not at parity; haptics still moves nothing.
**FIRST `presentation-verified` EVIDENCE OF THE RACK, after fifteen modules shipped unseen.** Four surfaces: `rack-row` selected/unselected, `rack-row-dot` armed/off, driven by real mouse input on a real desktop with a UIA read confirming state before any pixel. **Every check was proved to FAIL on a real capture of the wrong state.** The ceiling is unchanged and must keep being said: **no human has compared the port's rack to the WPF build** — that manual gate is named and OPEN.
**THE CENSUS MEASURED 42 -> 38** at the land; the four rows SP-123 predicted left, each because a fact now DRIVES the type. Three defects closed: `SystemSoundClock`'s containment (plus a second missing `ThrowIfNull` nobody had noticed), the bark resolver's "never throws" made unconditional, and the DTRH composition lifted and driven.
**THE PACKET'S PREMISE WAS FALSE AND THE LANE REFUSED IT.** SP-122 was authorised to add a probe to `Views/**`; it measured that rack rows already expose UIA peers, and that the probe would push five of fifteen rows below the scroll fold — displacing the rows it existed to photograph. Authorisation withdrawn, no product byte changed. **Expect packets to be wrong; reward the lane that measures the premise.**
**SIX GUARDS THAT DID NOT GUARD, in one wave about verification.** Lease guard green with the lease call deleted; a threshold nothing read; a clock fact watching an unrelated timer; a composition fact using the constant it claimed to pin; a census blind to its own drift; a denominator anchor that blocks any new shipped type. **Two are inherited into already-landed files.** All fixes mutation-proved before acceptance.
**THE DENOMINATOR ANCHOR IS A BUILD CHOKEPOINT (P0).** `ExecutionCensusTests.cs:254` compares live reflection against a scalar in the committed census, which is closed to lanes — so any packet adding one ordinary class reds with no remedy. Reproduced twice. Do not close it by regenerating; bind the two MECHANISMS to each other.
**FOUR PRE-EXISTING HARNESS DEFECTS CLOSED IN `capture.ps1`:** no real-desktop lease, an unfenced `CopyFromScreen`, no `exit 0` (so `self-test.ps1` read the previous command's exit code), and a deterministic start that cleared settings but not the preset store — so an `off` capture leaked into the next `armed` one, wrong since SP-098.
**The rack pin is only as true as the last headed run:** its fractions are hand-transcribed literals over gitignored captures, and nothing re-derives them.

## 2026-08-21 - wave 62 landed (SP-124 + SP-125)
Floor pin **2332 unit / 144 headless**. Next task ID **SP-126**. No product code in either packet. Rack fifteen of fifteen; product not at parity; haptics still moves nothing; no human has compared the port to WPF.
**THE PRODUCT PIPELINE IS UNBLOCKED.** SP-121's denominator anchor compared live reflection to a scalar in a census closed to lanes, so **any packet adding one ordinary shipped type reddened the suite with no remedy** — and every remaining v6.7 surface needs many. Replaced with both sides live on the same bytes (`census.mjs --metadata-json --dll` versus reflection); reviewer reproduced base-reds/head-passes with an identical throwaway class. **The cross-validation is stronger, not weaker** — proved by corrupting the ECMA-335 walk three ways. Trade stated: **no per-lane fact notices census drift any more**; that moved to `census.mjs --check-stale`, run at the land, which enumerates what it did NOT check so a quiet exit is never "the census is current". `ns` and excluded-row `hasIl` remain uncompared, named in code.
**FOR YOU FEED: REFUSED, with three owner decisions surfaced.** (1) **It uses the webcam** — `FypHostService.cs:903-1045` starts the camera, gates on consent, subscribes gaze; webcam is not one of the seven landed capabilities and `capability-inventory.md:70`/`:78` put it under owner review. (2) **It POSTs to `api.scrolller.com/admin`**, an unofficial third-party API, 12 explicit niches, **dwell-weighted — revealed preferences shape the requests**. (3) **Consent inside the feed unlocks remote media across five other surfaces** (`AppSettings.cs:3327`). Consent is properly gated; the finding is the **combination** — in ghost mode the window indicating eye control is parked off-screen and shown only as a click-through mirror.
**THE BOARD ROW WAS WRONG ABOUT SCOPE, NOT JUST COUNT:** 3 claimed, 9 real, and **`Services/Fyp/Online/` is 42% of the surface and NOT part of the feed** — multi-tenant infrastructure with 11 compile-time consumers outside it, two already shipped here. Separable unit named: **remote media alone**, 5 files, zero Win32, already needed by two shipped surfaces.
**GHOST MODE IS THE OnTopReplica TECHNIQUE** — real WebView2 window parked off the virtual desktop, a GDI window painting a live DWM thumbnail, translucent and click-through. **Upstream records the obvious approach as a SHIPPED FAILURE**: `SetLayeredWindowAttributes` on a WebView2 host turns content solid black, reproduced live 2026-08-03. That note is why a port attempt would not repeat it.
**SEVEN GUARDS THAT DID NOT GUARD across waves 60-62, and the pattern is one thing:** a DESCRIPTION outrunning its MECHANISM. The resolution that works is not a cleverer matcher but a written reach that matches the enforced reach — **and a claim about what a guard cannot do needs the same evidence as a claim about what it can.**
**Open and unowned:** the timing guard cannot see a bare `TimeSpan.From*` handed to a product API; a starvation-sensitive `SoundArbitrationTests` fact reddened 1-in-8 with two live mechanisms unexcluded (its own pool-blocking poll, and changed interleaving from lengthened neighbours) — `allowedSkips` is the WRONG instrument, it is red-on-starved by design; and `client/tools/citations/detect.mjs` is run by **no standing gate** despite having paid for itself on its first live delta.

## 2026-08-21 - wave 63 landed (SP-126 + SP-127)
Floor pin **2399 unit / 144 headless**. Next task ID **SP-128**. **First wave in five to ship PRODUCT BEHAVIOUR.** Census regenerated at the land: universe 649 -> 658, executed 611 -> 620, **zero-execution still 38** — all nine new types are driven.
**D179's COMPUTATION HALF IS CLOSED.** Nine of ten mappable sites command the sink with upstream's arithmetic: ladder 8 rungs `max(0.5·0.7^i, 0.06)` @450ms **span 3503ms** (not the doc's "~2s"); video layer `max(0.5·0.1,0.06)=0.06` latched; subliminal keyed off wording; bounce floored to 158ms. **Eight absent-by-decision sites untouched** — wiring one means inventing its trigger. **NOTHING MOVES:** `AdmittedRoutes` is `[]`, `SetOutputsAsync` unreachable on a product path, dot still `Off` after sixty commanded moments, `DeviceManualGate` as undischarged as at SP-119.
**THE C+ ARCHITECTURE CALL WAS THE ORCHESTRATOR'S**, taken under the owner's standing instruction (mechanism delegated, behaviour fixed): the thin option drops peak-of-sum, which **reachable** sites exercise, at **a factor of two on what a user feels** (0.70 at cap vs 0.35). The lane then found a **second** reachable instance the packet had not named. Proven by mutation: degrading to MAX reds four facts.
**THE PORT'S STOP IS DELIBERATELY BETTER THAN UPSTREAM'S.** Upstream reaches its video stop only via `Cleanup()`; `Stop()`, `CloseAll()`, `ForceCleanup()` carry none, and `ForceCleanup` is the panic key — a comment beside the stop falsely claims it runs on every path. The port stops on both paths, three facts, both call sites in one session so neither is deletable.
**EIGHTH GUARD-THAT-DID-NOT-GUARD, first in a safety path.** The ordering fact asserted end-state, which the reversal satisfies. **Fixed by exercising the hazard: the probe commands the limb at dispose and reports it no longer accepts.** Both reversals red at distinct lines. **Pin ordering by attempting the dangerous thing, never by recording a marker.**
**THE STALE-CHECK DIAGNOSTIC EARNED ITSELF WITHIN A DAY.** It reported the census agreeing at exactly the pre-merge numbers right after nine types landed; the write-time line showed the binary was 11 hours old. Rebuilt, it correctly said STALE. **Never trust a drift verdict without reading the input's age.**
**TWO LANES COLLIDE ON ANY SHARED APPEND TARGET.** Both packets said "divergences from D210" and both obeyed; merge conflicted on seven ids. `validate-wave.mjs` sees paths, not id ranges. Resolved by renumbering the second lane to D219-D225 with cross-references in four files. **Allocate disjoint id ranges at authoring.**
**TRAINER CARD: BUILDABLE-IN-PART.** The row's counts are exact and mean the wrong thing — `Views/Controls/`'s 113 is 3.5% this feature, and the row misses the directory, since the feature is twelve `MainWindow` partials. Consumer side **5 of 16 members, all pure arithmetic**. Buildable unit: the graded-run award path, 34 lines, no assets, no owner decision. **No sensor anywhere in this surface**, unlike FYP.
**IN THE SHIPPING APP: `honor_roll` fires EARLY and permanently** — consumer adds `e.Category` raw into a default `HashSet<string>` while producers disagree on casing (`Trim().ToLowerInvariant()` vs a PascalCase enum name). Latent: the classic launcher is `Visibility="Collapsed"`. **The port matches upstream verbatim on the side it ported, so the port does not carry it.**
**Corrected at the land:** the leaderboard **has no POST** — two GETs carrying `unified_id`, a `display_name` lookup and version headers. What leaves is identity, not a score.
