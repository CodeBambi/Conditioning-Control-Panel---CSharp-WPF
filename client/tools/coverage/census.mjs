...
//
// WHY THIS EXISTS (client/docs/task-board.md:32)
...
// tree it changes the zero count by nothing (0 false zeros, 0 missed zeros) and adds exactly one
// authored type to the universe - which is the point: it makes the census right by construction
...
function render(rule, tally, types, runs, metadata) {
  ...
  const platformConditional = platformConditionalFiles(zero);
  ...
  w();
  w("Answers `client/docs/task-board.md:32` mechanically. **This document sets no threshold, no");
  ...
  w();
  w("**5. The census is OS-CONDITIONED.** It was generated on one operating system. The suite carries");
  w("OS-gated tests (`client/tests/floor/floor.json`, `allowedSkipsMachineClasses`), so on a Windows");
  w("run the Linux legs never execute and their types land in the zero list because of the MACHINE,");
  w("not because the code is dead — and vice versa. A row below carries a `platform-conditional`");
  w("marker when the type's OWN source file holds a platform predicate (`OperatingSystem.Is*`,");
  w("`RuntimeInformation.IsOSPlatform`, `[SupportedOSPlatform]`). That is a checkable property of");
  w("the code, deliberately not a guess from matching type names against OS-gated test names — a");
  w("wrong `os-gated` marker would excuse a real defect as a machine artifact. The marker claims");
  w("only *check the other OS before believing this row*; it never claims the type is fine, and it");
  w("never changes the count. **A type is confidently dead only when both OS legs agree.**");
  w(`Marked below: **${platformConditional.size}** of ${zero.length}.`);
  w();
  ...
  for (const t of zero) {
    ...
    const marker = (platformConditional.has(t.name) ? " `platform-conditional`" : "")
...
// A zero row is marked `platform-conditional` when the type's OWN SOURCE FILE contains a platform
// predicate. Deliberately a checkable property of the code rather than a guess from test names: a
// marker derived by matching a type's name against an OS-gated test's name would be a guess, and a
// wrong `os-gated` marker is worse than none — it excuses a real defect as a machine artifact.
//
// It is a MARKER and never an exclusion: a marked row still counts in the total, and the marker
// claims only "a Windows generation cannot execute this file's Linux leg, so check the other OS
// before believing this row". It does not claim the type is fine.
const PLATFORM_PREDICATE = /OperatingSystem\.Is[A-Za-z]+\(|RuntimeInformation\.IsOSPlatform|\[SupportedOSPlatform/;

function platformConditionalFiles(zeroTypes) {
  ...
  for (const type of zeroTypes) {
    for (const file of type.files) {
      if (!cache.has(file)) {
        ...
        try {
          conditional = PLATFORM_PREDICATE.test(fs.readFileSync(path.join(REPO, file), "utf8"));
        } catch {
          conditional = false; // a source file the census cannot read is never marked
        }
        cache.set(file, conditional);
      }
      if (cache.get(file)) marked.add(type.name);
    }
  }
  return marked;
}
