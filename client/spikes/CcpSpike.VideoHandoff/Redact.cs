...
/// <summary>
/// Central secret registry + redaction. Packet honesty framing (b): cookie/header/signed-token
/// VALUES are never logged — presence + redacted shape only (e.g. cookie:present(len=31)).
/// Every log line in the spike passes through <see cref="Scrub"/>; the --audit-logs self-check
/// scans emitted logs for any registered secret value and FAILS on a hit.
/// </summary>
public static class Redact
{
    ...
    public static string NewSecret(string name, int hexChars = 24)
    {
        ...
        {
            "cookie" => "spkc_" + Convert.ToHexString(bytes).ToLowerInvariant(),
            "header" => "spkh_" + Convert.ToHexString(bytes).ToLowerInvariant(),
