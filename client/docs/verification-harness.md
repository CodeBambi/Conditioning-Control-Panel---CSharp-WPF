...
pwsh client/tools/verify/capture.ps1 -Surface audio-dial -State live
pwsh client/tools/verify/capture.ps1 -Surface audio-dial -State running
...
Surfaces are NAMED capture scopes, not app screens: `rail-door` = a navigation rail door rect taken from the app's own layout probe (Avalonia exposes no UIA peers for Border/Grid/StackPanel — harness surprise #1); `dashboard` = the whole window; `rack-row` = the Studio rack's Flash Images row rect; `rack-row-dot` = that row's 8-DIP state dot cell; `session-row` = one scripted-session row's 4-DIP difficulty stripe with a 10-DIP row gutter either side of it, on the SESSIONS row's panel; `session-start` = that panel's one start/stop button; `session-history` = a 200x23 DIP band that starts above the Recent Sessions list plate's own top border and ends 7 DIP into the list, where its first row is or is not.

**Two of those scopes carry a BOUNDARY rather than only their subject, and that is a correction the non-vacuity gate forced.** A capture cropped to a flat fill — the 4x20 DIP stripe, the 5 DIP band inside a row's top padding — is one colour, and one colour is not evidence that anything was drawn, so the capture step refuses it. The answer was not an exemption but a wider crop: the stripe is photographed with the row gutters that bound it and the history cell with the plate edge above it, and the named check then samples the subject's interior of that picture. Each check therefore says WHERE its colour stops as well as what it is, which is strictly more than the flat fill said.
...
**Every capture now holds the machine-wide real-desktop lease and fences its screen read.** `capture.ps1` puts a top-most window on the interactive desktop and reads that desktop back — exactly what `RealDesktopCollection`'s fixtures do — and it ran OUTSIDE their lease until the rack work, so a capture could race a floor run and take the measured failure mode (`Expected 0, Actual 676161`). It now takes `%TEMP%/ccp-real-desktop.lease` with the identical contract (`FileMode.Create` / `FileAccess.Write` / `FileShare.Read`, raw `pid=<n>` bytes so `RealDesktopLease.HolderProcessId` can name it), before the app launches and until after it exits, and it FAILS naming the holder's pid rather than capturing around a contended desktop. `CopyFromScreen` is now preceded by `DwmFlush` (measured: 34 misses in 1200 unfenced reads, 0 in 1500 fenced); a DWM that refuses fails the capture instead of being swallowed. **Re-anchored** from `dashboard-card`/`lit`/`unlit`, which named the retired `demo.status-ticker` demonstrator card. State drives differ per platform, honestly: **Windows** drives `selected` through REAL input (a left-click on a rail door, the route confirmed by a UIA read before any pixel is captured — the same user path a regression would break); **WSLg** needs no drive for the rail-door pair, because on cold start the Studio door is already `:checked` and the Companion door is not, so both states are capturable with zero input. ~~WSLg has no input automation (a named gate), so interaction evidence stays Windows-headed and WSLg captures are render/scale/session-facts only.~~ **BOTH HALVES OF THAT ARE NOW WRONG AND WERE CORRECTED BY MEASUREMENT — this paragraph outlived them by a day and by a lane.** (1) 2026-08-24: `xdotool` really is absent, but the X server advertises XTEST 2.2 and `libXtst.so.6` ships in the base X client libraries, so synthetic input needs no new package; `client/tools/verify/xinput.py` drives real clicks, and the same correction was made in `client/docs/window-behavior-manifest.md:74` while this line was missed. (2) 2026-08-25: the reason Linux reached only three named checks was never presentation, input or focus — it was that `capture.ps1` finds every surface through UIA and Linux has none. The element route is AT-SPI over D-Bus (`client/tools/verify/atspi.py`), it needed no new package either, and it took Linux from three named checks to ten with real left-clicks, right-clicks and wheel gestures driving the states. What Linux reaches, and the exact mechanism gating everything it does not, is recorded in `client/docs/linux-evidence.md` and kept honest by `LinuxEvidenceGateTests`.
...
- Checks are scoped to REAL current consumers only. **This line named the original demonstrator card until the rack work, and had been stale since the re-anchor retired it. It was stale AGAIN — it said eight after the Goon page and the Trainer Card had added four more — and is corrected on every change by counting the manifest rather than by memory.** The forty-five checks today are (counted from the manifest with `grep -c '"name":' client/tools/verify/checks.json`, never from memory - this line has now been stale four separate times, and the count is the least useful thing on it): the rail door's selected/unselected border bands and the dashboard background (shell); the Studio rack's selected marker, selected fill, unselected ground and its state dot armed/off (rack); the Goon page's backdrop and explainer card; the Trainer Card's ground and ink; the session rack's two difficulty stripes and its start button's two liveries; the Recent Sessions list's first-row cell in the two states upstream's retention rule produces; the AI permissions grid's panel fill against the window ground it replaces; the Studio dial's track in the two liveries a running session produces, on a control the session LOCKS but never WRITES so both captures are the same control at the same value; the toast's accent in the two states a real save and a real refusal produce; the companion's privacy dial and its transcript window; and the POP QUIZ CARD's ground and question ink, whose ink floor is set an order of magnitude BELOW the lowest of four measurements because the question is drawn at random and its inked fraction moves with the card - a floor near the mean redded a perfectly good card on its second run; and the TRAINER CARD LEVEL BAR in its two states, which is the first pair on this manifest whose claim is a FRACTION rather than a colour - the same 420x8 DIP bar painted in the same two colours in both captures, separated only by how much of the track the fill covers, and the band that reads it sits at 0.10..0.30 against a fill reaching 0.5949 because the pop quiz lesson is that a boundary set near a value the product moves reds a good capture; and the AUDIO ROW'S MASTER VOLUME in two states that are deliberately IDENTICAL, which is the first such pair here - a scripted session must leave that dial alone (`audio.json` is not one of the eleven documents a run borrows, and upstream names audio volume in its own never-lock list, `Features/SessionLock.cs:21-38`), so an OVER-LOCK is the regression these two exist to catch and both states expect the live track. **That track was `#0078D4` — the machine's Windows personalisation accent — until 2026-08-26, and it had been describing a colour the port stopped painting ever since the token layer shadowed Fluent's accent family. It is the product's own accent now, `#E84393` under the CCP Default theme, and a headed capture is what caught the drift: the manifest and the tests over it were self-consistent about a colour that was not on the screen. The fractions below are unchanged, because the geometry did not move.** Its inversion is therefore ACROSS SURFACES rather than across states: measured 208/884 = 0.235 on each of its own captures, 0.0000 for `studio-dial-locked-track` on the audio `running` capture taken under a real running session, 0.0000 for both audio checks on `windows-studio-dial-locked.png` (which still scores 208/936 = 0.222 on its own), and 0.0000 for both on a capture of the whole dashboard. Both floors sit at 0.12 against that 0.235, and what is pinned mechanically is the MARGIN RULE rather than the number (`AudioDialPresentationTests`: at most three quarters of the measurement, at least 0.05); and the TRAINER CARD'S AWARD RECORD in the two states the landed `trainer-card` surface does not have - a record the card READ and one it could NOT - plus the earned row that Read cannot be photographed without, because a Read record with nothing earned is NOT PRODUCIBLE by this build at all (`top_of_the_class` is awarded first and unconditionally on a top-marks run, so the first bytes that record ever holds already carry it, and that is a finding rather than a convenience). It is the first trio on this manifest, and the first surface whose states are separated by WHERE THE INK STOPS on a text line rather than by a colour: every line on that card is the same card ground behind the same two inks in all three states, so each state is the CONJUNCTION of one ink check and one ground check and NO SINGLE BAND separates all three - one that claimed to would be the vacuous green an all-black `CAPTURE PASS` already cost this board. Measured on its own captures: 1.0000 and 0.0783 (read), 1.0000 and 0.1918 (earned), 0.0756/0.0739/1.0000 (unreadable); every state fails BY NAME on both siblings (0.604, 0.612, 0.642 and three flat 0.0000s) and all seven fail on a capture of the whole dashboard. What is pinned mechanically is that EVERY PAIR OF STATES disagrees at a region BOTH of them declare (`TrainerCardRecordPresentationTests`), so no later edit can leave two states that no capture could tell apart, and that each state's seeded record still binds to the state its capture is named for. The capability surface is verified by TIER-2 UIA TEXT NEEDLES inside `capture.ps1` (`capability display-session: Available` etc.) — pixel checks cannot read text, so no manifest check claims it. No speculative checks for surfaces that do not exist (A-014). Each new surface adds its checks with its own task.
- Checks are scoped to REAL current consumers only. **This line named the original demonstrator card until the rack work, and had been stale since the re-anchor retired it. It was stale AGAIN — it said eight after the Goon page and the Trainer Card had added four more — and is corrected on every change by counting the manifest rather than by memory.** The thirty-six checks today are (counted from the manifest, never from memory - this line has now been stale four separate times, and the count is the least useful thing on it): the rail door's selected/unselected border bands and the dashboard background (shell); the Studio rack's selected marker, selected fill, unselected ground and its state dot armed/off (rack); the Goon page's backdrop and explainer card; the Trainer Card's ground and ink; the session rack's two difficulty stripes and its start button's two liveries; the Recent Sessions list's first-row cell in the two states upstream's retention rule produces; the AI permissions grid's panel fill against the window ground it replaces; the Studio dial's track in the two liveries a running session produces, on a control the session LOCKS but never WRITES so both captures are the same control at the same value; the toast's accent in the two states a real save and a real refusal produce; the companion's privacy dial and its transcript window; and the POP QUIZ CARD's ground and question ink, whose ink floor is set an order of magnitude BELOW the lowest of four measurements because the question is drawn at random and its inked fraction moves with the card - a floor near the mean redded a perfectly good card on its second run; and the TRAINER CARD LEVEL BAR in its two states, which is the first pair on this manifest whose claim is a FRACTION rather than a colour - the same 420x8 DIP bar painted in the same two colours in both captures, separated only by how much of the track the fill covers, and the band that reads it sits at 0.10..0.30 against a fill reaching 0.5949 because the pop quiz lesson is that a boundary set near a value the product moves reds a good capture. And the TYPED MANTRA GAME's own line in its two states, which is the first pair here taken inside a MAXIMIZED, CHROMELESS window that covers the shell: the same derived band of the game's own line, dim before a keystroke and lit after all but its last two characters have arrived on a real keyboard (`mantra-window-fresh-dim` 0.281 on its own capture, 0.003 on the other's and 0.000 on a capture of the whole dashboard; `mantra-window-typed-lit` 0.273 and 0.000 EXACT on both of the others). The mantra is DRAWN AT RANDOM, so a pair may or may not show the same sentence - the band is cut from the line's own UIA rect rather than fixed, the floors sit an order of magnitude below the measurements for the pop quiz card's reason, and TWO pairs were taken (one with two different sentences at two different Viewbox scales, one with the same sentence twice) which inverted identically - that indifference is what says the checks read the character state and not the layout. The capability surface is verified by TIER-2 UIA TEXT NEEDLES inside `capture.ps1` (`capability display-session: Available` etc.) — pixel checks cannot read text, so no manifest check claims it. No speculative checks for surfaces that do not exist (A-014). Each new surface adds its checks with its own task.
...
**A VOLUME DIAL IS NOT IN THIS CLASS AT ALL, and the `audio-dial` captures must not be read as if it
were.** Those two captures are `presentation-verified`: they say the master volume slider is drawn,
is at the value the document holds, and stays the user's while a scripted session runs. Nothing
about them touches the device. Nothing anywhere proves the number that dial writes changes how loud
anything is — this build has exactly one reader of it (`Features/Dtrh/DtrhHostWindow.axaml.cs:268`
into `Companion/BarkPipeline.cs:613`, taken when the window is built), and the video volume beside
it has none at all. The panel says both in words rather than leaving the picture to imply an effect;
the pixel checks say only that the picture is right.
