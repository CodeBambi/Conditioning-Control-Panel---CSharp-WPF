...
- Checks are scoped to REAL current consumers only. **This line named the original demonstrator card until the rack work, and had been stale since the re-anchor retired it. It was stale AGAIN — it said eight after the Goon page and the Trainer Card had added four more — and is corrected on every change by counting the manifest rather than by memory.** The twenty-six checks today are: the rail door's selected/unselected border bands and the dashboard background (shell); the Studio rack's selected marker, selected fill, unselected ground and its state dot armed/off (rack); the Goon page's backdrop and explainer card; the Trainer Card's ground and ink; the session rack's two difficulty stripes and its start button's two liveries; the Recent Sessions list's first-row cell in the two states upstream's retention rule produces; the AI permissions grid's panel fill against the window ground it replaces; the Studio dial's track in the two liveries a running session produces, on a control the session LOCKS but never WRITES so both captures are the same control at the same value; and the toast's accent in the two states a real save and a real refusal produce. The capability surface is verified by TIER-2 UIA TEXT NEEDLES inside `capture.ps1` (`capability display-session: Available` etc.) — pixel checks cannot read text, so no manifest check claims it. No speculative checks for surfaces that do not exist (A-014). Each new surface adds its checks with its own task.
