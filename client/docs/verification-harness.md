...
- `CopyFromScreen` reads the composited desktop, so unlike SP-115's `PrintWindow` it is not structurally blind to transparent-versus-black. It IS blind to a foreign top-most window owning the point, and the lease cannot exclude one (`RealDesktopCollection.cs:44-48`). That residue is unchanged.
