...
- Checks are scoped to REAL current consumers only. **This line named the original demonstrator card until the rack work, and had been stale since the re-anchor retired it.** The eight checks today are: the rail door's selected/unselected border bands and the dashboard background (shell), and the Studio rack's selected marker, selected fill, unselected ground and its state dot armed/off (rack). The capability surface is verified by TIER-2 UIA TEXT NEEDLES inside `capture.ps1` (`capability display-session: Available` etc.) — pixel checks cannot read text, so no manifest check claims it. No speculative checks for surfaces that do not exist (A-014). Each new surface adds its checks with its own task.
