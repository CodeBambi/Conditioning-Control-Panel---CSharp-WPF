...
- **Tear down everything the wave scoped, not just the lock.** Closing `.port/WAVE-LOCK` is machinery and it works; the rest was memory, and memory does not survive five waves and a context compaction. At every land also: **remove the landed lanes' worktrees** (`git worktree remove <path> --force`, about 10GB each — verify redundancy first by diffing the lane's OWN files against the branch, and expect `floor-delta.json` to be the one difference because the land deletes it by design), and **stop any monitor or background watcher armed for that wave** (`TaskList`, then `TaskStop`). Learned 2026-08-18: a wave-31 resource monitor ran for 70 hours past its wave and an SP-090 worktree held 10GB of an already-merged branch, because the land checklist has no step that names them.
