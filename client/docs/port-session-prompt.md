...
8. **Consult gates (continuous mode — owner decision 2026-07-18):** the run does not stop for per-task owner approval. Solo consults on `anthropic/claude-fable-5` gate phase decomposition, each packet's pre-approach/pre-completion, and every land of P0/high-risk work; record verdicts in task evidence. When Phase 1 completes, re-derive the next phase scope from unblocked board rows (spikes landing may unblock BLOCKED rows), consult on the decomposition, update `spine-tasks/CONTEXT.md`, and continue. **Pause protocol:** if the Fable 5 consult route errors or times out, assume the 5-hour subscription window is exhausted — park in-flight work safely (spine state is durable), write `.spine/handoff.md`, delete/pause loops and monitors, and stop until the owner resumes with this prompt. Never proceed past a required gate unconsulted; never downgrade a gate to a weaker model. Rows needing genuinely unavailable headed/manual evidence stay `WIP`/`BLOCKED` with the exact gate named — they are not failures and do not stop other work.
