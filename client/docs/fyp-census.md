...
`...tiFYProperty`). Unanchored, the shipping tree reports 100+ files; anchored so the `f` is not
preceded by a letter, 53. My plan anticipated this for `feed` and `ghost` and **failed to anticipate
...
All 14 FYP settings properties, and the consent union, are defined in
...
| Premium entitlement | Out of scope here and already a named blocker: every entry point runs `TierGate.RequiresPremium(..., "fyp")` (`App.xaml.cs:2462`, `MainWindow.Lab.cs:291`). |
