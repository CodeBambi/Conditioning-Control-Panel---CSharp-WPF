...
| **D263** | **The `IntakeHostContext.cs:126-127` half of the board row cannot be seeded, and the packet step that asked for it was defective.** `IntakeHostContext.cs` is a PORT file (`client/src/CcpClient.Desktop/Features/Intake/IntakeHostContext.cs`); no file of that name exists anywhere under `ConditioningControlPanel/` | **Out of scope, structurally, and NOT worked around** | The inventory's 297 entries are all `ConditioningControlPanel/`-keyed and the detector's universes are built from that tree alone (`detect.mjs:386-390`, `:406-410`). Widening the key space to port paths would put a spurious `CITATION-GONE` row (`:580-593`) on every port entry the regenerator does not credit, plus a `DELTA-MISMATCH` on any carrying `changedAtSync`, since numstat is scoped `-- ConditioningControlPanel/` (`:319`). **The port-internal citation class is real and unguarded** — the surviving instance is `PersistenceStore.cs:204` citing `IntakeHostContext.cs:212-214`, a port file citing a port file — and the place that class already lives is SP-129's §10.5 port-anchor table. **A separate board row, not this one.** Confirmed at the plan gate as an authoring error in the packet |
