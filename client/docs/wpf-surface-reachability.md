...
| **D207** | For You Feed is reachable upstream by three real gestures, all premium-gated: the Play page card calls `ShowTab("fyp")` which `MainWindow.TabNavigation.cs:130` intercepts because *"fyp is a window, not a tab"*; the Exclusives/premium rail chip routes through `MainWindow.PremiumRail.cs:335`; and `--fyp` opens it directly (`App.xaml.cs:2458`), each landing on `TierGate.RequiresPremium(..., "fyp")` (`App.xaml.cs:2462`, `MainWindow.Lab.cs:291`) before `FypHostService.Launch()` | **No port entry point, and SP-125 deliberately did not add one.** The surface is REFUSED with an inventory (`client/docs/fyp-census.md`) rather than sized | Ghost mode — the row's own title behaviour — is a DWM live-thumbnail mirror with no mechanism in the port on Windows and no analogue at all on Wayland (§4 of the census). A door to a surface that cannot render its defining behaviour would be a worse divergence than no door. Recorded now so the absence is a decision on the record rather than a gap someone rediscovers |
...
| **D209** | The 14 FYP settings properties and the consent union `HasRemoteMediaConsent => _remoteMediaConsented || _fypOnlineConsented` are defined in `ConditioningControlPanel/CCP.Core/Models/AppSettings.cs:3143-3327` — inside the tree `docs/constitution.md:32` classes as first-attempt "lessons/failure evidence only" | **Read as behavioural evidence of the SHIPPING product**, because `ConditioningControlPanel.csproj:52` carries `<ProjectReference Include="CCP.CoreCCP.Core.csproj" />` while `:10` excludes those sources from direct compilation. No topology imported, no status claim admitted | Flagged rather than resolved unilaterally: the constitution's read-only classification and the shipping csproj disagree about what `CCP.Core` is, and a census that silently picked one would be deciding an owner question. The narrow resolution used here — a linked project is behavioural evidence for the product that links it — is the smallest one that lets the settings surface be cited at all |
