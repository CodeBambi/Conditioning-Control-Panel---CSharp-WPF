...
| **D102** | Both services keep the whole rack row's dot on the persisted flag and play whenever the roll wins, with the endpoint gate one call deeper (`MindWipeService.cs:770`, `BrainDrainService.cs:211` — "endpoint down — stay quiet, don't spin") | The dot has a **second clause read back from the OPERATING SYSTEM**: `Live = a firing is on the clock && the OS reports an active render session for this process` | The **dot's fifth meaning**, and the first that is a claim about a resource this process does not own and cannot see (`Effects/AudioCueEffect.cs`). It is the SP-105 rule applied to a channel nobody can check by looking: a module playing into a machine with no output is not running, and on audio the lie is inaudible. Upstream draws the same line, one call deeper |
| **D103** | Each service opens a fresh `WaveOutEvent` **per clip** (`MindWipeService.cs:783`, `BrainDrainService.cs:231`) and had to move the whole build off-thread because that is a half-second blocking call that hitched the app every few seconds (`MindWipeService.cs:779-781`, issue #890) | **One playback device for the whole app**, opened at the first arm of an audio module and disposed at participant stop; the per-clip cost is a player on the shared MasterMixer | The port's backend is SoundFlow/miniaudio with one device and a mixer (`Audio/SoundFlowAudioBackend.cs`, audio-backend-spike.md §6). Same user-visible outcome, and the hitch upstream engineered around does not arise. It also means this process holds a WASAPI session for as long as an audio module has been armed — visible in the volume mixer, which upstream's per-clip devices are too |
| **D104** | Clips ship WITH the app: `Resources/sounds/mindwipe` (one file) and `Resources/sounds/braindrain` (an empty folder in the shipping tree). A missing folder is CREATED on scan so the user can find it (`MindWipeService.cs:152-157`) | Clips are read from `<dataDir>/assets/sounds/<module>` — the same user-media root every other port pool uses — and a missing folder is **not created**; the panel names the path instead | Those bytes belong to the legacy tree and are never forked into `client/` (the payload rule). A pool scan that writes to disk is a side effect in the middle of a read, and the port's standing answer to "where do I put them" is the panel naming the folder, which is the shape the flash panel already uses. A fresh install therefore has an EMPTY pool, which is a state upstream also has and handles identically |
| **D105** | Mind Wipe's `Volume` setter pushes a moved slider onto the **clip that is playing** as well as the next one (`MindWipeService.cs:103-118`), and Brain Drain re-reads master volume at publish (`BrainDrainService.cs:282`) | A moved volume slider takes effect on the **next cue** | `IAudioPresence` has no live-gain seam. The window in which the two differ is one short clip. **Closes with a per-player gain call on the presence** |
