...
| **D91** | WPF's services have no generations: a stop is a flag and a set of windows, so a stop that arrives after a restart simply stops the restarted thing | The port **refuses a stale generation's release**: a teardown belonging to a cancelled generation no longer takes a newer generation's work down, and the test and the release are one act under the module's gate (`Session/OwnedSessionEffect.ReleaseIfStillOurs`) | A port-side defect found by this module and fixed for all four (record §4). The parked operation's tail runs on a thread-pool continuation, so it can arrive after the module has been switched back on — and for a MOVING module it would stop the picture as well as taking it down. It has no upstream counterpart because upstream has nothing to be stale about. **The predicate is pinned deterministically** (both arms, proved to red 5/5 with the guard deleted); **the ATOMICITY is argued, not pinned** — distinguishing "read then release" from "read-and-release" needs a thread parked between the two, which needs a cross-thread rendezvous and a bounded wait. Both halves of that disposition are stated in record §4.1 |
...
`presentation-verified` remains the orchestrator's; every part of Linux, where the overlay refuses by
design and this module therefore arms, reports its refusal verbatim and shows nothing; multi-monitor
(D73); the holders, pulses and recovery paths of D74/D75/D76, which own the spiral upstream too; the
ATOMICITY half of D91's fix, which is argued rather than pinned (record §4.1); and one unexplained
3-failure run whose test names were lost and which has not recurred in eleven clean suites since
(record §6).
