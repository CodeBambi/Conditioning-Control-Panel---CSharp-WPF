...
| **D142** | Bubble Pop has THREE render paths — per-window (cap 3, `MAX_BUBBLES` at `:25`), a Canvas shared host and a compositor `BubbleLayer` (cap 40, `:26`) — and the two hosted paths decide which bubble was hit in USER SPACE from a disc snapshot rebuilt once per UI tick, read on the global mouse hook's own thread (primer §4c/§4e, gotcha 4) | **Only the per-window path is ported**, and its cap of three comes with it | SP-110 predicted this packet's central race: *the hit test's answer is a function of a position that changes between asking and clicking*. The hosted paths HAVE that race by construction; the per-window path does not, because the arbiter is the window manager at the instant of the click over the rectangle the OS itself holds. Taking the per-window path removes the race from the product and leaves only a bounded staleness in the CONFIRMATION — one animation step, `12.81` px worst case against a `30` px smallest legal radius, measured in both directions with real clicks. The cost is upstream's own: three bubbles at a time, because every move is a `SetWindowPos` (`:25`), and the shared host's dense field of forty is not available |
...
| **D148** | The bubble spawn goes to any monitor when `DualMonitorEnabled` is on (`BubbleService.cs:851-857`) | Primary display only | D66's standing single-display limit, unchanged. It is the play AREA that is limited, not the capability: `IPointerSurface` takes virtual-desktop physical pixels and would place a target on any display it was given |
