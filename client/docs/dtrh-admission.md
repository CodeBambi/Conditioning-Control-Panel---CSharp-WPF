...
Identical on both platforms: loopback origin serving (SP-011 L8 — shared assembly), the protocol v1 message vocabulary, the boot contract (ready → init+manifest → engine live). Per-platform divergence: host→page channel only.
...
4. **MIME allowlist:** fixed extension→type table (html/js/mjs/css/json, png/jpg/jpeg/gif/webp/svg/ico, webm/mp4, mp3/wav/ogg — the spike's table is the starting set), `X-Content-Type-Options: nosniff` on every response, unknown extension → **415 deny-by-default** (tightening over the spike's octet-stream fallback; the payload's extension set is finite and known).
...
| **b3** | Native SFX/audio/video + freeze + rendered tint safety | native SFX/audio/video, freeze, rendered tint safety | SFX cues (native-cue message path per W15), audio/video playback, freeze behavior, tint layering over the rendered web surface — **WH** (pixel-verified); Linux render/session facts — **WX** |
