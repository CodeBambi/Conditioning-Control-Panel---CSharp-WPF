...
- **Council consult attempted and FAILED** (2026-07-18): synthesizer route `kimi-openai-completions` not registered. `.pi/providers/kimi-coding/config.json` exists on disk and the board's probe row says "Restart trusted Pi" first, so this is evidence the council probe is unfinished — not yet proof the roster is broken. Recorded as partial evidence for the OPEN "Probe bpx-consult council and task integration" row.
...
- [ ] Owner has reviewed scope, order, exclusions, and gates above. Date/initials: ______
- [ ] "Pilot pinned pi-task workflow" row `DONE` on the board.
- [ ] "Probe bpx-consult council and task integration" row `DONE` on the board.
- [ ] Sentry mitigation decided **or** MCP declared out of scope for this run.
- [ ] Row-6 surface approved. Pre-declared candidate: **main dashboard window with one real toggleable feature card** — exercising selectors/pseudo-classes, compiled bindings including a named/ancestor case, one direct `ICommand`, `IsVisible` layout intent, an `avares://` asset, keyboard/pointer input, scaling, and teardown (the board row's own acceptance list).
- [ ] Linux execution environment (X11 + Wayland session) identified and available, **or** owner accepts that rows 1–5 and 7–9 end `WIP`-until-Linux under the documented-blocker rule.
