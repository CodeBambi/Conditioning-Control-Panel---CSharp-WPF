...
- Project profile: `.spine/spine-config.json` — worker `kimi-coding/k3` (high), reviewer `anthropic/claude-fable-5` (medium), plan reviewer `uva/gpt-5.6-sol` (low), `lanes.maxParallel` 3 (the kimi-coding provider allows 4 concurrent sessions; one slot is reserved for the orchestrator's own K3 use — workflow medium tier, K3 image review, council synthesizer), base branch `feat/crossplatform`, contract mode required. A machine-local `pi.exe` shim at `%APPDATA%\npm\pi.exe` (source `%USERPROFILE%\pi-exe-shim`) makes `pi` spawnable by Node without a shell — required for subprocess workers and cross-model reviewer spawns on Windows; rebuild it if the pi npm install path changes. Worker standing orders: `docs/constitution.md`. Packets + execution phases: `spine-tasks/`.
...
- Status: **owner-admitted 2026-07-18 as advisory** for UI/effects/dashboard AXAML packets (Sentry risk accepted under a snippets-only redaction policy; still never an implementation authority). Connection health must be verified at session reconciliation before packets rely on it. Historical audit context: originally optional and not admitted as an implementation authority. Upstream has no release, one June 2025 commit, Avalonia `11.3.1`, .NET 9, and preview MCP dependencies. Pi had no cached `avalonia` tool inventory at inspection time, so successful connection/tool discovery remains unproven.
...
- **Blind audit after every HEAD-moving phase.** A fresh `pi -p -ne -ns -nc -np -t read,bash,grep,find,ls` process — no extensions, no skills, no context files, no spine/consult/MCP tools, no memory of the session that did the work — re-derives the floor from the pushed tree and compares it against the claims in `port-digest.md` and `CONTEXT.md`. It **fails closed**: a count mismatch in either direction, any skip, a dirty or unpushed tree, or a missing verdict line all write `.spine/STOP`. Every failure this project has actually shipped (stale gate evidence, a red base, a vacuously green pin) was self-certified by the context that produced it; this is the cheapest structural answer. Design borrowed from `pi-goal-list-loop-audit`'s detached auditor — the package itself is deliberately NOT installed (it drives its own goal queue and would compete with spine and the board; its own docs state the Two-Driver Rule).
- **The auditor's prompt lives in `client/tools/port-audit-prompt.md` and is passed as `@file`.** A multi-line argument does not survive `pi.cmd` (a batch shim): the first implementation inlined the prompt, cmd.exe delivered nothing, the auditor asked what to audit, and the fail-closed default halted the loop on a false FAIL. Edit the prompt file, never inline it.
- **Auditor model must be provider-resolvable without extensions.** Verified 2026-08-12: `anthropic/claude-opus-5` and `kimi-coding/kimi-for-coding-highspeed` resolve in an extension-less session; `kimi-api/k3` does NOT (`Model not found` — it is extension-registered). Re-check with a one-line probe before changing `-AuditModel`.
