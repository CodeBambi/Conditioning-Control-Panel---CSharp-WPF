...
9. **Trust your own merged-state run over any reported evidence.** Gate artifacts have been stale, and at one land were a base-tree run reporting failures the lane had already fixed. The decisive check is `git diff` EMPTY between the scratch tree you verified and the integrated tip.
