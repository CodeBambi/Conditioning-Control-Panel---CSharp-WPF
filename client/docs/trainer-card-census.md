...
|---|---|---|---|---|---|---|
| B1 | Profile rebuilt as a Trainer Card | `MainWindow/MainWindow.ProfileCard.cs` (337 lines) + `Views/Tabs/DiscordTabView.xaml` (1077) | A page that renders a portrait, a name, a level and a stat block from local state | `client/src/CcpClient.Desktop/Features/Progression/TrainerCard.cs` + `Views/Pages/IntakePage.axaml` — the card, mounted on the Graded Intake page | **PARTIAL, and the half that landed is the half the port can stand behind.** *Amended 2026-08-23, later the same day:* an XP and level store now EXISTS (`Features/Progression/ProgressionLedger.cs` over `progression.json`, fed by the intake, the descent and the Arcademy), so the earlier "no XP, level, streak or rank exists anywhere in `client/src`" holds only for streak and rank. No surface renders the level yet — that is this row's remaining work, not the ledger's — and `TrainerCard.LevelNote (renamed 2026-08-24 when the Trainer Card began rendering the level; the note now says the level unlocks nothing)` says exactly that rather than claiming none exists. So the card renders the award ledger and SAYS, in words a user reads, what it does not show, and that an unreadable record is not the same as an empty one. Still missing: the portrait, the name, the level READOUT, the pinned badges and the banner — the level now has state behind it; the rest still needs state or bytes this build does not have | Windows: unproven — gate: a headed capture of the page; the headless facts are draw-level only. **Linux: unproven** — no WSL distro on this machine (`client/memories/port-status.md:89-93 @ a8d32c219`) |
