...
3. `async void` is forbidden except for genuine event handlers (UI/lifetime events that have no caller to observe a task — e.g., the lifetime `Exit` handler shape the startup contract already uses). An event handler that starts required work must register that work as an owned operation.
...
- `Failed(kind, reason, exception?)` — the operation faulted; `kind` reuses the startup contract's failure classification vocabulary:
  - `Recoverable` — the operation failed; the owning feature continues without it, with a named reason. **Activated by this contract** (reserved in the startup contract §2 with "first consumer is row 3").
...
The classification vocabulary is shared with `InitFailureKind`; the outcome type is separate (it also carries `Completed`/`Cancelled`, which are not failure kinds). The startup taxonomy itself is untouched.
...
4. The panic backstop is unchanged: `TaskScheduler.UnobservedTaskException` stays installed from startup phase 1 as a last-ditch logger. Because every required operation's completion is owned and awaited, that hook must never fire for registered work; it is untested here (GC-timing nondeterminism) and its silence is asserted indirectly by the zero-unobserved rule (§7 rule 4).
...
5. Posted delegates must be harmless if they run late or never: during teardown the UI thread is blocked inside `ShutdownAsync` (the startup contract invokes it synchronously from the lifetime `Exit` handler), so queued posts may execute stale or not at all. The generation check inside the delegate is what makes this safe.
7. **An abandoned construction must never reach the live system.** When a caller's bounded wait for a resource construction expires, the construction keeps running — so the abandoned result never reaches the mixer, never plays, and is disposed **exactly once**; its disposal is **ordered** against device teardown by the backend's lifecycle lock, and caller-side waits on that lock are always bounded. Added at a later land (the packet is read-only for this file; wording proposed by the worker and applied by the orchestrator, per the standing precedent). Rule 6 bounds the wait; this rule governs what happens to the work the caller stopped waiting for. **Bounding without it is strictly worse than blocking:** a freeze becomes a ghost sound plus a leak.
6. **Teardown paths reachable from the UI thread must bound their wait on any native or backgrounded work.** The unbounded portion runs off the UI thread, and the give-up path must not touch the resource the backgrounded teardown owns. Added at a later land: rule 1 removes the *awaitable* deadlock class by construction, but a `lock`/`Join`/native call taken directly on a UI-thread teardown path is the same hazard reached without the dispatcher — `SoundArbitration.Dispose` held `_initLock` across a wedged native init on the DTRH host close handler. A timeout that then **continues** is not a fix: it reintroduces the concurrent-native-call class the lock exists to prevent. Bound the *observation*, never the lock — and the backgrounded portion must not hold an OS thread waiting on that lock either. It publishes the owed disposal and exits; the thread holding the lock performs the teardown after its own release, at the same instant the parked thread would have. **The owed flag must be read after the release, never inside the lock body:** a holder that reads it inside can be preempted before releasing while the hand-off's bounded acquisition fails, and the resource is then never disposed at all. Extended at a later land (worker proposes, orchestrator applies — the standing precedent); the read-inside-the-lock variant was caught at plan review, before implementation, as a permanent native-device leak.

## 6. Shutdown ordering for async work

Teardown remains the startup contract's single guarded entry point (`ApplicationHost.ShutdownAsync`). This contract extends its body; there is **no second teardown path** for async operations:
...
3. **Record unobserved** — operations that have not completed after the bounded wait are recorded via the `ILogSink` seam and remain counted in registry state. The startup contract's "teardown never throws" invariant stands: the zero-unobserved guarantee is asserted by **tests reading registry state**, not by teardown throwing.
4. **Stop participants** — the existing reverse-order participant stop runs, unchanged.
...
3. **No blanket catch-as-success** — a faulting required operation surfaces a typed outcome, synchronously awaitable through the registry. Tests assert deterministic registry routing; they do **not** test via `TaskScheduler.UnobservedTaskException` (GC-timing flaky; that hook stays as the untested startup-contract backstop).
4. **No `SynchronizationContext.Current` capture** (§5 rule 2).
5. **No second teardown path** — in-flight cancellation and completion-drain flow through the startup contract's guarded `ShutdownAsync` only.
...
- All startup-contract tests still pass; the single guarded teardown entry point is undisturbed.
