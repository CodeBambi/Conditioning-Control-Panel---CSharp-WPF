...
### 1. Approved OpenAI-compatible endpoint — cancellation
...
6. **Finding F1** — duplicate-key validator gap reported for follow-up (owner decision: reject-on-duplicate vs document last-wins).
7. Retry policy values (one bounded retry, ≤2s clamp) are the WPF-observed shape used by the spike harness, not an owner-approved contract value.
