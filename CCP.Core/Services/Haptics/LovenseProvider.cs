...
namespace ConditioningControlPanel.Services.Haptics
{
    ...
    public class LovenseProvider : IHapticProvider
    {
        ...
        public async Task<bool> ConnectAsync()
        {
            try
            {
                Log.Information("Connecting to Lovense in {Mode} mode at {Host}", _mode, UrlLog.Host(_baseUrl));
                ...
                Log.Information("Lovense GetToys response ({Bytes} bytes)", content?.Length ?? 0);
        ...
        private bool ParseToys(JsonElement toys)
        {
            ...
            foreach (var toy in toys.EnumerateObject())
            {
                ...
                Log.Information("Found toy: {Id} ({Type}, battery {Battery}%, nicknamed={HasNick})",
        ...
        public async Task VibrateAsync(double intensity, int durationMs)
        {
            ...
            try
            {
                ...
                if (_mode == LovenseConnectionMode.Lan)
                {
                    ...
                    Log.Information("Lovense sending Vibrate:{Level} for {Seconds}s (LAN)", i, sec);
                    var json = new StringContent(cmdJson, Encoding.UTF8, "application/json");
                    var response = await _client.PostAsync($"{_baseUrl}/command", json);
                    var responseContent = await response.Content.ReadAsStringAsync();
                    Log.Information("Lovense response: {Status} ({Bytes} bytes)", (int)response.StatusCode, responseContent?.Length ?? 0);
                }
                else
                {
                    ...
                    Log.Debug("Lovense sending GET Vibrate to {Host} (level {Level})", UrlLog.Host(url), i);
                    var response = await _client.GetAsync(url);
                    var responseContent = await response.Content.ReadAsStringAsync();
                    Log.Debug("Lovense response: {Status} ({Bytes} bytes)", (int)response.StatusCode, responseContent?.Length ?? 0);
                }
            }
        ...
        public async Task VibratePatternAsync(int[] levels, int totalDurationMs)
        {
            ...
            try
            {
                ...
                if (_mode == LovenseConnectionMode.Lan)
                {
                    ...
                    Log.Information("Lovense sending Vibrate:{Level} for {Seconds}s (LAN pattern)", levelToSend, durationSec);
                    var json = new StringContent(cmdJson, Encoding.UTF8, "application/json");
                    var response = await _client.PostAsync($"{_baseUrl}/command", json);
                    var responseContent = await response.Content.ReadAsStringAsync();
                    Log.Information("Lovense response: {Status} ({Bytes} bytes)", (int)response.StatusCode, responseContent?.Length ?? 0);
