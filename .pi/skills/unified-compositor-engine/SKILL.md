...
5. **Draw persistent `SKImage`s, not `SKBitmap`s.** `SKCanvas.DrawBitmap(SKBitmap, ...)` recreates an `SKImage` every call (~480 MB/s of allocations measured in `VideoLayer`). Convert once, cache, draw the `SKImage`. Note: fixing `VideoLayer` itself is plan Phase D and is still OPEN; the code still calls `DrawBitmap` per frame today.
