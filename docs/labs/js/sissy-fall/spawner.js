...
import { Q } from '../rabbit-hole/quality.js';
import { isMuted } from '../rabbit-hole/audioMute.js';
import { getLevel } from './audioLevels.js';
import { loadTexture } from '../rabbit-hole/assets.js';
...
export function createSpawner({ scene, layout, media, renderer, camera, onCardApproach, onVeilPass }) {
  ...
  async function loadGifReader() {
    if (!gifReaderCtor) {
      ({ GifReader: gifReaderCtor } = await import('../../assets/vendor/omggif/omggif.module.js'));
    }
  ...
  function ensureGifWorker() {
    ...
    try { gifWorker = new Worker('js/sissy-fall/gifWorker.js', { type: 'module' }); }
  ...
  const VEIL_KINDS = [
    { kind: 'spiral', w: 5, url: '../../assets/bubbles/effects/spiral.png', spin: true },
    { kind: 'pinkfilter', w: 5, url: '../../assets/bubbles/effects/pinkfilter.png' },
    { kind: 'glitch', w: 4, url: '../../assets/bubbles/effects/glitch.png' },
    { kind: 'flash', w: 3, url: '../../assets/bubbles/effects/flash.png' },
  ];
