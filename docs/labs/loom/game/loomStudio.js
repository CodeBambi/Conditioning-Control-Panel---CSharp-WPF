...
export function createLoomStudio({ bridge, sfx, previewSize = 288, hotkeys = false }) {
  ...
  function ensureWorker() {
    ...
    try {
      worker = new Worker(new URL('../engine/loomWorker.js', import.meta.url), { type: 'module' });
  ...
  function onResult(res) {
    ...
    if (res.op === 'save') {
      ...
      if (res.ok) { status = `woven — ${res.slug}.gif is in your downloads.`; if (sfx) sfx('boon_pick', 0.4); }
      else status = ({
