 ...
 * the loop timing still matches the preview); >HARD_CAP errors out. Downloads are
 * client-side (bridge.js), so the caps just keep files sane, not bandwidth. No
 * WebGL in the worker? Frames fall back to the v1 drawSpiral projection - same
 * look the preview falls back to.
 *
 * Resolution/frames match the preview: SIZE is the 768 preview backing, and the
 * frame count per loop comes from framesFor2() (the same table loopMs2 uses), so
 * the file is as sharp and as smooth as the pane.
 * ==========================================================================*/
...
// Matches the standalone page's 768 preview backing (loomBoot.js).
const SIZE = 768;
const RETRY_SIZE = 576;   // oversize fallback: fewer pixels, SAME frames (timing intact)
// Dithering scatters neighbouring pixels, so LZW compresses a touch looser -
// caps are a bit higher than the flat-nearest days. Retry still keeps frames.
const SOFT_CAP = 12 * 1024 * 1024, HARD_CAP = 22 * 1024 * 1024;
