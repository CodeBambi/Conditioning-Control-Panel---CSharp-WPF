...
public class FavoritesRailArtTests
{
    ...
    public void Every_art_path_exists_on_disk()
    {
        ...
        {
            // The art is shared bytes under the repo-root /Assets tree; the head links it back
            // under Resources\, so the logical path FavoritesRailArt names is unchanged.
            var full = Path.Combine(RepoRoot(), "Assets", path.Replace('/', Path.DirectorySeparatorChar));
            Assert.True(File.Exists(full), $"FavoritesRailArt names {path}, which is not under Assets/");
        }
    }
    ...
    public void The_shipped_vault_faces_are_reachable_through_the_suffix()
    {
        ...
        foreach (var suffix in new[] { "_bambi", "_sissy", "_drone", "_locked" })
        {
            ...
            Assert.True(File.Exists(Path.Combine(RepoRoot(), "Assets", themed.Replace('/', Path.DirectorySeparatorChar))),
