...
/// <summary>
/// The heads-up clock and the ladder: CCP-added time is counted gross (added only, nothing taken
/// off lowers it), per UTC month and lifetime; the ladder's number is the server's reading, asked
/// for at most every 15 minutes, only while linked with a lock picked; the wire degrades to null.
/// </summary>
public class ChasterLadderTests : IDisposable
{
    ...
    public void The_board_parses_and_keeps_ten()
    {
        ...
        var o = new JObject { ["ok"] = true, ["month"] = "2026-09", ["show_name"] = true, ["rows"] = rows, ["you"] = new JObject { ["rank"] = 17, ["name"] = "Me", ["named"] = true, ["added_seconds"] = 61 } };
    ...
    public void The_own_row_is_not_repeated_when_it_is_in_the_ten()
    {
        ...
        var board = new LadderBoard("2026-09", new[] { new LadderRow(1, "Locked A1B2C3", false, 900, false), mine }, mine, true);
    ...
    public void A_verify_reply_is_worded()
    {
        ...
        Assert.Equal(new LadderVerify(true, 3600, null), ChasterLadderApi.ParseVerify(new JObject { ["ok"] = true, ["added_seconds"] = 3600 }));
    ...
    [Fact]
    public async Task A_proxy_without_the_routes_is_simply_no_ladder()
    {
        var h = new Handler();
        var api = new ChasterLadderApi(new HttpClient(h), () => ("u_test0001", "tok"), "https://proxy.test");

        Assert.Null(await api.TopAsync());
        ...
        Assert.Equal("/chaster/ladder/verify", h.Seen[1].Path);
    ...
    private sealed class FakeLadder : IChasterLadderApi
    {
        ...
        public Task<bool> OptInAsync(bool showName, CancellationToken ct = default)
        {
            OptIns.Add(showName);
        ...
        public Task<LadderBoard?> TopAsync(CancellationToken ct = default) =>
            Task.FromResult<LadderBoard?>(new LadderBoard("2026-09", Array.Empty<LadderRow>(), null, ServerShowName));
    }
    ...
    [Fact]
    public async Task The_board_brings_the_server_name_switch_in_line()
    {
        var ladder = new FakeLadder();
        _showName = true;
        using var service = Make(ladder);
    ...
    [Fact]
    public async Task No_ladder_api_is_no_board()
    {
