...
5. Run the mechanical gate and judge the tests:
   ...
   ```
   Confirm it reports green against the pin in `client/tests/floor/floor.json`. If `total` moved, confirm it moved in the same commit as the test change and that the message states why. Then READ the new tests and judge whether they pin the claimed semantics or merely execute the code. A test that passes against a deliberately broken implementation is a finding, not coverage.
