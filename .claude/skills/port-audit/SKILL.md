...
- reconcile lane branches, worktrees, and packet records only as local evidence, never authority.
...
Use `app-visual-verification` for targeted screenshots of recent, high-risk, or suspicious surfaces, and read the captures as images. Run the broader theme/language/scaling/window matrix only at a named milestone or release gate. Do not infer interaction from still images.
...
Check camera data remains memory-only, secrets use approved storage, sensitive URLs/headers/content are not logged, paths are validated, capture policy matches contracts, consent is not broadened, local HTTP servers bind/protect correctly, and advisor transcripts and packet records contain no sensitive data.
...
Put release verdicts and findings involving architecture, security/privacy, packages, browser/media, shared composition, input/windowing, or cross-platform degradation to `port-advisor` and `port-advisor-critic`. Supply primary evidence. Agreement is advisory; empirical evidence and owner decisions win. Both seats are Anthropic, so agreement between them is weak evidence and correlated blind spots are likely: weight the mechanical checks (the floor wrapper, a 0W/0E build, the scoped diff) above any verdict.
...
## Advisor verdict, dissent, and evidence reconciliation
## Release blockers
```
