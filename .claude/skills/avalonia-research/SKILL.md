...
### 3a. Optional: an Avalonia MCP server as a secondary reviewer

**Three Avalonia MCP seats are registered at user scope (2026-08-14):** `avalonia-docs` (official Build MCP, HTTP), `avalonia-ui` (local stdio, `decriptor/AvaloniaUI.MCP`), and `avalonia-live` (HTTP `127.0.0.1:3001`, which only connects while the CCP client runs with `CCP_MCP=1` and reads as disconnected otherwise, by design).

This subsection stays conditional regardless: check the seat is actually connected before relying on it, and if it is not, skip it and continue with official sources. A disconnected seat is a named limit, never a blocker, and never a reason to delay a decision.

If the `decriptor/AvaloniaUI.MCP` server is registered and connected, use its focused tools after reading the client contract and before finalizing Avalonia-shaped code. It is useful for:
...
The upstream server also configures a fixed external Sentry DSN with tracing and profiling at startup. Until the local installation is verified to disable outbound telemetry, do not send proprietary source, secrets, paths, user data, or security/privacy-sensitive material to it. If the MCP is unavailable or not admitted, continue with official sources; absence is not a blocker.
...
After primary research, and only then:

- `port-advisor` for a bounded API interpretation you have already researched;
- `port-advisor` and `port-advisor-critic` together before dependency admission, native interop, rendering/windowing/input architecture, security/privacy, or cross-platform capability decisions;
- both seats on the same question when two materially different approaches remain viable, then reconcile the disagreement yourself rather than averaging it.

Supply sources and alternatives. Advice is advisory and cannot replace runtime evidence. Both seats are Anthropic models, so their agreement is weak evidence: prefer one claim you can check against a build or an official page over any verdict.
