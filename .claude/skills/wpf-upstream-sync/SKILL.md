...
- `git checkout main` mid-wave is destructive: `main` carries neither `client/` nor `spine-tasks/`, so the entire port vanishes from the working tree. Use `git fetch origin main:main` instead: same result, no checkout.
