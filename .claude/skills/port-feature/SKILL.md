---
name: port-feature
description: "Implement or change one approved feature in the greenfield Windows/Linux Avalonia client under client/. Use for controls, dialogs, windows, services, overlays, media, AvatarTube, dashboard behavior, and bug fixes. Requires a client task-board row, WPF behavior archaeology, current Avalonia v12 research, advisory review, task-specific Windows/Linux verification, and durable evidence. Never ports the first Avalonia attempt literally."
---

# port-feature

Workflow: understand, plan, take advice, implement, prove, review, record.
...
- Read `client/docs/port-workflow.md` for advisory gates, wave execution, commit, and stop rules.
...
If the `avalonia-ui` seat is connected (it is registered at user scope; confirm before relying on it, and record a disconnected seat as a named limit), use its XAML validation and applicable diagnostic, accessibility, or performance tools as an advisory pass on the smallest relevant snippet. Do not invoke its project, architecture, WPF conversion, custom-control, theme, animation/storyboard, service, data, authentication, or test generators for production code. The upstream knowledge and templates target Avalonia 11.3.1 and can confidently emit invalid v12 or WPF syntax. Redact sensitive content and follow the telemetry gate in `avalonia-research`.

## 4. Plan and take advice

Use `port-plan` for non-trivial slices. Before substantive work, put the approach to `port-advisor` after gathering primary evidence. Add `port-advisor-critic` for platform seams, dependencies, security/privacy, composition, windowing, input, browser, or capability degradation. Advice is advisory and must be reconciled with evidence; both seats are Anthropic, so their agreement is weak evidence and a checkable claim beats a verdict.
...
For user-facing surfaces, use `app-visual-verification` at its targeted checkpoint and read the capture as an image. Do not run a whole-app screenshot crawl for routine edits. Screenshots supplement, never replace, headed interaction gates.
...
Run the pre-completion review with `port-advisor`; add `port-advisor-critic` for P0 or high-risk work. Reconcile advice with tests and primary sources, and record dissent rather than smoothing it away.
