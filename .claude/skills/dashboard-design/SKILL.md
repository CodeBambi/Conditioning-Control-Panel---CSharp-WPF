...
Use `decriptor/AvaloniaUI.MCP` (the registered `avalonia-ui` seat) as a bounded design-review assistant, not a dashboard generator. Confirm the seat is connected before relying on it; if it is not, record the absence as a named limit and continue.
...
For a user-facing slice, the recommended chain is: contract and screenshots -> current official v12 docs -> hand-authored smallest AXAML -> optional advisory MCP review if one is connected -> real build and headed interaction -> read the screenshot via `app-visual-verification`. Record accepted and rejected MCP findings rather than copying its response.
...
Use `port-advisor`, and add `port-advisor-critic`, for new theme architecture, major responsive layout, window-shell changes, accessibility tradeoffs, or divergence from the approved screenshots and behavior. Supply screenshots, contracts, and platform evidence.
