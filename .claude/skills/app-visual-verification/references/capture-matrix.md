...
Select the applicable rows for the task. A row is complete only when the artifact manifest identifies its contract or reference and the review verdict.
...
```json
{
  ...
  "reviewer": "the session that read the capture, or the subagent it was delegated to",
