...
`port-advisor` plus `port-advisor-critic` are required before selecting native interop, global hooks, Wayland degradation, capture architecture, or changing an effect's input policy. Stop for unknown Linux equivalence, ambiguous product policy, privacy/safety changes, or an unverified v12 mechanism.
