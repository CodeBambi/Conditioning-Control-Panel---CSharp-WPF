...
## Advisory review and stop conditions

`port-advisor` plus `port-advisor-critic` are required for render architecture, package or decoder admission, native surfaces, the input/capture split, layer-order decisions, and cross-platform degradation. When two designs remain viable, put the same question to both seats and reconcile the disagreement yourself. Stop if Linux behavior, tint safety, frame ownership, or headed proof is unresolved.
