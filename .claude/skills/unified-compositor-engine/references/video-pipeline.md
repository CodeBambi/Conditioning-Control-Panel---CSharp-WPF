...
**HISTORICAL. This file describes the ABANDONED first Avalonia attempt under
`ConditioningControlPanel/CCP.*`, which the constitution designates read-only failure evidence.**
Its present-tense statements ("now triple-buffers", "the UCE engine already creates") describe
that tree, not the greenfield client under `client/`, and its file and line references are stale
after the upstream merges. Read it for the traps it records, never as a description of current
code and never as a design to copy.
