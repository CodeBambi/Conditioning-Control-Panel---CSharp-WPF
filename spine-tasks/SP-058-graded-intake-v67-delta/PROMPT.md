...
(e) **Use SP-057's isolation seam for every headed run — it landed at `c42d82ff` and this task is its first consumer.** Concretely: set **`CCP_DATA_ROOT`** (absolute, fully-qualified — a relative or drive-relative value throws `DataRootOverrideException`; there is no fallback path) for every headed run, and reuse `spine-tasks/SP-057-profile-isolation-seam/evidence/run.ps1` as the bracket (path-hashed pre/post manifests of the real `%APPDATA%\CcpClient`, both-directions diff, positive controls). Byte-identity **without** positive controls proving the run really did the work is vacuous — include the intake equivalents (`data-root override active`, host boot, the drive script's DONE line). Claim scope inherited: `%APPDATA%\CcpClient` only (WebView2/LibVLC may write under LocalAppData/Temp — state it, do not sweep it up). The seam is currently **opt-in** (its own follow-up row); forgetting it means writing the owner's real profile.
...
- 2026-08-12 (post-SP-057 land, orchestrator): SP-057 landed `c42d82ff` — seam name is `CCP_DATA_ROOT`, honored inside `CompositionRoot.DefaultSettingsPath()`, and a guard test now FAILS the suite if `SpecialFolder.ApplicationData`/`UserProfile` appears in `client/src/**` outside `Lifecycle/CompositionRoot.cs` (do not add one in `Features/Intake/**`; route through the choke point). Test floor is now **847 unit / 33 headless**. Two SP-057 limits are DTRH-specific and do not apply here (the m2test 7/8 baseline; the m2test fixture); the DISPLAY3-absent disposition may still apply — probe `Screen.AllScreens` and fall back loudly to a visible point rather than capturing black at an unattached origin.
