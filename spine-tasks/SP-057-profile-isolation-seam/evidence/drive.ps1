# SP-026 headed driver (owner DISPLAY3 convention 2026-07-21, rect-persistence BINDING
...
# SP-026: adds the "sweep" action (grid click-sweep for run gameplay evidence).
# SP-057: -X/-Y override the placement target (DISPLAY3 absent in the 2026-08-12
# session — only DISPLAY1 attached; the run script passes a visible point instead).
param(
  [Parameter(Mandatory=$true)][string]$Action,   # dump | click | capture | clickrel | key | sweep
  [string]$Arg = "",                             # click: element Name; capture: out path; clickrel: "x,y"; key: esc|enter; sweep: "passes,cols,rows"
...
$h = $found
if (-not $NoMove) {
  # Placement target (DISPLAY3 owner convention by default; -X/-Y when absent) + verify BEFORE any capture/click.
  [W3]::SetWindowPos($h, [IntPtr]::Zero, $X, $Y, 0, 0, 0x0001 -bor 0x0040) | Out-Null
...
if (-not $NoMove -and ($r.Left -ne $X -or $r.Top -ne $Y)) { Write-Error "placement NOT at requested origin ($X,$Y)"; exit 2 }
