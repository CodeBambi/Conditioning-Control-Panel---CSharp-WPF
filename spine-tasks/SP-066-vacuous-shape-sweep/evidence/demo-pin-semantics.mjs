#!/usr/bin/env node
// SP-066 pin-semantics demonstration harness (evidence, not shipped mechanism).
// Imports the REAL verifyProjectResults / discoverTestProjects from the wrapper and feeds
// it synthetic TRX fixtures — re-demonstrates every SP-065 fail-closed mode against the
// NEW {total, allowedSkips} pin, plus the four new verdicts:
//   (i)   non-allowlisted skip -> RED naming the test
//   (ii)  the same skip allowlisted -> GREEN (may-skip semantics)
//   (iii) total drift +1 -> RED
//   (iv)  total drift -1 -> RED
// No real probe fact is perturbed (pre-approach consult correction #3).
...
const NOW = Date.now();
const PIN = { total: 898, allowedSkips: [] };

function trx({ passed = 898, skipped = 0, skippedName = "FakeSuite.FakeTests.ConditionalFact",
  failed = 0, exotic = null, outcome = "Completed", creation = new Date(NOW).toISOString(),
  truncate = false, totalOverride = null, countersPassedOverride = null, dropTestName = false } = {}) {
  const results = [];
  let id = 0;
  const push = (o, n, name) => {
    for (let i = 0; i < n; i++) {
      id += 1;
      const nm = name ?? `Fake.T${id}`;
      const nameAttr = dropTestName ? "" : ` testName="${nm}"`;
      results.push(`    <UnitTestResult executionId="${id}" testId="${id}"${nameAttr} outcome="${o}" />`);
    }
  };
  push("Passed", passed);
  push("NotExecuted", skipped, skippedName);
  push("Failed", failed);
...
const root = fs.mkdtempSync(path.join(os.tmpdir(), "ccp-floor-demo66-"));
...
const stale = new Date(NOW - 3_600_000);
const staleIso = stale.toISOString();

// --- Every SP-065 fail-closed mode, re-demonstrated against the new pin shape ---
const cases = [
  ...
  ["failed category nonzero", dir("failed", [["results.trx", trx({ passed: 897, failed: 1 })]]), "failed", PIN],
  ["outcome not Completed", dir("outcome", [["results.trx", trx({ passed: 897, failed: 1, outcome: "Failed" })]]), "did not finish cleanly", PIN],
  ...
  ["exotic outcome in result list", dir("exotic", [["results.trx", trx({ passed: 897, exotic: "Warning" })]]), "unexpected outcome", PIN],
  ["counters not self-closing", dir("counters", [["results.trx", trx().replace('pending="0" />', 'pending="0"></Counters>')]]), "not a self-closing tag", PIN],
  ["result lacking testName (unparseable)", dir("noname", [["results.trx", trx({ dropTestName: true })]]), "unparseable results", PIN],
];

// --- The four NEW verdicts (SP-066 framing e) ---
cases.push(
  ["NEW (i) non-allowlisted skip -> RED naming the test", dir("skip-red", [["results.trx", trx({ passed: 897, skipped: 1 })]]), "unexpected skip: FakeSuite.FakeTests.ConditionalFact", PIN],
  ["NEW (iii) total drift +1 -> RED", dir("drift-up", [["results.trx", trx({ passed: 899 })]]), "FLOOR VIOLATION — total drift: 899", PIN],
  ["NEW (iv) total drift -1 -> RED", dir("drift-down", [["results.trx", trx({ passed: 897 })]]), "FLOOR VIOLATION — total drift: 897", PIN],
);
...
// Positive controls, incl. NEW verdict (ii): the same skip with the name allowlisted -> GREEN.
const positives = [
  ["positive control — valid on-floor results accepted", trx(), PIN],
  ["NEW (ii) allowlisted skip -> GREEN (may-skip semantics)",
    trx({ passed: 897, skipped: 1 }), { total: 898, allowedSkips: ["FakeSuite.FakeTests.ConditionalFact"] }],
  ["NEW (ii-b) allowlisted THEORY ROW skip (testName carries arguments) -> GREEN via pre-'(' match",
    trx({ passed: 897, skipped: 1, skippedName: "FakeSuite.FakeTests.ConditionalTheory(x: 1)" }), { total: 898, allowedSkips: ["FakeSuite.FakeTests.ConditionalTheory"] }],
];
...
// Discovery fail-closed cases against the new pin shape.
const SLN_OK = `
...
EndProject
`;
const PIN_BOTH = { projects: { "CcpClient.Tests": { total: 1, allowedSkips: [] }, "CcpClient.HeadlessTests": { total: 1, allowedSkips: [] } } };
...
const discoveryCases = [
  ...
  ["pinned project absent from sln", SLN_OK, { projects: { ...PIN_BOTH.projects, "CcpClient.GoneTests": { total: 1, allowedSkips: [] } } }, "stale pin"],
...
console.log(allOk ? "ALL PIN-SEMANTICS CASES HELD" : "SOME CASE DID NOT HOLD");
