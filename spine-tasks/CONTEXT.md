...
| SP-014-quick-toggle-dispatch | Row: replace card-title quick-toggle dispatch — SCOPE to the demonstrator card with named limits (one card, one theme exist; full multi-card/multi-theme acceptance awaits real cards) | Authored 2026-07-20 (pre-authoring Fable consult applied: core = identity-by-title-string REPLACEMENT with stable-ID keying; title-mutation negative test as the falsifiable core claim; localization absent/A-014 — language+theme invariance named limits; toggle-while-popup-open cross-proof; exceptions contract-only, no fake cards; Size M) | SP-013 |
