...
| SP-005-persistence-migration-contract | Row 4: persistence + migration contract — schema authority, atomic temp+rename write, serialized writer via OperationRegistry, quarantine/Degraded, unknown-member preserve, migration journal, replacement notification, secret seam, STJ decision, teardown-flush activation, WSL2 gate in-packet | Authored 2026-07-19 (pre-authoring Fable consult applied: teardown-flush obligation, Desktop-not-Core, STJ tolerance stack, debounce policy-only; consult reply truncated — encoded in packet Amendments) | SP-004 |
