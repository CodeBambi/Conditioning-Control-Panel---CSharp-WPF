...
**Wave 45 (2026-08-19) LANDED:** SP-105 continuous effect (Pink Filter) + rack rows for Pink Filter and Subliminals (closes D72). Floor **1314/81 -> 1372/87**. Reviews: code APPROVE, final PASS. Template verdict CORRECTED: `ISessionEffect` IS a spine and fit unchanged (all nine members, no edit); `PacedSessionEffect<TFiring>` had been standing in for the spine since SP-098 because `Arm()` *was* `ScheduleNext()`. New seam `Session/OwnedSessionEffect.cs` (three abstracts, no clock). Divergences D73-D82; D4 corrected. Anti-fake-timer guard EXECUTED (clock injected, 4 facts red, reverted byte-identically) with its bound stated: it sees the session clock only. NOT proven: no headed capture, every overlay a recording double, no test proves the tint reaches a real screen, Linux is a refusal path only, multi-monitor untested.


**Wave 46 (2026-08-19) LANDED:** SP-106 moving effect. Floor **1372/87 -> 1472/90**. Reviews: code REVISE x2 then APPROVE, final PASS. Packet named Bouncing Text; lane invoked the escape clause with evidence committed BEFORE product code (per-pixel alpha refused by `Overlay/OverlayFrame.cs:14-22`; `Present` clears click-through at `Win32OverlayPresence.cs:552-576`) and built **Spiral Overlay** instead. Third template verdict: `OwnedSessionEffect` carries per-frame work with **no scheduler in the module** - the cadence rides the injected clock in the SURFACE PRESENTER. An interval deciding when a MODULE is due differs from a cadence keeping a SURFACE correct. `OverlaySurfaceSet.Repaint` added (one caller, additive). Divergences D83-D91; D5/D6 closed for the four rows that exist. **Shared-body defect found and fixed** (`ReleaseIfStillOurs`): a cancelled generation's teardown could withdraw a NEWER generation's surface. **Closing the remaining window under `Gate` produced an ABBA lock inversion and was REVERTED** - see `OwnedSessionEffect` method doc, `record.md` §4.2, D91, and the new P1 board row. NOT proven: no headed capture, every overlay a recording double, nothing proves a spiral reaches a real screen, no bundled art, Linux refusal-only, multi-monitor untested, and **motion-by-elapsed-time is untested** - do not read this wave as 'per-frame work is proven'. One unexplained 3-failure run recorded in §6, names lost, never reproduced across twelve clean suites.

**Wave 46 flake, RESOLVED IN PART at land:** the intermittent reproduced on the land's first floor run and its name was captured -- `FlashDrawTests.ARealImageFile_ReachesTheCompositedDesktop_AndLeavesItWhenTheFlashIsHidden`, SP-100's REAL-composited-desktop fact, environment-sensitive by construction and predating SP-106. This CORRECTS the `InlineDispatch`/spiral-rig hypothesis in SP-106 §6 and D91 rather than confirming it. Caveat kept: the lane's event was 3 failures, this was 1, and the other two names are still lost. See SP-106 `record.md` §6.1.


**Wave 47 (2026-08-19) LANDED:** SP-107 gate determinism. Floor **1472/90 -> 1477/90**. Reviews: code REVISE x2 then APPROVE; final REVISE then PASS. **Cause of the SP-106 flake, reproduced not inferred:** every real-desktop fixture addressed machine-global state by CONSTANT, and `with-slot.mjs --slots 3` permits three concurrent gate runs. **0/20 red sequential vs 8/12 red at 3-concurrent** before; **4/76 (5.3%)** after. Fix: `RealDesktopCollection` + cross-process `RealDesktopLease` (file handle not `Mutex` - `Mutex` has thread affinity and xunit may ctor/dispose a fixture on different threads; OS releases a handle on process death so no reaper). Exclusion pinned PER RUN. `check-floor.mjs` gained failure-name preservation ONLY - no retry - and it was demonstrated against a deliberate red. **NO `allowedSkips`, NO retry, NO weakened assertion.** **Residual: ~5% on `FlashDrawTests.ARealImageFile_ReachesTheCompositedDesktop_...`, instrumented but NOT diagnosed** - new P0 row, fix gated on which of four verdicts fires. **Provenance recorded: none of the 76 runs was taken on the landed tree.** THREE CONSECUTIVE GREENS NOW MEAN: no two gate processes fought over the desktop. They do NOT mean the tree is green.


**Wave 48 (2026-08-19) LANDED:** SP-108 non-drawing module (**Intensity Ramp**, TIMING group). Floor **1477/90 -> 1527/95**. Reviews: code REVISE then APPROVE; final REVISE then PASS. **Fourth template verdict: non-drawing `Live` is a claim about CUSTODY** - `Running = cadence scheduled && holds >=1 linked dial's base value`. Transferable corollary: **having a timer is NOT sufficient to be paced** - an interval deciding when a MODULE is due differs from a cadence keeping something correct. Seam held: `ISessionEffect`, `OwnedSessionEffect`, `PacedSessionEffect`, `SessionEngine`, `OverlaySurfaceSet` and all four landed modules byte-identical to base. Rack now **2 groups / 5 rows** vs WPF's 4/15. Divergences D92-D100. **D95 RETRACTED at review: the WPF app does NOT have the claimed mid-ramp data-loss bug** - all three exit routes call `StopEngine()` (the restore) first. **§9.5's rule, four occurrences in one packet:** *when what justifies a statement could have been produced by something not entitled to produce it, the statement is unproven* (a test double's clamp, a forbidden tree, a stale draft, a truncated grep), and *a sweep bounded by where the last instance was found is not a sweep*. The fourth instance was in the REVIEWER's own work. **Next gap is a CAPABILITY (audio), not a module** - new P0 row; it closes one row outright and half of a second, and inherits a fifth `EffectDotState` decision (new P1 row).


**Wave 49 (2026-08-19) LANDED:** SP-109 audio capability. Floor **1527/95 -> 1589/100**. Reviews: code REVISE then APPROVE; final PASS. **Seven modules of fifteen** (Mind Wipe whole; Brain Drain AUDIO HALF only). **The refusal that blocked audio since SP-098 is ANSWERED, not relocated:** four-link OS chain - `EnumAudioEndpoints` -> `IAudioSessionControl2::GetProcessId` -> `GetState` -> **`IAudioMeterInformation::GetPeakValue` measured 0 -> 0.405 -> 0**, with a device-open-nothing-cued zero control asserted unconditionally. Stops at the Windows audio engine: NOT an unmuted endpoint, a DAC, speakers, the right clip, or a human ear. `audible-verified` is a new evidence class, manual on EVERY platform. **Fifth dot meaning, REACH:** `Live = a firing on the clock && the OS reports an active render session for this process` - first dot clause about a resource the process does not own; subset sub-rule: *the dot is scoped to what the row IS, and a subset row must say so where the user reads it*. **A 24-mutation sweep found TEN survivors, SIX real holes** - the worst (M-b) meant **another application's audio would have earned this process `Available`**. Two real defects fixed: recursive clip enumeration (upstream is top-level) and a panel branch telling a running session to start. `PacedSessionEffect.WorkIsRunning` unsealed - narrow-only contract is a COMMENT, not a gate (new P1 row). D101-D109; **D108 = the shipping WPF source contradicts itself** (comment '30-second window', timer 10s, `:734` divides by 360); the port took the arithmetic.


**Wave 50 (2026-08-19) LANDED:** SP-110 input-capturing window. Floor **1589/100 -> 1648/104**. Reviews: code REVISE x2 then APPROVE; final REVISE x2 then PASS. **EIGHT modules of fifteen**; Lock Card is the first that asks the user anything. **THE TRAP, found by a throwaway probe BEFORE product code: `GetGUIThreadInfo(ourThreadId).hwndFocus` is THREAD-LOCAL and answers 'our window' while input goes elsewhere.** `SetForegroundWindow` returned FALSE, zero keystrokes arrived, thread-local read still said yes. **Only `GetGUIThreadInfo(0)` is truthful.** Five-link chain + ink differential. **A user-harm bug was caught in review:** a `Degraded` card was left on screen holding the foreground with Escape dead, suppressing every future card while the dot read `Live` - three separate safeguards pointed at it (interface doc, module doc, a test named for it) and none fired, because the recording double set `IsPrompting` only for `Available`, diverging from the product exactly at the trapping state. **Dot's sixth meaning, DEMAND:** `Live` while the prompt is up and unanswered - waiting for a human IS the work. **D112 is a standing safety rule: this port never ships a window the user cannot dismiss, even if upstream would.** D115: `AttachThreadInput` escalation, unmeasured, failure mode is a FROZEN UI THREAD with a card half-up (new P1). Sweep 68/59/7; M-m/M-p survive by REDUNDANCY not by the predicate (new P2). D110-D120.


**Wave 51 (2026-08-19) LANDED:** SP-111 video capability. Floor **1648/104 -> 1742/107**. Reviews: code APPROVE; final PASS. **NINE modules of fifteen**; Bubble Count unblocked. Five-link chain ending PAST the window at the **composited desktop changing frame to frame**, with the negative control that handing the same picture twice does NOT advance it. `MFCreateSourceReaderFromURL`/`ReadSample` is **named as the trap level**: decoded is not displayed. **V3 bit before the capability existed** - a plain topmost window's hit test answered the shipping WPF app's HWND. **Asserted not confessed:** a surface at (-8000,-8000) still reads back the picture, so a window read-back is NOT a monitor. **Dot's seventh meaning, MOTION** - firing && the OS says a surface can reach a display && (nothing playing || the OS's copy ADVANCED). First of the seven that can be FALSE while every call succeeded; explicitly not 'the decoder is fed', citing upstream's own 'useless here' (`VideoService.cs:5538-5540`). Sweep **78 mutations / 64 caught / 14 survive**; M-bw was closed in round 2, never re-run, and sat mis-columned until the ARITHMETIC didn't reconcile - round 3 re-ran it, caught. `Z:\CCP Vids` ABSENT: all media synthesised; cost confined to `clip-decodable` (M-y, M-w), new P1 row. **A lane wrote into the SHARED checkout root** (`path.join(ROOT,'..','..','..')`) - new P2 row for a guard. D121-D132.


**Wave 52 (2026-08-20) LANDED:** SP-112 second consumer (**Bubble Count**). Floor **1742/107 -> 1830/112**. Reviews: code REVISE then APPROVE; final REVISE x2 then PASS. **TEN modules of fifteen.** **Capability verdict: they generalise.** `Input/**` byte-identical; `Video/**` +1 interface +1 constant, both justified by the FIRST consumer (the constant closes a decoder leak `MandatoryVideoEffect.Compose`'s clock-thread guard could not). No eighth dot meaning - reuses MOTION and DEMAND with 'of MINE' qualifiers pinned, because surface and presence are SHARED. **Bubble Pop REFUSED with a census: `Input/**` has no mouse surface at all** - five message types, none mouse; one window; one `WindowFromPoint`; one `SetWindowPos` (no move seam); `Confirmed` requires foreground+keyboard focus, the inverse of a bubble. **It needs a SECOND capability, not a second consumer.** **Three corrections ran against the author's interest:** its red-gate attribution (the shipping app was exonerated by the reviewer's own interleaved control - base 14/14 green with the app up), its sweep books (M-al counted caught on a smoke run whose log it deleted), and its M-s equivalence proof (disproved geometrically: nearest pixel centre <= sqrt(0.5), bounding box always contains it). **The red gate was SP-110's fragility exposed by class-ordering**, not SP-112's code - fixed with `WS_EX_NOACTIVATE`. D133-D140.


**Wave 53 (2026-08-20) LANDED:** SP-113 pointer surface (**Bubble Pop**). Floor **1830/112 -> 1930/117**. Reviews: code REVISE then APPROVE; final REVISE x2 then PASS. **ELEVEN modules of fifteen.** Hardest chain yet: a click landing on a window that must NOT take focus, so it could not lean on foreground/focus as SP-110 did. Six OS read-backs + `WM_LBUTTONDOWN`/`UP` in the target's own procedure with `GetForegroundWindow` byte-identical either side + **a decoy leg that catches the click when the target moves 200px**. **The race was designed OUT** (each target its own window; the window manager arbitrates at click time), residue 12.816px vs a 30px radius, measured both ways. **METHOD RULE ADOPTED into `client/docs/port-workflow.md`:** an equivalence claim is inadmissible until every consumer of the mutated symbol is enumerated by grep and discharged by name. **Four false equivalence claims across three waves** - SP-112 M-s (forgot the pixel grid), SP-113 M-ch (forgot the null receiver, **and was asserted in shipped product source**), M-au (forgot the reader), M-aq (forgot that the painter is not the only writer). All same shape: true of ONE consumer, generalised to 'no input distinguishes'. **Port-wide gap filed: nothing mechanical observes compiler warnings** - the lane's own filter could never match `warning xUnit2013` and it had reported 0 warnings four times off a filtered stream. D141-D151.


**Wave 54 (2026-08-20) LANDED:** SP-114 warning gate. Floor **1930/117 -> 1938/117**. Reviews: code REVISE then APPROVE; final REVISE x2 then PASS. **No module - this repairs the instrument that certified the other eleven.** **TWO independent false-green mechanisms proven:** SP-113's filter could never match `warning xUnit2013`; and **MSBuild skips `CoreCompile` on a rebuild**, so a plain build over an unchanged tree holding a live `CS0219` prints 0 warnings - independently reproduced by a reviewer in a scratchpad project, defeated only by `--no-incremental`. **The second means every rebuild-based claim was unobservable BY CONSTRUCTION.** New `check-warnings.mjs`; `check-floor.mjs` untouched and pinned. Wired into `port-wave.workflow.mjs`, the auditor prompt, and at land `port-slice-executor.md` + root `CLAUDE.md`. Census was wrong TWICE (missed `.globalconfig`; missed `[assembly:`/`[module:` forms) - both found by EXECUTING the pattern, not reading it; it now self-checks against five shapes before asserting an absence. **Retroactive: 31 digest claims annotated UNPROVEN-not-false**, bounded by a genuine cold build of `4332b8b9` at 0/0. **NEW FLAKE FILED: `SpiralOverlayEffectTests.DisarmReleasesTheWorkUNCONDITIONALLY` failed once then passed on an untouched tree - contradicting SP-106 6.2's retirement of the spiral-rig hypothesis, because it touches no OS.**


**Wave 55 (2026-08-20) LANDED:** SP-115 per-pixel alpha (**Bouncing Text**). Floor **1938/117 -> 2062/121**. Reviews: code REVISE then APPROVE; final REVISE x2 then PASS. **TWELVE modules of fifteen.** Closes D83/D84, blocked since wave 46. **SP-099's ghost hazard answered FOUR ways**, one by the OS: ULW on a uniform-alpha window returns FALSE err 87 with the overlay's alpha 153 intact, asserted at four moments; `SetLayeredWindowAttributes` not declared anywhere in `Glyph/**`. **`PrintWindow` is structurally blind to transparent-vs-black - ASSERTED, not confessed.** Sweep 72/49/23. **TOLERANCE RULE ADOPTED into `port-workflow.md`:** a tolerance sized to an observed error is exactly the size of the defect it will next hide. The lane rightly refused to tune its oracle to a datum, then set a +/-1 tolerance - and review found the residue was **the oracle's own three roundings where the formula has one**. Single rounding reproduces **8 of 8** measured points; the tolerance was DELETED, not justified. **THIRD FLAKE STRAND, and the worst: the suite is intermittently red AT BASE** - the lane ran the unmodified base tree seven times and got one red on a test it had not touched. Three strands, no shared explanation. **Every three-green land claim is weaker than it reads.** D152-D170.


**Wave 56 (2026-08-20) LANDED:** SP-116 flake characterisation. Floor **2062/121 -> 2067/121**. Reviews: code REVISE then APPROVE; final REVISE x2 then PASS. **No module - it makes 'three consecutive greens' mean something again.** Protocol committed BEFORE the first sample (n=49.2 power analysis, 60/arm). Base **3 red in 60 (5.0%)**. **Strands 1+3 are ONE fault and it is NOT occlusion** - which SP-107 AND SP-115 both guessed: on all 34 misses `isOurs=True attempts=1 rendersOwnColour=True` with an **identical** `majority=0x26171E count=14323`, refuting a varying occluder too. The read had no happens-before edge with the compositor. **34/1200 unfenced vs 0/1500 fenced**; `GdiFlush` alone 8/300, `DwmFlush` alone 0/300. Fence at `FlashPixelProbe.CaptureDesktop`, the single `CAPTUREBLT` choke point (glyph and video route through it). **Strand 2: a test reaching around its own module** - a third `ReleaseWork` on a pool continuation after `Disarm` returned - forced deterministically with no wall-clock wait, which SP-106 called impossible. `client/src/**` untouched; the late release is correct idempotent product behaviour. **The lane's OWN first fix introduced a flake of the same class** (pool reuses the test thread's managed id), caught by the floor, disclosed with the red run committed. **`TestWait` verdict defect found and fixed:** every `Task`-overload expiry reported `ENVIRONMENT-STARVED` ("rerun") instead of `CONDITION-NEVER-TRUE` - in the packet whose prohibition is re-running. **RECORDED AS BOUNDED, NOT ZERO:** 0.20% fenced, 9.5% suite, six post-fix runs = 0.34 expected events. **Line-ending trap recorded in `port-workflow.md`.**


**Wave 57 (2026-08-20) LANDED:** SP-117 last three (**Visuals**). Floor **2067/121 -> 2123/125**. Reviews: code APPROVE; final REVISE then PASS. **THIRTEEN of fifteen rack rows.** **THE FINDING: the session effect SPINE IS COMPLETE and the PRODUCT IS NOT AT PARITY.** Census committed BEFORE any product edit. **Scheduler is not a session module** - `SchedulerTimer_Tick` (`MainWindow.StartStop.cs:601-637`) calls `StartEngine()` at `:618` and `StopEngine()` at `:628` from a 30s timer **while nothing is running**; it drives the engine from outside. **Haptics is app-scoped and never engine-started** (zero hits for `App.Haptics` in the start/stop file; 9193 lines/21 files; 99 sites/29 files; Buttplug ws://127.0.0.1:12345 or Lovense http://127.0.0.1:20010, both CLIENTS of a separate server). Four blockers, none of them the absent device. **D179: the thirteen ported modules are SILENT to a sink the shipping app drives from EIGHT sites in three of them** (`FlashService.cs:1453/1480/1516/1915`, `VideoService.cs:2580/4585/6580`, `SubliminalService.cs:230`) - previously recorded only at D68/D145/D161, none covering Flash or Video. **Visuals has NO service** - a settings page over Flash Images' draw dials the overlay already took as parameters. **Landed data-loss defect found and repaired:** SP-115's `_bouncingTextPreset` was in NONE of `StartAsync`/`LogIfDegraded`/`StopAsync`/`FlushAsync`, so Bouncing Text dials never loaded and never saved. **Sweep 60 mutations, 60 caught, ZERO survivors, no equivalence claim made anywhere.** `NOT COMPILED` added as its own sweep outcome, explicitly NOT gating rounds 1-4. D171-D179.
