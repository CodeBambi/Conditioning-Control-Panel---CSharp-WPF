...
- 2026-07-21 (worker, documented per worker discipline FR-WORK-06): **File Scope expanded by four wiring files the packet's own checkboxes are unsatisfiable without:** `client/src/CcpClient.Desktop/Program.cs` + `App.axaml.cs` (the `--dtrh-demo` bounded launch path — the boot matrix's "in-product" requirement has no other entry), `client/src/CcpClient.Desktop/Lifecycle/CompositionRoot.cs` (Step-2 checkbox "registered through the existing composition root" names this exact file — it is the ONLY registration site), `client/src/CcpClient.Desktop/Manifest/AssetManifest.cs` (Step-2 checkbox "`--verify-assets` green" for copied entries — the ONLY verifier; the SP-009 doc names this row's extension point). `fileScopeMustNotChange` untouched; no other files outside the original scope modified. Recorded in STATUS.md Discoveries + record.md; flagged to the pre-completion consult.
