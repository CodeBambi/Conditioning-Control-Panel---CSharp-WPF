...
## Step 3 — Windows evidence matrix (named observations; full detail in webview-dtrh-spike.md)

Worker bugs found+fixed during the runs (worker-owned, not payload/host findings): (a) `spike-run` message shape — spike.js reads video/image at TOP level of the message; my first send nested them under `assets` → vacuous `spike-done pass=true count=0` (recorded so "count=0" is never misread as a pass); (b) `Get()` arg helper threw on optional args; (c) one transient loopback `Start()` failure (process exited before loopback logging; bind retry loop now logs every bind failure and catches all exception types).

Highlights (each a named observation in the deliverable):
- spike.js M0 surface ALL PASS on the two-origin loopback: range-fetch 206 len=100 Content-Range `bytes 100-199/311752`; video-seek duration=1.0s landed=1.0s; webgl-video glError=0; webgl-image glError=0 + 2D getImageData untainted; spike-pointer x=632 y=369 (real SendInput click); input-keydown key=y AFTER a click.
- FOCUS: a synthesized key BEFORE any click never reached the page; after one real click, keys arrive. Matches the WPF host comment ("Keyboard focus does not land in the WebView2 child on a fresh launch until a click - claim it now" — WPF calls FocusWeb() at ready). Host row must claim focus explicitly.
- WEBAUDIO/AUTOPLAY: AudioContext starts `suspended`; unmuted play() rejects NotAllowedError without a gesture. Mitigation = WPF-parity Chromium flag `--autoplay-policy=no-user-gesture-required` (DtrhHostService.cs:120) via `WindowsWebView2EnvironmentRequestedEventArgs.AdditionalBrowserArguments` → re-run: `state=running`, unmuted play() resolved WITHOUT gesture. Module worker (`/dtrh/engine/gifWorker.js`, the payload's real worker path) constructed clean; WebAudio decoded a 20.88s mono bark via CORS fetch.
- FULLSCREEN: real F11 (after focus) → page `fullscreen-set on=True` → host WindowState=FullScreen → rendered 1920x1080 borderless capture. OFF-toggle NOT triggered by a second synthesized F11 (page-side semantics not exercised; round-trip belongs to the host row).
- EXIT: ESC held 1500ms (real keybd_event) → page `exit` at its 1.2s threshold → host Close → idempotent teardown → exit 0. exit-done not awaited (host row's watchdog owns that).
- BRIDGE ORDERING: `ready` arrives BEFORE NavigationCompleted; host queued init/manifest until ready (contract kept). preBuffer replay PROVEN: host sent `probe-buffered` pre-registration; bridge.js replayed it synchronously on late `on()` (probe.html + log).
- ROUTES: GET-only (POST → 411 framework-level before handler; recorded precisely), /dtrh/* overlay-first, traversal → 403 (encoded `..%2F`/`%2e%2e`; literal `..` is normalized away by curl client-side — recorded), unknown → 404, Range → 206/416, OPTIONS preflight → 204 `allow-headers: range`, `/dtrh/` → index.html default.
- FAILURE INJECTION ×3: (1) renderer kill — all 7 msedgewebview2 processes for the spike's UserDataFolder killed at t≈12–13s: web surface BLACK from capture (t≈24s), `AdapterDestroyed` NEVER fired; heartbeats continued ~28s post-kill (#16 t=32.7s) then STOPPED (20 beats at t=60.5s teardown ⇒ silence from t≈40.7s) — transient zombie (black pixels, live bridge) evolving to bridge silence, with no Avalonia-level fault event at any point. **Pre-completion consult correction: the Step-3 draft of this record said "continued uninterrupted" — beat counts disprove it; corrected here and in the evidence doc.** Detection implication routed to the host row (native ProcessFailed via TryGetPlatformHandle = documented immediate signal, not exercised); (2) blocked media route — first attempt: CORS-less 403 → fetch() throws TypeError → spike.js run() aborted SILENTLY (vacuous pass) — loopback-contract lesson: error responses on CORS-scoped origins MUST carry CORS headers; fixed (refusals now CORS-clean) → all 4 probes FAIL diagnosably (`status=403`), spike-done pass=false, page survives; (3) missing media — 404 → diagnosable FAILs, page survives.
- STARTUP/FRAMES: cold time-to-engine-live 1502ms first-ever run, 1080ms later run (profile warm); nav-completed ~750ms; steady-state rAF 360.4/361.4/363.0/363.2 fps across runs (WebGL-accelerated); no blanks/artifacts at boot/engine-live/fullscreen captures; black ONLY in the renderer-kill zombie state.
- Captures: index-boot-early.png (loader), index-engine2.png (Warren), index-fullscreen.png (1920x1080 borderless), after-renderer-kill.png (zombie black), probe-boot.png. (spike-results.png capture attempt raced auto-quit; bridge log holds the authoritative PASS lines.)

## Step 4 — WSL2 gate (Ubuntu 26.04, dotnet 10.0.110, WSLg)

- Native-dir copy: `~/ccp-sp011/client` via rsync from the worktree (bin/obj/scratch/.git excluded; NEVER /mnt/e for build/run — SP-005 pattern). Payload served READ-ONLY from the /mnt/e worktree path (never copied — packet).
- Natives via `wsl -u root` (no passwordless sudo — port-lessons): **WPE attempt: `apt-get install libwpewebkit-2.0-1` → "Unable to locate package"** (apt-cache search wpewebkit empty on Ubuntu 26.04). Installed instead: **libgtk-3-0t64 3.24.52-0ubuntu1, libwebkit2gtk-4.1-0 2.52.3-0ubuntu0.26.04.2**.
- Contract testCommand on WSL2: `dotnet build CcpClient.sln` 0W/0E; `CcpClient.Tests` 118/118; `CcpClient.HeadlessTests` 3/3; spike build 0W/0E; rc=0.
- Session facts: `WAYLAND_DISPLAY=wayland-0 DISPLAY=:0` (WSLg; X11 via XWayland — Wayland native NOT claimed, owner question §5.1 untouched).
- **Boot attempt 1 (embedded NativeWebView, WPE-default):** EnvironmentRequested arrived as `GtkWebViewEnvironmentRequestedEventArgs` — Avalonia AUTO-fell-back to WebKitGTK (no WPE present; `PreferWebKitGtkInstead` not even needed). AdapterCreated `Type = WebKitGtk, Engine = WebKit, Version = 2.52.3, IsSupported = True, IsInstalled = True, SupportedScenarios = NativeDialog`. Page navigated + fetched the FULL module graph over loopback (boot.js … three.module.min.js) — **but XGetImage capture shows the web content area DARK (never presents)** while Avalonia chrome renders. Precisely-diagnosed embedded non-boot on WSLg/X11: the auto-fallback GTK adapter declares NativeDialog-only scenarios and indeed does not present embedded content in this session. libEGL DRI3 warnings on stderr (software fallback).
- **Transport on WebKitGTK (probe page):** `window.chrome.webview` ABSENT; `invokeCSharpAction` = function; page→host via invokeCSharpAction WORKS (`probe-p2h-ica` arrived over WebMessageReceived); bridge.js `isHosted=false` → **the unchanged WebView2-shaped bridge CANNOT transport on Linux** — check3 SKIPPED honestly (plan-B shim is admit-row material; shim must exist BEFORE bridge.js module eval, so a transport-only `bridge.js` edit OR an early-injected shim is the admit row's documented choice).
- **Boot attempt 2 (NativeWebDialog, the adapter's declared scenario):** dialog window **RENDERS the DTRH loader for real** (XGetImage: "Opening the hole…" + spinner, 800x600 dedicated window). Constraint recorded: NativeWebDialog has NO InvokeScript → host→page messaging unavailable on the dialog path even with a shim; game boot (init/manifest) cannot complete there as-is.
- Budgets: Windows spike build cold 2.46s, Linux spike build 4.24s (incremental; Linux cold figure lost with /tmp churn — recorded honestly, systemd PrivateTmp wiped /tmp mid-step; artifacts moved to ~/ccp-sp011). Windows cold engine-live 1502ms (first run) / 1080ms (warm profile); Linux embedded non-presenting (N/A); dialog loader rendered at the 11s capture mark (bounded observation).
- Product suite stayed green throughout (contract base unchanged — quarantine holds).

## Step 5 — evidence, board reconciliation, consults

- Deliverable `client/docs/webview-dtrh-spike.md` written (named observation per acceptance item, package facts, restore outcome, origin/route shape, Linux diagnosis, budgets, Decisions-needed answers, explicit non-claims, artifact index).
- Board: spike row OPEN → **WIP** with evidence citing the doc; "Admit DTRH browser and origin design" STAYS BLOCKED; Decisions-needed gained the sharpened WPE/transport question (real-hardware Ubuntu 24.04 gate vs another WPE source).
