...
- FAILURE INJECTION ×3: (1) renderer kill — all 7 msedgewebview2 processes for the spike's UserDataFolder killed: web surface rendered BLACK, `AdapterDestroyed` NEVER fired, page heartbeats CONTINUED uninterrupted (zombie: bridge live, pixels dead) — renderer-process kill is NOT signaled through Avalonia events here and is invisible to a heartbeat watchdog; recovery/detection implication routed to the host row (native ProcessFailed via TryGetPlatformHandle is the documented handle, not exercised in the spike); (2) blocked media route — first attempt: CORS-less 403 → fetch() throws TypeError → spike.js run() aborted SILENTLY (vacuous pass) — loopback-contract lesson: error responses on CORS-scoped origins MUST carry CORS headers; fixed (refusals now CORS-clean) → all 4 probes FAIL diagnosably (`status=403`), spike-done pass=false, page survives; (3) missing media — 404 → diagnosable FAILs, page survives.
