...
| G5 | **`HonorRoll_DoesNotFireEarly_WhenProducersDisagreeAboutCase`** — `"sissy"`, `"Sissy"`, `"bambi"` ⇒ 2 distinct, NO award | ~~remove the entry normalisation **or** flip the comparer to `Ordinal` (either alone reds it)~~ — **WITHDRAWN, retro-corrected here after measurement.** Demos 1 and 2 disproved it: either layer ALONE still yields the right answer, so this fact reds only when **BOTH** regress (demo 3). The single-layer regressions are caught by G6/G7 and by the normalisation facts. `record.md` §4.2 and §4's matrix are the standing statement |
