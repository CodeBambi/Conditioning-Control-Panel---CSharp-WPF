...
test('an owned bundle reveals only its display and a granted demo hides its mockup',()=>{
 const {display,counter}=setup();display.apply({owned:['rt_bundle_2'],demo:true,racing:true},'rt_bundle_2');
 ...
 display.apply({owned:['rt_bundle_2'],demo:true,racing:true});assert.equal(materials[0].opacity,alpha,'refresh never restarts a reveal');
...
test('revocation mid-reveal and a later restored grant never leave a translucent cabinet',()=>{
 const {display}=setup();display.apply({owned:['rt_demo'],demo:true,racing:true},'rt_demo');display.update(.2,false);
 display.apply({owned:[],demo:false,racing:false});assert.equal(display.cabinet.visible,false);
 display.apply({owned:['rt_demo'],demo:true,racing:true});assert.equal(display.cabinet.position.y,0);
...
test('cleanup never disposes the borrowed prize meshes or source materials',()=>{
 ...
 display.apply({owned:['flashes_v2'],demo:false,racing:false},'flashes_v2');display.update(2,false);
...
test('purchased expansion travels to its shelf, sparkles expire, and motion off settles both',()=>{
 const {display,scene}=setup();display.apply({owned:['rt_bundle_1'],demo:true,racing:true},'rt_bundle_1');
 ...
 display.apply({owned:['rt_bundle_1'],demo:true,racing:true});assert.equal(scene.getObjectByName('prize_sparkles').count,0,'state refresh cannot replay particles');
 display.dispose();assert.equal(scene.getObjectByName('prize_sparkles'),undefined);
});

test('bundle one opens the cabinet without showing an unowned demo model',()=>{
 const {display,counter}=setup();display.apply({owned:['rt_bundle_1'],demo:false,racing:true});
 assert.equal(display.cabinet.visible,true);assert.equal(display.rack.getObjectByName('owned_rt_demo').visible,false);
 assert.equal(counter.getObjectByName('shelf_rt_demo').visible,true);display.dispose();
});
