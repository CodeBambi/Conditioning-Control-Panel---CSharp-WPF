...
export const LIFETIME_BOONS = [
  ...
  { id: 'vibe_popping', cat: 'skill', rankFloor: RANK.Slipping, name: 'VibePopping', glyph: '🔸',
    desc: 'press for a 5/6/8/10s buzz by level. while it buzzes, everything your cursor brushes over pops instantly, and live ones snap clean for full pay. 20s cooldown.',
    flavor: 'you don’t have to aim. just let the hand wander.',
    unlockCost: 400, upgradeCosts: [500, 800, 1200], levelValues: [5, 6, 8, 10],
    value: (v) => `${v.toFixed(0)}s buzz`,
    capstone: 'the buzz reaches twice as far.',
    activeUse: true, cooldownSec: 20 },
  { id: 'freeze_trigger', cat: 'skill', rankFloor: RANK.Slipping, name: 'Freeze Trigger', glyph: '❄',
    desc: 'press to freeze the whole field for 3.5s, exactly like a caught freeze bubble — fuses pause and everything stays poppable. 3 uses per grab.',
    flavor: 'stillness on demand. she lends it, never gives it.',
    unlockCost: 500, upgradeCosts: [800, 1300, 1800], levelValues: [3, 3, 3, 3],
  ...
  { id: 'the_wand', cat: 'skill', rankFloor: RANK.Slipping, name: 'The Wand', glyph: '🪄', webOnly: true,
    desc: 'press: for 2s the wand draws wherever your cursor goes — a shimmering trail that lingers 4 more seconds. every treat that touches it pops itself, and live fuses it grazes snap clean. 3/3/4 charges per grab by level.',
    flavor: 'draw the line. watch them cross it.',
    unlockCost: 600, upgradeCosts: [900, 1400], levelValues: [3, 3, 4],
  ...
  { id: 'the_pump', cat: 'skill', rankFloor: RANK.Slipping, name: 'The Pump', glyph: '🫙', webOnly: true,
    ...
    unlockCost: 500, upgradeCosts: [700, 1100], levelValues: [3, 4, 5],
    value: (v) => `${v.toFixed(0)}s suction`,
    activeUse: true, cooldownSec: 45 },
  ...
  { id: 'sticky_fingers', cat: 'accessory', rankFloor: RANK.Tempted, name: 'Sticky Fingers', glyph: '🍯', webOnly: true,
    desc: 'click and hold RIPS a gif right off the wall (and still grabs any drifting picture) to wield it as a paddle - it pops treats, snap-defuses lives and flings rabbits. Sticky Fingers pays a x1.2/x1.4 premium on the treats it sweeps by level; at level 3 they also leak gold droplets.',
    ...
    capstone: 'letting go HURLS the card down the tube — a treat shower follows the impact.' },
  ...
  { id: 'intrusive_thoughts', cat: 'accessory', rankFloor: RANK.Slipping, name: 'Intrusive Thoughts', glyph: '💭',
    desc: 'every 5 seconds a stray thought races across the screen for 6/8/10s by level, popping whatever it touches.',
    flavor: 'they aren’t yours. they pop things anyway.',
    unlockCost: 250, upgradeCosts: [350, 550], levelValues: [6, 8, 10],
  ...
  { id: 'drip_feed', cat: 'utility', rankFloor: RANK.Entranced, name: 'Drip Feed', glyph: '💧',
    desc: 'every treat popped and every trance snapped earns +1/+2/+3/+4 emotes by level — up to 60/90/120/150 a descent — collected when you surface.',
...
export const DIARY_CODEX = [
  ...
  { codex: 'bubble:golden', name: 'Lucky Bubble', glyph: '🍀', tint: '255,215,0',
    desc: 'A lucky bubble. Rare, quick, gone before you know it. Pop it for real gold, earned on the spot. Let it fade and your streak halves.' },
  ...
  { codex: 'pickup:condensation', name: 'Condensation', glyph: '💧', tint: '255,215,0',
    desc: 'Golden droplets bead on the tunnel wall and streak past. Click one before it\'s gone for a few emotes, earned when you surface. The walls sweat when you\'re close.' },
  ...
  { codex: 'material:silicone', name: 'Silicone', glyph: '🍮', tint: '255,214,222',
    desc: 'A material. Soft, patient, endlessly forgiving — it takes whatever shape you press it into. Grabs go straight to your bag; THE BOUDOIR turns pictures of it into things.' },
...
export const HOWTO_CARDS = [
  ...
  { title: 'What you keep', image: 'howto_5', lines: [
    { body: 'Every descent earns **XP** toward your normal level, plus two purses: **emotes ✦** collected when you surface, and **gold 🪙** snatched live during the fall.' },
