...
function Test-SmokeResult($Mode, $Code, [string]$Output, [string]$Errors) {
    ...
    if ($Mode -eq 'green') {
        return $Code -eq 0 -and $passes -eq 151 -and $fails.Count -eq 0 -and
    ...
    return $Code -eq 1 -and $passes -eq 127 -and $fails.Count -eq 18 -and
        ($fails -join "`n") -ceq ($expected -join "`n") -and $Output.TrimEnd().EndsWith('18 assertion(s) failed.')
}

if ($SelfCheck) {
    $ok = "  [PASS] URL prompt bootstrap has no desktop lifetime`n" + ("  [PASS] fixture`n" * 150) + 'Linux head can produce every value it renders.'
...
try {
    ...
    foreach ($entry in @(@('CCP.Avalonia', 'd7132141ba702f1a6205c8015d726dab44680c69'), @('CCP.Core', 'ec2d4c2a60bf3683ce7ec97c4d9e1c5c6d84b0c3'))) {
