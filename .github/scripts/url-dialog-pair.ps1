...
function Test-SmokeResult($Mode, $Code, [string]$Output, [string]$Errors) {
    ...
    if ($Mode -eq 'green') {
        return $Code -eq 0 -and $passes -eq 150 -and $fails.Count -eq 0 -and
    ...
    return $Code -eq 1 -and $passes -eq 126 -and $fails.Count -eq 18 -and
        ($fails -join "`n") -ceq ($expected -join "`n") -and $Output.TrimEnd().EndsWith('18 assertion(s) failed.')
}

if ($SelfCheck) {
    $ok = "  [PASS] URL prompt bootstrap has no desktop lifetime`n" + ("  [PASS] fixture`n" * 149) + 'Linux head can produce every value it renders.'
...
try {
    ...
    foreach ($entry in @(@('CCP.Avalonia', '51b1a2b7129a84686f8a4265f59afbf7d0c5f1c6'), @('CCP.Core', 'ec2d4c2a60bf3683ce7ec97c4d9e1c5c6d84b0c3'))) {
