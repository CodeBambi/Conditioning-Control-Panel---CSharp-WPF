...
function Test-SmokeResult($Mode, $Code, [string]$Output, [string]$Errors) {
    ...
    if ($Mode -eq 'green') {
        return $Code -eq 0 -and $passes -eq 115 -and $fails.Count -eq 0 -and
    ...
    return $Code -eq 1 -and $passes -eq 91 -and $fails.Count -eq 18 -and
        ($fails -join "`n") -ceq ($expected -join "`n") -and $Output.TrimEnd().EndsWith('18 assertion(s) failed.')
}

if ($SelfCheck) {
    $ok = "  [PASS] URL prompt bootstrap has no desktop lifetime`n" + ("  [PASS] fixture`n" * 114) + 'Linux head can produce every value it renders.'
...
try {
    ...
    foreach ($entry in @(@('CCP.Avalonia', 'c8010ae68541a5f158c74a085b87838630d18791'), @('CCP.Core', 'ec2d4c2a60bf3683ce7ec97c4d9e1c5c6d84b0c3'))) {
    ...
    foreach ($mode in 'red', 'green') {
        ...
        if ($testHash -ne '0f2c8938613800eb85853eabfd4d453ef66b93ce8ce7acdafcaa22fb6e840af4') { throw 'Test bytes changed' }
